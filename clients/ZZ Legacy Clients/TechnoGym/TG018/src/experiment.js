...
var TG018 = function () {
    ...
    var UCPoller = function () {
        ...
        poller(['.onestepcheckout-threecolumns.checkoutcontainer','.onestepcheckout-summary', function () {
            if (window.jQuery) {
                return true;
            }
        }], function () {
            JQ.getScript('//cdn.jsdelivr.net/jquery.slick/1.6.0/slick.min.js', init);
        });
    }();

    function init() {
        // Load in slick library

        fullStory('TG018', 'Variation 1');

        var cacheDom = function () {
            //Cache useful selectors for later use
            var bodyVar = JQ('body'),
                checkoutWrap = JQ('.onestepcheckout-threecolumns.checkoutcontainer'),
                minicartWrap = JQ('#cart-sidebar'),
                reviewTable = JQ('.onestepcheckout-column-right .onestepcheckout-summary'),
				prodLang = 'Product Price';

			if(window.location.pathname.indexOf('/it/') > -1){
				prodLang = 'Prezzo di prodotto';
			}

            // New elements to cache later on
            var prodWrap;

            bodyVar.addClass('TG018');

            //Retun the selectors we want to reference in other parts of the test
            return {
                bodyVar: bodyVar,
                checkoutWrap: checkoutWrap,
        ...
        var productSlider = {
            // Create the wrap for the new product blocks
            contentBuilder: function contentBuilder() {
                cacheDom.prodWrap = JQ('\n\t\t\t\t\t<div class="TG018_product-wrap clearfix">\n\t\t\t\t\t</div>\n\t\t\t\t');
                cacheDom.checkoutWrap.prepend(cacheDom.prodWrap);
            },
            /* 
            	Go through each item in the basket
            	Store relevent fields in variables to pass into the appendProductToWrap function
            	find the relevent row on the visible tabel to get the price so it matches, (mini basket includes delivery so this is to prevent confusion)
            	If the row has an image then run function, this is to stop putting unlisted items in the slider (POLAR SENSOR+STRAP for example)
            */
            productBagLoop: function productBagLoop() {
                cacheDom.minicartWrap.find('> li.item').each(function () {
                    var el = JQ(this),
                        elIndex = el.index(),
                        elNameWrap = el.find('td.name'),
                        elName = JQ.trim(elNameWrap.find('.product-name a').text()),
                        tableRow = cacheDom.reviewTable.find('tbody tr').eq(elIndex),
                        elPrice = tableRow.find('.total .price').text(),
                        elVersion = '',
                        elImg = '';

                    if (hardcodedImages[elName]) {
                        if (tableRow.find('.onestepcheckout-custom-options').length > 0) {
                            elVersion = JQ.trim(tableRow.find('.onestepcheckout-custom-options').text());
                        }
                        elImg = hardcodedImages[elName].img;
                        appendProductToWrap(elImg, elName, elVersion, elPrice);
                    }
                });
            }
        };
        ...
        // Append slide to the slideshow wrap before init
        var appendProductToWrap = function () {
            return function (imgSrc, productName, productVersion, priceTotal) {
                // If the items have a version make sure they get the version field
                if (productVersion === '') {
                    cacheDom.prodWrap.append(['<div class="TG018_product-block">', '<div class="TG018_inner-slide clearfix">', '<div class="TG018_inner-img" style="background-image: url(\'' + imgSrc + '\');"></div>', '<div class="TG018_inner-content">', '<h3>' + productName + '</h3>', '<strong>' + cacheDom.prodLang + ': ' + priceTotal + '</strong>', '</div>', '</div>', '</div>'].join(''));
                } else {
                    cacheDom.prodWrap.append(['<div class="TG018_product-block">', '<div class="TG018_inner-slide clearfix">', '<div class="TG018_inner-img" style="background-image: url(\'' + imgSrc + '\');"></div>', '<div class="TG018_inner-content">', '<h3>' + productName + '</h3>', '<p>' + productVersion + '</p>', '<strong>' + cacheDom.prodLang + ': ' + priceTotal + '</strong>', '</div>', '</div>', '</div>'].join(''));
                }
            };
        }();

        //Build slider wrap
        productSlider.contentBuilder();
        //Create slides
        productSlider.productBagLoop();
        //Init slideshow
