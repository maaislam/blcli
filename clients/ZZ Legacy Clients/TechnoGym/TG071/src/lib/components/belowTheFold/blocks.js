 ...
 */
export default () => {
  const blockSectionContainer = document.querySelector('.TG071-blocks');
  const iconSection = document.querySelector('.TG071-yellowIcons');
  ...
  const blocks = {
    ...
    [`${__('MYRUN APP')}`]: {
      description: `${__('Download the MyRun App on your device, position it on the console and begin a running experience that will improve your lifestyle.')} <div class="TG071-secondary_button TG071-button TG071-app_button"><a target="_blank" href="https://www.technogym.com/gb/wellness/myrun-app/">${__('Discover the App')}</a></div>`,
      background: '//cdn.optimizely.com/img/8355110909/85df0d1a602e48519424ad69f0512b26.jpg',
    },
    [`${__('Open Challenge')}`]: {
      description: `${__('Train and compete with other runners from around the world with Zwift* or run through epic landscapes and make your solo runs even more enjoyable.')} <p class="TG069_disclaimer">${__('*MyRun is compatible with Zwift only in the iPad version (third generation or more recent). Android versions are not currently supported.')}</p><div class="TG071-apps"><span class="TG071-logo_app TG071-swift_app"></span></div>`,
      background: '//cdn.optimizely.com/img/8355110909/e74c2ee084fa4426891e8740e669c5f6.jpg',
    },
    [`${__('Outdoor running indoors')}`]: {
      description: `${__('With MYRUNNING LOGBOOK, take outdoor running indoors. Track your outdoor sessions and relive them at home.')}<br></br> ${__('Compatible with Garmin and Strava apps.')}<div class="TG071-apps"><span class="TG071-logo_app TG071-garva_app"></span><span class="TG071-logo_app TG071-strava_app"></span></div>`,
  ...
  Object.keys(blocks).forEach((i) => {
    ...
    blockSection.classList.add('TG071-block_row');
    blockSection.innerHTML = `
    <div class="TG071-block_text_wrap" style="background-image:url('${element.background}')">
      <div class="TG071-block_text">
        <h2>${[i][0]}</h2>
  ...
  */
  ...
  Object.keys(yellowContent).forEach((i) => {
    ...
    iconContent.classList.add('TG071-block_icons');
    iconContent.innerHTML = `
    <div class="TG071-yellow_icon" style="background-image: url('${element.icon}')"></div>
    <h3>${[i][0]}</h3>
    <p>${element.text}</p>`;
    iconSection.appendChild(iconContent);
  });
  document.querySelector('.TG071-block_row:nth-child(4)').setAttribute('id', 'TG071-app');
};
