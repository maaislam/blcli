...
export default () => {
  const specContainer = document.querySelector('.TG071-specification');
  const faqSection = document.createElement('div');
  faqSection.classList.add('TG071-faqs');
  faqSection.classList.add('TG071-container');
  faqSection.innerHTML = `<h2>${__('FAQ\'s')}</h2><div class="TG071-faq_questions"></div><div class="TG071-faqs_all"><div class="TG071-button TG071-secondary_button"><a target="_blank"href="${__('https://www.technogym.com/gb/myrun-faqs/')}">${__('View All FAQ\'s')}</a></div></div>`;
  ...
  Object.keys(allFaqs).forEach((i) => {
    ...
    faqQuestion.classList.add('TG071-faq_question');
    faqQuestion.innerHTML = `<h4>${[i][0]}</h4><p>${data}</p>`;
    document.querySelector('.TG071-faq_questions').appendChild(faqQuestion);
  });


  // On click of the FAQs
  const faqQuestion = document.querySelectorAll('.TG071-faq_question');

  const showAnswer = (e) => {
    e.preventDefault();
    const active = document.querySelector('.TG071_faq-active');
    if (active) {
      active.classList.remove('TG071_faq-active');
    }
    // close or show based on whether the question is currently active
    if (e.currentTarget === active) {
      e.currentTarget.classList.remove('TG071_faq-active');
    } else {
      e.currentTarget.classList.add('TG071_faq-active');
    }
  };
