...
export default () => {
  ...
  const titleText = document.querySelector('.TG071-textContent');
  const arrow = document.querySelector('.TG071-bottomArrow');
  const playVideoButton = document.querySelector('.TG071-play_video');
  /**
  ...
  const createVideo = () => {
    ...
    productVideo.id = 'TG071-videoPlayer';
    ...
    // add video the page
    const video = document.querySelector('.TG071-video');
   ...
   */
  const runVideo = () => {
    pollerLite(['.TG071-video video'], () => {
      const video = document.querySelector('.TG071-video video');
      titleText.classList.add('TG071-hide_text');
      video.play();
    });
  };
  ...
  */
  const removeTheVideo = () => {
    const video = document.querySelector('#TG071-videoPlayer');
    titleText.classList.remove('TG071-hide_text');
    video.remove();
  };
  ...
  const arrowScroll = () => {
    ...
    if (settings.VARIATION === '1') {
      overView = document.querySelector('#TG071-overview');
    } else if (settings.VARIATION === '2') {
      overView = document.querySelector('#TG071-mainProduct');
    }
  ...
  */
  const videoEnded = () => {
    pollerLite(['#TG071-videoPlayer'], () => {
      document.querySelector('#TG071-videoPlayer').addEventListener('ended', () => {
        arrowScroll();

        document.querySelector('.TG071-textContent').classList.remove('TG071-hide_text');
        document.querySelector('#TG071-videoPlayer').remove();
      });
    });
  };

  pollerLite(['.TG071-textContent', '.TG071-play_video'], () => {
    ...
    arrow.addEventListener('click', () => {
      if (document.querySelector('.TG071-video video')) {
        removeTheVideo();
      }
    });
  });
