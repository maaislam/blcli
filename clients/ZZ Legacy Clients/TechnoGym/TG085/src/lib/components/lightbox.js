...
export default class Lightbox {
  ...
  bindEvents() {
    ...
    const zwiftImage = document.querySelector(`.${cache.ID}_zwiftSection`);
    zwiftImage.addEventListener('click', () => {
      this.open();
    });
    ...
    // on click of the download and request button
    const form = document.querySelector('.TG082_form');
    cache.component.querySelector(`.${cache.ID}-lightboxCTA`).addEventListener('click', () => {
      this.close();
