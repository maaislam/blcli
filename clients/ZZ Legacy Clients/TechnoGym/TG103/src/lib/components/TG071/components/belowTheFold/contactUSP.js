...
export default () => {
  const contactSection = document.querySelector('.TG103-contact');
  ...
  uspSection.innerHTML = `<div class="TG103-runnerImage"></div><ul class="TG103-usps_wrapper"><h2>${__('Why Buy From')} <span>technogym</span></h2></ul><div class="TG103-readMore"><span>${__('Read more')}</span></div>`;
  ...
  const readMore = uspSection.querySelector('.TG103-readMore');
  readMore.addEventListener('click', () => {
    if (uspSection.classList.contains('TG103-all_showing')) {
      uspSection.classList.remove('TG103-all_showing');
      readMore.querySelector('span').textContent = `${__('Read more')}`;
    } else {
      uspSection.classList.add('TG103-all_showing');
