...
export default class Lightbox {
  ...
  bindEvents() {
    ...
    // Open the form on click of any contact buttons
    const contactButtons = document.querySelectorAll(`.${cache.ID}-email.${cache.ID}-button`);
    for (let index = 0; index < contactButtons.length; index += 1) {
      const element = contactButtons[index];
      element.addEventListener('click', () => {
        this.open();
        appState.downloadBrochure = 'no';
      });
    }

    // open the form on click of any of the download brochure clicks
    const downloadButtons = document.querySelectorAll(`.${cache.ID}-download.${cache.ID}-button`);
  ...
  render() {
    ...
    document.querySelector(`.${cache.ID}-lightboxForm`).appendChild(contactForm);
  }
