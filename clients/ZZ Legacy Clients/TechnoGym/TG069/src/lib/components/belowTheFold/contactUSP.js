...
export default () => {
  const contactSection = document.querySelector('.TG069-contact'); // UP TO HERE TRANSLATIONS
  ...
  <div class="TG069-text_container"> 
    ...
    <div class="TG069-button TG069-secondary_button"><a target="_blank" href="${__('https://www.technogym.com/gb/contacts/?reason=call&sku=MYRUN-CONFIGURABLE-EUROPE&type=conf')}">${__('Speak to A Wellness Expert Now')}</a></div>
    <span></span>
  </div>`;


  /* USP section */
  const uspSection = document.querySelector('.TG069-section.TG069-usps');
  uspSection.innerHTML = `<div class="TG069-runnerImage"></div><ul class="TG069-usps_wrapper"><h2>${__('Why Buy From')} <span>technogym</span></h2></ul>`;
  ...
  for (let index = 0; index < usps.length; index += 1) {
    ...
    newUsp.classList.add('TG069-usp');
    newUsp.innerHTML = `${element}`;
    uspSection.querySelector('.TG069-usps_wrapper').appendChild(newUsp);
  }
};
