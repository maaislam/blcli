...
export default () => {
  const newProductSection = document.querySelector('.TG069-mainProduct');
  ...
  const createMainProduct = () => {
    ...
    <div class="TG069-product"></div>`;
    document.querySelector('.TG069-product').appendChild(mainProduct);
    ...
    addButton.classList.add('TG069-primary_button');
    addButton.classList.add('TG069-button');
    finalAddButton.classList.add('TG069-primary_button');
    finalAddButton.classList.add('TG069-button');
  ...
  const colourContainer = () => {
    ...
    newOptionsContainer.classList.add('TG069-colour_options');
    newOptionsContainer.innerHTML = `<h4>${__('Choose your Version')}:</h4><div class="TG069-inner_options"></div>`;
  ...
  const addOptions = () => {
    ...
    for (let index = 0; index < options.length; index += 1) {
      ...
      newOption.classList.add('TG069-option');
      newOption.innerHTML = `
      <div data-match="${element.id}" class="TG069-colour">
        <div class="TG069-colour_image" style="background-image: url('${colourImage}')"></div>
        <p>${colourText}</p>
      </div>`;
      document.querySelector('.TG069-inner_options').appendChild(newOption);
    }
  };

  /* On click of the new options */
  const selectOptions = () => {
    const newOptions = document.querySelectorAll('.TG069-inner_options .TG069-colour');

    const handleClick = (e) => {
      e.preventDefault();
      const active = document.querySelector('.TG069-option_selected');
      if (active) {
        active.classList.remove('TG069-option_selected');
      }
      e.currentTarget.classList.add('TG069-option_selected');
  ...
  const includesSection = () => {
    ...
    includesContent.classList.add('TG069-includes_section');
    ...
    Object.keys(includesText).forEach((i) => {
      ...
      includedText.classList.add('TG069-included_text');
    ...
    bottomText.classList.add('TG069-finance_text');
    ...
    document.querySelector('.TG069-includes_section').insertAdjacentElement('afterend', bottomText);

    /*const closeFinance = document.querySelector('.TG069-closeFinance');
  ...
  const errorMessage = () => {
    ...
    errorText.classList.add('TG069-error');
    ...
    const activeColour = document.querySelector('.TG069-colour');
    nextButton.addEventListener('click', () => {
      if (!activeColour.classList.contains('TG069-option_selected')) {
        nextButton.parentNode.insertAdjacentElement('afterend', errorText);
      } else {
        const errorMessageOnPage = document.querySelector('.TG069-error');
  ...
  const changeButtonOnBuy = () => {
    const buyOnlineButton = document.querySelector('.TG069-cta .TG069-primary_button.TG069-button');
    ...
    const stickyNav = document.querySelector('.TG069-sticky_nav_wrapper .TG069-cta');
    stickyNav.insertAdjacentElement('afterend', navBasketIcon);

    const checkoutLink = document.querySelector('.TG069-sticky_nav_wrapper .mini-cart.items-in-cart a');
  ...
  const changeAddToCheckout = () => {
    ...
    addedToBagMessage.classList.add('TG069-added');
    ...
    if (checkoutButton.style.display !== 'none') {
      ...
      checkoutButton.querySelector('.button').classList.add('TG069-primary_button');
      checkoutButton.querySelector('.button').classList.add('TG069-button'); // add classes to checkout button

      // add added to bag message
      if (!document.querySelector('.TG069-added')) {
  ...
  const moveNewProductSection = () => {
    ...
    document.querySelector(`#TG069-mainProduct`).appendChild(mainproductArea);
