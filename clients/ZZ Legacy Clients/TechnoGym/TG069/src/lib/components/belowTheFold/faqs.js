...
export default () => {
  const specContainer = document.querySelector('.TG069-specification');
  const faqSection = document.createElement('div');
  faqSection.classList.add('TG069-faqs');
  faqSection.classList.add('TG069-container');
  faqSection.innerHTML = `<h2>${__('FAQ\'s')}</h2><div class="TG069-faq_questions"></div><div class="TG069-faqs_all"><div class="TG069-button TG069-secondary_button"><a target="_blank"href="${__('https://www.technogym.com/gb/myrun-faqs/')}">${__('View All FAQ\'s')}</a></div></div>`;
  ...
  Object.keys(allFaqs).forEach((i) => {
    ...
    faqQuestion.classList.add('TG069-faq_question');
    faqQuestion.innerHTML = `<h4>${[i][0]}</h4><p>${data}</p>`;
    document.querySelector('.TG069-faq_questions').appendChild(faqQuestion);
  });


  // On click of the FAQs
  const faqQuestion = document.querySelectorAll('.TG069-faq_question');

  const showAnswer = (e) => {
    e.preventDefault();
    const active = document.querySelector('.TG069_faq-active');
    if (active) {
      active.classList.remove('TG069_faq-active');
    }
    // close or show based on whether the question is currently active
    if (e.currentTarget === active) {
      e.currentTarget.classList.remove('TG069_faq-active');
    } else {
      e.currentTarget.classList.add('TG069_faq-active');
    }
  };
