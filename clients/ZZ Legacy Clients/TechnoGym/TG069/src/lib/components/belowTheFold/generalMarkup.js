...
export default () => {
  ...
  const mainContent = () => {
    let sections;
    const header = document.querySelector('.TG069-topContainer');
    ...
    belowFold.classList.add('TG069-belowFold_wrapper');
    ...
    for (let index = 0; index < sections.length; index += 1) {
      ...
      section.classList.add('TG069-section');
      section.classList.add(`TG069-${element}`);
      section.id = `TG069-${element}`;
  ...
  const moveNewsroom = () => {
    pollerLite(['.TG040-related-posts'], () => {
      const articleSection = document.querySelector('.TG069-article');
