...
export default class CustomiseBoxes {
  ...
  bindEvents() {
    ...
    const selectBoxes = document.querySelectorAll('local-product-custom-options .pos-relative.m-b .select option');
    ...
    for (let index = 0; index < allBoxes.length; index += 1) {
      ...
      addEventListener(element, 'click', (e) => {
        ...
        const customError = document.querySelector(`.${ID}_custom_error`);
        if(customError) {
  ...
  render() {
    const { component } = this;
    const productImage = document.querySelector('local-product-custom-options .p-t-3 [name*="choose type"] label');
    if(productImage) {
      productImage.insertAdjacentElement('afterend', component);

      // if the input box is empty, stop click on buy now
      addEventListener(document.querySelector(`.${ID}_customBox.${ID}_personalised`), 'click', () => {
        const inputPersonal = document.querySelector('[ng-switch-when="field|area"] input');
        addPoller([inputPersonal, `.${ID}-ctaButtons .${ID}-buyButton`], () => {
          const addToBagNew = document.querySelector(`.${ID}-ctaButtons .${ID}-buyButton`);
          if (inputPersonal.value === '') {
            addToBagNew.classList.add(`${ID}-disabled`);
          }
          addEventListener(inputPersonal, 'keyup', () => {
            if (inputPersonal.value === '') {
              addToBagNew.classList.add(`${ID}-disabled`);
            } else {
              addToBagNew.classList.remove(`${ID}-disabled`);
            }
          });
        });
      });
      ...
      addPoller([`.${ID}-ctaButtons .${ID}-buyButton`], () => {
        const addToBagNew = document.querySelector(`.${ID}-ctaButtons .${ID}-buyButton`);
        addEventListener(document.querySelector(`.${ID}_customBox`), 'click', () => {
          if (addToBagNew.classList.contains(`${ID}-disabled`)) {
            addToBagNew.classList.remove(`${ID}-disabled`);
          }
        });
      });
