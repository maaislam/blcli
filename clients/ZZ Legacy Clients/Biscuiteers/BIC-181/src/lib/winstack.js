...
export const stack = experiments[shared.ID] = experiments[shared.ID] || {};
 ...
 * @param {object} poller
 */
export const addPoller = (conditionsArray, callbackFunction, shared = {}) => {
  const p = poller(conditionsArray, callbackFunction, shared);
...
export const destroyPollers = () => {
  if((window.UC.experiments[shared.ID] || {}).pollers) {
    const pollers = window.UC.experiments[shared.ID].pollers;
    for(let i = 0; i < pollers.length; i++) {
        pollers[i].destroy();
    }

    window.UC.experiments[shared.ID].pollers = [];
  }
};
...
export const destroyIntervals = () => {
  if((window.UC.experiments[shared.ID] || {}).intervals) {
    const intervals = window.UC.experiments[shared.ID].intervals;
    for(let i = 0; i < intervals.length; i++) {
        clearInterval(intervals[i]);
    }

    window.UC.experiments[shared.ID].intervals = [];
  }
};
...
export const killAllEventListeners = () => {
  ...
  window.UC.experiments[shared.ID].eventListeners = [];
};
...
export const killObservers = () => {
  ...
  window.UC.experiments[shared.ID].observers = [];
};
