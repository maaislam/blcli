...
export default () => {
  ...
  const newBuyNowButton = document.querySelector('.BI039-ctaButtons .BI039-button.BI039-buyButton');
  const deliveryButton = document.querySelector('.BI039-ctaButtons .BI039-button.BI039-delivery');

  const deliverySlideTab = document.getElementById('BI039-deliveryTab');
  const buySlideTab = document.getElementById('BI039-buyNow');
    ...
    if (!addToBagButton.classList.contains('is-disabled')) {
      ...
      poller([
        ...
        timeoutCallback: () => {
          ...
          document.body.classList.add('BI039-no_scroll');
  ...
  const deliveryButtonClick = () => {
    ...
    if (document.querySelector(`.${ID}_custom_error`)) {
      ...
      if (!document.querySelector('.BI039-active_box')) {
        ...
        document.querySelector('.input-wrap-1 select').classList.add('BI039-error');
      } else {
        document.querySelector('.input-wrap-1 select').classList.remove('BI039-error');
  ...
  const closeDelivery = () => {
    ...
    tabClose.addEventListener('click', () => {
      ...
      document.body.classList.remove('BI039-no_scroll');
      pageHTML.style = 'overflow-y: auto';
      document.querySelector('.BI039-deliver_text').remove();
    });
  };
  ...
  deliveryButton.addEventListener('click', () => {
    ...
    addPoller(['.BI039-occasion_content product-when-your-occasion'], () => {
      calanderTextUpdate();
    });
  });
};
