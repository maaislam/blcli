...
export const moveOccasionToTab = () => {
  ...
  const deliveryTabOccasion = document.querySelector('.BI039-occasion_content');
...
export const calanderTextUpdate = () => {
  ...
  const occasionInDeliverySlide = document.querySelector('.BI039-occasion_content product-when-your-occasion');
  const newDeliveryText = document.createElement('div');
  newDeliveryText.classList.add('BI039-deliver_text');
  ...
  const occasionBox = document.querySelector('.BI039-occasion_content');
  addObserver([occasionBox.querySelector('input')], () => {
    addPoller(['.BI039-occasion_content .bg-col-15 .fw-bold.block'], () => {
      ...
      document.querySelector('.BI039-deliver_text span').textContent = deliveryDate;

      dataLayer.push({
        'event': 'BI039 Date Selected',
