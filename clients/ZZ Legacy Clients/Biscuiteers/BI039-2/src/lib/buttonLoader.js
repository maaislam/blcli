...
export default () => {
  ...
  for (let index = 0; index < allCTAButtons.length; index += 1) {
    ...
    element.addEventListener('click', (e) => {
      ...
      if(!currenttarget.classList.contains('BI039-2-disabled')) {

      currenttarget.querySelector(`.${ID}-buttonLoader`).classList.add(`${ID}-loader_show`);

        // if the upsell ctas are clicked
        if (currenttarget.classList.contains('BI039-2-upsell_buy')) {
          timeoutTime = 3000;
        // if the buy now in delivery is clicked
        } else if (currenttarget.classList.contains('BI039-2-deliveryBuybutton')) {
          timeoutTime = 5000;
        } else {
          timeoutTime = 3000;
        }
