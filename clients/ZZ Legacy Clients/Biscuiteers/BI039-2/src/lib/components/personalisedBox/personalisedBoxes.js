...
export default class CustomiseBoxes {
  ...
  render() {
    ...
    document.querySelector('.BI039-2_customBox.BI039-2_personalised').addEventListener('click', () => {
      const inputPersonal = document.querySelector('[ng-switch-when="field|area"] input');
      pollerLite([inputPersonal, '.BI039-2-ctaButtons .BI039-2-buyButton'], () => {
        const addToBagNew = document.querySelector('.BI039-2-ctaButtons .BI039-2-buyButton');
        if (inputPersonal.value === '') {
          addToBagNew.classList.add('BI039-2-disabled');
        }
        inputPersonal.addEventListener('keyup', () => {
          if (inputPersonal.value === '') {
            addToBagNew.classList.add('BI039-2-disabled');
          } else {
            addToBagNew.classList.remove('BI039-2-disabled');
          }
        }, false);
      });
    });
    ...
    pollerLite(['.BI039-2-ctaButtons .BI039-2-buyButton'], () => {
      const addToBagNew = document.querySelector('.BI039-2-ctaButtons .BI039-2-buyButton');
      document.querySelector('.BI039-2_customBox').addEventListener('click', () => {
        if (addToBagNew.classList.contains('BI039-2-disabled')) {
          addToBagNew.classList.remove('BI039-2-disabled');
        }
      });
    });
