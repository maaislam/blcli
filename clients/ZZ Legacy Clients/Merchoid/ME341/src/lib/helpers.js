...
export const newProductPrice = () => {
  ...
  const recPrices = document.querySelectorAll('.section-content.products.wrapper.grid.products-grid.brand-section-grid .item.product.product-item.brand-section-item');
  if(recPrices) {
    for (let index = 0; index < recPrices.length; index += 1) {
      const element = recPrices[index];
      const oldPrice = element.querySelector('.price-container.price-final_price');
      const newRecPrice = calculateNewPrice(oldPrice);
      oldPrice.insertAdjacentHTML('beforebegin', `<div class="${ID}-new-price"><span>${siteCurrency}${newRecPrice}</span></div>`);
    }
  }
}
  ...
  if(document.querySelector('#coupon_code').value === voucher) {
    ...
    for (let index = 0; index < allProducts.length; index++) {
      ...
      if(regex.test(elName)) {
        ...
        jumperPrice.push(newPrice.toFixed(2));
      }
    }
