   ...
   */
   const bottomContent = () => {
    ...
    if(shared.VARIATION === '1') {
      message = 
      `<div class="${ID}-priceMessages">
        <span>Why not <b>treat yourself</b> to this officially licensed ${productName.textContent.trim().replace(/^[^_]*:/, '')}</span>
      </div>`;
    ...
    if(shared.VARIATION === '2') {
      message = 
      `<div class="${ID}-priceMessages">
        <span>Imagine how good you’re going to feel with this Officially Licensed ${productName.textContent.trim().replace(/^[^_]*:/, '')}. <b>Go on, treat yourself!</b></span>
      </div>`;
