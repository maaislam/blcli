...
export default class MobileNavigation {
    ...
    bindEvents() {
      ...
      const navToggle = document.querySelector('.header.content .action.nav-toggle');
       ...
       // make brands go back to default styles
      const revertBrands = () => {
        const brandsNav = document.querySelector(`.${ID}_brands`);

        if(brandsNav.querySelector(`.${ID}-allBrandsContainer`).classList.contains(`${ID}_allBrands_active`)) {
          const brandButton = document.querySelector(`.${ID}_allBrands`);
          brandsNav.querySelector(`.${ID}-allBrandsContainer`).classList.remove(`${ID}_allBrands_active`);
          brandButton.style.display = 'block';

        }
        if(brandsNav.querySelector(`.${ID}_secondLevel_active`)){
          const topPos = brandsNav.offsetTop;
          scrollToElement(document.querySelector(`.${ID}_brands .${ID}_lvl2Links`), topPos, 0);
        }
      }
