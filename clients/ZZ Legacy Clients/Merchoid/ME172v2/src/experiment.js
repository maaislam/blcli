...
const Run = () => {
  ...
  const Exp = {
    ...
    init: () => {
      ...
      if (Exp.cache.sizeSelect.querySelector('option[value="l"]').textContent.indexOf('39-40"') > -1) {
    ...
    components: {
      ...
      render() {
        ...
        const addtoBag = bodyVar.querySelector('.single_add_to_cart_button');
        Exp.cache.sizeSelect.classList.add('ME172_disabled');
        Exp.cache.sizeSelect.setAttribute('disabled', 'disabled');
        addtoBag.setAttribute('disabled', 'disabled');

        selectWrap.insertAdjacentHTML('afterbegin', `
          <div class="ME172_select-wrap">
            <p>Please select a gender before choosing a size</p>
            <select class="ME172_select-gender">
              <option class="ME172_def" value="select">Please select a gender...</option>
        ...
        bodyVar.querySelector('.woocommerce-variation-add-to-cart').insertAdjacentHTML('beforeend', `<a class="ME172_fake-btn"></a>`);
      },
      changeEvent() {
        const genderWrap = bodyVar.querySelector('.ME172_select-wrap')
        const genderSelect = bodyVar.querySelector('.ME172_select-gender');
        const addtoBag = bodyVar.querySelector('.single_add_to_cart_button');
        const addToBagWrap = bodyVar.querySelector('.ME172_fake-btn');

        addToBagWrap.addEventListener('click', () => {
          if (addtoBag.getAttribute('disabled')) {
            genderWrap.classList.add('ME172_error');
          }
        });
        ...
        genderSelect.addEventListener('change', () => {
          ...
          if (active !== 'select') {
            const firstOption = genderSelect.querySelector('option.ME172_def');
            if (firstOption) {
              addtoBag.removeAttribute('disabled');
              firstOption.parentNode.removeChild(firstOption);
              Exp.cache.sizeSelect.classList.remove('ME172_disabled');
              Exp.cache.sizeSelect.removeAttribute('disabled');
              genderWrap.classList.remove('ME172_error');
              bodyVar.classList.add('ME172_hide');
            }
          }
