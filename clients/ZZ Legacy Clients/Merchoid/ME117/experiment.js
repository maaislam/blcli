var _ME117 = (function($) {
    ...
    // Use flexbox if browser supports it for a smoother animation on brands
    // Else use fallback
    var doc = document.body || document.documentElement;
    var style = doc.style;

    if (!style.webkitFlexWrap == '' || !style.msFlexWrap == '' || !style.flexWrap == '' ) {
      $('body').addClass('ME117_no-flex-support');
    }
        ...
        '<div class="ME117_brandText">Search over <span>3,000</span> pieces of <span>Officially Licensed Geek Merch!</span></div>',		
        '<div class="ME117_brandTitle">Start by selecting your favourite brand(s).</div>',
      ...
      $('.textwidget').hide();
      var _ROW = '<div class="search-again-wrap"><span class="search-again">Search Again ></span></div>';
      $(_ROW).insertBefore('.entry-header.text-center');	
		...
		var menuToggle = (function() {
			...
			$options.each(function(i) {
				...
				$el.attr('data-filters-selected', '0');
				...
				var openMenu = function() {
					...
					$selectedOpts.find('#ME117_selected-opts__num').text($el.attr('data-filters-selected'));
				};
				...
				function removeFilter($opt) {
					...
					var filterCount = $el.attr('data-filters-selected');
					var newFilterCount = parseInt(filterCount) - 1;
					$el.attr('data-filters-selected', newFilterCount);
					
					if ($selectedOpts.length) {
						// Hide selected opts if visible
            if (newFilterCount === 0 && $selectedOpts.hasClass('ME117_show-selected-opts')) {
						  $selectedOpts.removeClass('ME117_show-selected-opts');
						}

						$selectedOpts.find('#ME117_selected-opts__num').text(newFilterCount);
					}
				...
				function addFilter($opt) {
					...
					if ($selectedOpts.length) {
						...
						if (!$selectedOpts.hasClass('ME117_show-selected-opts')) {
							$selectedOpts.addClass('ME117_show-selected-opts');
						}
		...
		$clear.click(function() {
			...
			$('[data-filters-selected]').attr('data-filters-selected', '0');
  ...
  //     var $noResultsFound = $([
  //       '<div class="ME117_no-results">',	
  //         'Sorry, we couldn\'t find any products using the filters you selected. Please try again with different selections.',
  //       '</div>'
  //     ].join(''));

  //     $('.ME117_prod-finder').removeClass('hidden').before($noResultsFound);
  //   }
  // })();

  /*AMEND*/
  var $brandBox = $('.ME117_opt.ME117_brands');
  $brandBox.click(function(){
    if ($(this).hasClass('ME117_opt--active')) {
      $('html, body').animate({
        scrollTop: $brandBox.offset().top
      }, 500);
    }
  });
