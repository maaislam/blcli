...
const ME131 = (() => {
    ...
    const activate = () => {
		...
		const $productTitle = $('.mobile-target-product-title').text().trim(),
		...
		const addBar = () => {
			...
			$('.ME131-exit').click(function(){
				localStorage.setItem('ME131-exit',1);
			...
			const conditions = () =>{
				...
				if(!utils.getCookie('ME131-landing')){
					...
					utils.setCookie('ME131-landing',currentproductTitle);
					
				}else if(!utils.getCookie('ME131-first') && utils.getCookie('ME131-landing')){ //If the product is the first visit to product page		
					...
					utils.setCookie('ME131-first',1);

				}else if(!utils.getCookie('ME131-second') && utils.getCookie('ME131-first')){ //If the product is the second visit to product page
					...
					utils.setCookie('ME131-second',1);

				}else if(!utils.getCookie('ME131-third') && utils.getCookie('ME131-second')){ //if it's the third visit and based on users location
					const location = window["optimizely"].get("visitor").location.country;
					if(location != 'undefined'){
						if(location === 'US'){
							messageText.text(allContent.thirdvisit.allUS);
						}else{
							messageText.text(allContent.thirdvisit.allUK);
						}
					}else{
						$bar.addClass('ME131-notext');
					}
					utils.setCookie('ME131-third',1);
				}
			}
			conditions();
		
			//if all cookies have been added, show the message that will stay
			if(utils.getCookie('ME131-landing') && utils.getCookie('ME131-second') && utils.getCookie('ME131-first')){
				const location = window["optimizely"].get("visitor").location.country;
				if(location != 'undefined'){
					if(location === 'US'){
						messageText.text(allContent.thirdvisit.allUS);
		...
		cookieStored.forEach(element => {
			if(element.match('ME131-landing='+currentproductTitle)){
		...
		if(landingPageTitle != currentproductTitle){
			if(!localStorage.getItem('ME131-exit')){
				addBar();
			}	
		}
	}
    ...
    const triggers = ((options) => {
            ...
            () => {
            	...
            	$ = window.jQuery;
				 utils.fullStory('ME131', 'Variation 1');
				 	activate();
			});
    })();
