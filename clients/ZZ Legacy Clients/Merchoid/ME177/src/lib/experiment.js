...
/**
 * {{ME177}} - {{Product Imagery Messaging}}
 */
  ...
  const Exp = {
    ...
    services: {
      setTestContent: () => {
        ...
        Exp.cache.messageArray = [`Premium ${brandName} ${productTypeText}`, `Officially Licensed By ${brandName}`, 'Limited Quantities Available. Get Yours Now!'];
        // Cache images
    ...
    render: {
      slideShowContainer: () => {
        ...
        Exp.cache.productGallery.insertAdjacentHTML('beforebegin', `
        <div class="ME177_SlideShow_Container">
          ...
          <p class="ME177_Message">${Exp.cache.messageArray[0]}</p>
        </div>
        `);
        // Store Selectors
