...
const Experiment = {
  ...
  components: {
    ...
    trackEvents: function trackEvents() {
      const { settings } = Experiment;
      const navItem = document.querySelector('.ME141-innerCat_link a');
      for (let i = 0; i < navItem.length; i += 1) {
        const element = navItem[i];
        element.addEventListener('click', () => {
          const navURL = element.getAttribute('href');
          events.send(`ME141 Mobile Navigation v${settings.VARIATION}`, 'Nav item selected', `${navURL}`, { sendOnce: true });
        });
      }
      const searchSubmit = document.querySelector('.shiftnav-search-submit');
      const searchVal = document.querySelector('.shiftnav-search-input').value;
      searchSubmit.addEventListener('click', () => {
        if (searchVal !== '') {
          events.send(`ME141 Mobile Navigation v${settings.VARIATION}`, 'Search used', `${searchVal}`, { sendOnce: true });
        }
      });
    },
  },
