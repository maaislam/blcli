 ...
 */
  const itemToHtml = (prod) => {
    ...
    if (prod && prod.id) {
      html += `
      <li class="${settings.ID}-gridChristmas product-small grid-2-break grid-3-break grid-4-break grid-5-break  grid3 grid-normal" style="background-image: url('${prod.url}')">
        <span> ${prod.text}</span>
      </li> 
      `;
    }
