...
export default () => {
  ...
  genderBannersWrapper.classList.add(`${settings.ID}-genderBanners`);
  genderBannersWrapper.innerHTML = `
  <h2>Officially Licensed Christmas ${jumperText}</h2>
  <div class="${settings.ID}-banner ${settings.ID}-for_her"></div>
  <div class="${settings.ID}-banner ${settings.ID}-for_him"></div>`;
  ...
  const addQuery = () => {
    ...
    for (let index = 0; index < genderBanners.length; index += 1) {
      ...
      element.addEventListener('click', () => {
        if (element.classList.contains(`${settings.ID}-for_her`)) {
          query = `${window.location.pathname}?gender=womens`;
        }
        if (element.classList.contains(`${settings.ID}-for_him`)) {
  ...
  const addTopProducts = () => {
    ...
    document.querySelector(`.${settings.ID}-genderBanners h2`).textContent = `Officially Licensed Christmas ${jumperText} ${gender}`;

    const topProducts = document.createElement('div');
    topProducts.classList.add(`${settings.ID}-top_products`);
    topProducts.innerHTML = `<div class="ME178-top"><h2>${jumperText} ${gender}</h2></div>`;
    document.querySelector(`.${settings.ID}-genderBanners`).insertAdjacentElement('afterend', topProducts);
    ...
    pollerLite([`.${settings.ID}-top_products .${settings.ID}-top`], () => {
        ...
        if (productName) {
          [].forEach.call(topGenderProducts, (topProduct) => {
            if (productName.textContent.trim() === topProduct) {
              ...
              document.querySelector(`.${settings.ID}-top_products .${settings.ID}-top`).appendChild(topProductHtml);
            }
          });
        }
      }
