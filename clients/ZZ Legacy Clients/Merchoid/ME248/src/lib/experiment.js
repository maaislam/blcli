...
export default () => {
  ...
  const showGifts = () => {
    const shopGiftsButton = document.querySelector(`.${shared.ID}-giftbanner .${shared.ID}-cta`);
    const bestSellers = document.querySelector(`.${shared.ID}-bestSellers`);
    if(shopGiftsButton) {
      if(bestSellers) {
        shopGiftsButton.addEventListener('click', () => {
          bestSellers.classList.add(`${shared.ID}-showProducts`);
          jQuery(`.${shared.ID}-products`).slick('setPosition'); 
          scrollToElement(bestSellers);
        });
      }
    }
  }
