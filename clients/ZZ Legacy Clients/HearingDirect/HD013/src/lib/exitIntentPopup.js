...
const exitIntentPopup = {
  init: function init() {
    // Setup
    const { services } = exitIntentPopup;
    document.body.classList.add('HD007');
    ...
    const controlPopup = () => {
      ...
      if (popup) {
        closeBtn.addEventListener('click', function() {
          ...
          events.send('HD013', 'Exit Intent Closed', 'User has closed exit intent coupon');
        });
        // Continue shopping
    ...
    if (services.mobileAndTabletcheck() === false) {
      // if (!visited) {
      /*
      ...
      */
      ...
      onExit.init(function(){
        ...
        const appendPopup = () => {
          ...
          // event tracking
          events.send('HD013', 'Exit Intent shown', 'User has seen exit intent coupon');
        };
        appendPopup();  
        controlPopup();
    
      }, 'HD013_popupShown', 'hearingdirect.com');
        /* eslint-enable */
      // }
      /*
      * Set cookie for the current user
      */
      // setCookie('returningUser', 'true', 799);
    }
    if (services.mobileAndTabletcheck() === true) {
      /*
      * User at this point is new and on
      * either mobile or tablet
      */
      const fromCheckoutPage = () => {
        const url = window.location.pathname;
        const page = url.trim();
        if (page.match(/^\/onestepcheckout/i) || url.match(/^\/onestepcheckout/i)) {
          setCookie('fromCheckout', 'true', 970);
        }
        const fromCheckout = getCookie('fromCheckout');
        if (fromCheckout && url.match(/^\/checkout\/cart/i)) {
          const mobilePopup = getCookie('mobileExitIntent');
          if (!mobilePopup) {
            // Append popup
            const appendPopup = () => {
              const popupHTML = services.popup();
              ...
              events.send('HD013', 'Exit Intent shown', 'User has seen exit intent coupon');
              // set cookie
              setCookie('mobileExitIntent', 'true', 970);
            };
            appendPopup();
            controlPopup();
          }
