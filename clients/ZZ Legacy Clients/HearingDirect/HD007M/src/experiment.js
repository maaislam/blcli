...
const Experiment = {
  ...
  init: function init() {
    // Bail out if any of the below tests are running
    ...
    _gaq.push(['_trackEvent', 'HD007M', 'Active Experiment', 'HD007M is active', null, true]); // eslint-disable-line
    ...
    const controlPopup = () => {
      ...
      if (popup) {
        closeBtn.addEventListener('click', function() {
          ...
          _gaq.push(['_trackEvent', 'HD007M', 'Popup Closed', 'User has closed the popup', null, true]); // eslint-disable-line
        });
        // Continue shopping
      ...
      if (fromCheckout && url.match(/^\/checkout\/cart/i)) {
        ...
        if (!mobilePopup) {
          ...
          const appendPopup = () => {
            ...
            // event tracking
            _gaq.push(['_trackEvent', 'HD007M', 'Popup Shown', 'User has seen the popup', null, true]); // eslint-disable-line
            // set cookie
