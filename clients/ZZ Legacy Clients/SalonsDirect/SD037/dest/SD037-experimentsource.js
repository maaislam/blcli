	...
	function run(){
		...
		UC.poller([
			...
			window.FS.setUserVars({
				experiment_str: 'SD037',
				variation_str: 'Variation 1'
			});
		}, { multiplier: 1.2, timeout: 0 });

		$('body').addClass('SD037 SD015');
...
        
        var tooltipMessage = "The Seller's policy is to dispatch goods within 24 hours of receiving an order, however this can vary subject to stock availability. If orders are received and fully completed before 4pm (UK time) between Monday to Thursday, we aim to deliver them the next working day to UK Mainland."; 
...
        /**** Left column changes ****/
        $left.prepend('<div class="SD015_col-title">Delivery Address</div>');
...
            $voucherToggle = $('<div class="SD015_voucher-toggle">Apply Offer Code</div>');
        
...
        });
		...
		        //AMENDS
		$('.onestepcheckout-shipping-method-block').insertAfter('#billing_address');
		$('.shipping-address-title').insertBefore('.input-box.input-different-shipping');
		...
		$('.onestepcheckout-threecolumns').before([
			'<div class="SD037_threecol_wrap clearfix">',
				'<div class="SD037_col-wrap SD037_first-col">',
					'<div class="SD037_delivery-title">1. Delivery Details</div>',
				'</div>',
				'<div class="SD037_col-wrap SD037_second-col">',
				'</div>',
				'<div class="SD037_col-wrap SD037_third-col">',
					'<div class="SD037_delivery-title">3. Review Order</div>',
				'</div>',
			'</div>',
		].join('')).hide();

		$('.onestepcheckout-column-left').appendTo('.SD037_first-col');
		$delivery.show().insertAfter('.onestepcheckout-column-left > .SD015_col-title');
		$('#shipping_address .form-alt label').hide();
		$('.SD037_second-col').append('<div class="SD037_delivery-title SD037_billing-pre">2. Payment Details</div>');
		$('.onestepcheckout-shipping-method-block .shipment-methods').before('<div class="SD037_delivery-header"><h3>Delivery Method</h3></div>');

		$('.SD037_billing-pre').after([
			'<div class="SD037_billing-wrap">',
				'<div class="SD037_delivery-header">',
					'<h3>Billing Address</h3>',
					'<a href="#">Select a different billing address</a>',
				'</div>',
				'<div class="SD037_input_chk SD037_billing-check checked">',
					'<label>Billing address is same a delivery address</label>',
					'<input type="checkbox" />',
				'</div>',
			'</div>',
		].join(''));

		$('.onestepcheckout-column-right').appendTo('.SD037_second-col');
		$billing.hide().insertAfter($('.SD037_billing-wrap'));
		$('#billing_address ul li').css('display', '');
		$('.onestepcheckout-column-middle').appendTo('.SD037_third-col');

		if($('.SD037_third-col .SD037_delivery-title').is(':visible')){
			$('.SD037_third-col').append([
				'<div class="SD037_place-order">',
					'<div class="SD037_price-total"><strong>Total: <span></span></strong> inc VAT</div>',
				'</div>',
			].join(''));
		}
		else{
			$('.SD037_second-col').append([
				'<div class="SD037_place-order">',
					'<div class="SD037_price-total"><strong>Total: <span></span></strong> inc VAT</div>',
				'</div>',
			].join(''));
		...
		$('#finalstep').appendTo('.SD037_place-order');
		$('.SD015_final-del-msg').appendTo('.SD037_place-order');
		
		

		UC.observer.connect($('.onestepcheckout-summary'), function(el, mutation) {
            $('.SD037_price-total span').text($('.onestepcheckout-totals .grand-total .price').text());
        }, {
            config: { childList: true, attributes: false, subtree: false }
        })

		var billingInputWrap = $('.SD037_billing-check'),
			slideQ = false;

		setSelectedIndex($('#billing-address-select')[0], $('#shipping-address-select')[0].selectedIndex);

		billingInputWrap
			.find('input')
			.prop('checked', true);

		$('#billing .input-different-shipping input')
			.prop('checked', false);

		$('#shipping-address-select').on('change', function(){
			if($('.SD037_billing-check.checked').length > 1){
				setSelectedIndex($('#billing-address-select')[0], $('#shipping-address-select')[0].selectedIndex);
			}
		});

		$('.SD037_billing-wrap .SD037_delivery-header a, .SD037_billing-check').on('click', function(e){
			e.preventDefault();

			if(slideQ === false){
				slideQ = true;
				if($billing.hasClass('SD037_active')){
					$billing
						.removeClass('SD037_active')
			...
			input.on('change', function () {
				...
				if (chk.is(':checked')) {
					if (chk.attr('type') == "radio") {
						...
						radioOther.closest('.SD037_input_chk').removeClass('checked');
					}
					el.addClass('checked');
				} else {
		...
		$('.SD037_input_chk').click(function () {
			this.blur();
			this.focus();
			var el = $(this),
				input = el.find('input');
		...
		$(window).resize(function(){
			if($('.SD037_third-col .SD037_delivery-title').is(':visible')){
				$('.SD037_place-order').appendTo('.SD037_third-col');
			}
			else{
				$('.SD037_place-order').appendTo('.SD037_second-col');
			}
		});
	}
})();