var _SD038 = (function () {
    ...
    // ---------------------------------------------
        ...
        // Insert the multi-buy wrapper after the 'SPECIAL OFFER!' message
        var $specialOfferContainer = $('.product-des').find('.spl-offer');
        $multi_buyWrapper.insertAfter($specialOfferContainer);

        var $discountTable = $('.product-des').find('.product-shop').find('.multiple-item-table');
        ...
        $('table.multiple-item-table tr:eq(0), table.multiple-item-table tr:eq(1)').remove();

        // Extract the data from the current table and display to subsequently display it in a tidier format
        var $extractTableData = $('table.multiple-item-table tr').map(function(){
            return [
