...
import { comparisonChart } from "./experiment";
...
class BraSizes extends Component {
  ...
  onChange(e) {
    if(e.target.dataset.type == 'back') {
      fireEvent(`Changed Back`, e.target.value);
      ...
      fireEvent(`Changed Cup`, e.target.value);
          ...
          <div class={ID + '-sizes__cup'}>
            ...
            >
              ...
              {this.state.availableVariants.map((variant) => {
                ...
                let prodCode = this.state.chosenBack;
                let compChart = comparisonChart.find(element => element.ukSize.substring(2, element.ukSize.length) == variant.value);
                return <option {...status} data-status={variant.stockStatus} value={variant.skuCode}
                  >{variant.value}{variant.stockStatusKey == 'Unavailable' ? ' (Sold out)' : ''} <span class="${ID}-ussize">(US {compChart.usSize.substring(2, compChart.usSize.length)})</span></option>
              })}
            </select>
          </div>
        </div>
