...
class BraSizes extends Component {
  ...
  onChange(e) {
    if(e.target.dataset.type == 'back') {
      events.send(shared.ID + '-' + shared.VARIATION, `Changed Back`, e.target.value);
      ...
      events.send(shared.ID + '-' + shared.VARIATION, `Changed Cup`, e.target.value);
      ...
      if(btn && !btn.classList.contains('xlistener-added')) {
        ...
        btn.addEventListener('click', (e) => {
          if(!this.state.chosenCup || !this.state.chosenBack) {
            ...
            events.send(shared.ID + '-' + shared.VARIATION, `Added To Bag Error`);

            this.setState({
              showError: true
            });
          } else {
            events.send(shared.ID + '-' + shared.VARIATION, `Added To Basket`);
          }
        });
      }
    }, 1500);
