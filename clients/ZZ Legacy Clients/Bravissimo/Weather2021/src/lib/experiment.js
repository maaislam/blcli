/**
 * BV023 - Pop up mobile
 * @author User Conversion
 */
import { setup } from './services';
import { cacheDom } from '../../../../../lib/cache-dom';
import { getCookie, events, getUrlParameter } from '../../../../../lib/utils';
import settings from './settings';
import { getWeatherFromAPI } from '../../../lib/weather';

// Overrides all other checks
const RUN_ANYWAY = true;
  
// Weather blacklist
const blacklist = [/rain/, /storm/, /drizzle/, /snow/, /dust/, /fog/, /haze/, /mist/, /overcast/, /sleet/];

// Weather whitelist if we really want to reign in showing it
const whitelist = [/clear sky/];

// Min temperature required
const TEMP_THRESHOLD = 18;

/**
 * Capitalize string words
 */
const capitalise = (str, lower = false) => (lower ? str.toLowerCase() : str).replace(/(?:^|\s|["'([{])+\S/g, match => match.toUpperCase());

/**
 * Helper is blacklisted weather
 */
const isBlacklistedWeather = (weatherInfo) => blacklist.some((regex) => regex.test(weatherInfo.condition));

/**
 * Hepler is whitelisted weather
 */
const isWhitelistedWeather = (weatherInfo) => whitelist.some((regex) => regex.test(weatherInfo.condition));

/**
 * Helper meets temp threshold
 */
const meetsThreshold = (weatherInfo) => weatherInfo.temperatureValue > TEMP_THRESHOLD;

/**
 * Custom widget html
 */
const getCustomWidgetHtml = (data) => {
  return `
    <a class="${settings.ID}-weather" href="/collections/all-swimwear/">
      <div class="${settings.ID}-weather__intro">
        It's hotting up! Shop bra-sized swimwear.
      </div>
      <div class="${settings.ID}-weather__link">
        <span>Shop Swimwear</span>
      </div>
      <div class="${settings.ID}-weather__meta">
        <div class="${settings.ID}-weather__temp">
          <img src="data:image/gif;base64,
">
          <span>
            ${Math.ceil(data.weather.temperature)}
            °C
            Sunny
          </span>
        </div>
        ${data.city ? `
          <div class="${settings.ID}-weather__location">${data.city}</div>
        ` : ''}
        
      </div>
    </div>
  `;
};

/**
 * Entry point post polling
 */
const activate = () => {
  // Scrape weather
  const main = document.querySelector('.c-page');

  if(main) {
    getWeatherFromAPI().then(weatherInfo => {
      if(weatherInfo && weatherInfo.city && weatherInfo.weather) {

        if(RUN_ANYWAY || (!isBlacklistedWeather(weatherInfo) && meetsThreshold(weatherInfo)) || getUrlParameter('force')) {
          const html = getCustomWidgetHtml(weatherInfo);

          setup();

          if(settings.VARIATION != 'control') {
            main.insertAdjacentHTML('afterbegin', html);

            const banner = document.querySelector(`.${settings.ID}-weather`);
            if(banner) {
              banner.addEventListener('click', () => {
                events.send(`${settings.ID}-${settings.VARIATION}`, 'clicked-weather-banner');
              });
            }
          }

          events.send(`${settings.ID}-${settings.VARIATION}`, 'displayed-weather');
          events.send(`${settings.ID}-${settings.VARIATION}`, 'temperature', weatherInfo.weather.temperature);
          events.send(`${settings.ID}-${settings.VARIATION}`, 'city', weatherInfo.city);
          events.send(`${settings.ID}-${settings.VARIATION}`, 'condition', weatherInfo.weather.main);
        }
      }
    });
  }

};

export default activate;
