...
// For GO builds, comment this
//import config from '../../../BD006/src/lib/config';

export default () => {
  // For GO builds, uncomment this
  const config = window.BD009config;
  ...
  const addBtn = ref => {
    ...
    if (document.querySelector('.BD009-toggle')) {
      let btn = document.querySelector('.BD009-toggle');
    ...
    if (!document.querySelector('.BD009-toggle')) {
      ref.insertAdjacentHTML('afterend', `<button class="BD009-toggle">
        Fill My Box
      ...
      let thisBtn = document.querySelector('.BD009-toggle');
  ...
  const menu = (ref) => {
    if (document.querySelector('.BD009-menu')) { // If exists, remove and reset
      let el = document.querySelector('.BD009-menu');
    ...
    console.log('qty = ', qty);
    thisConf = config[qty];
    console.log('thsConfg = ', thisConf);
    if (!thisConf) return;

    // if (24 - qty === 0) return;

    const thisFormKey = document.querySelector('input[name="form_key"]');
    console.log('thisFormKey = ', thisFormKey);
    if (!thisFormKey) return;


    let uencKey = null;
    let keyEl = document.querySelector('[data-post]');

    if (keyEl) {
      let keyJson = JSON.parse(keyEl.getAttribute('data-post'));
      uencKey = keyJson.data.uenc;
    }
    console.log('uencKey = ', uencKey); 
    ...
    [].forEach.call(document.querySelectorAll(`.${ID}-menu .tocart.button`), (f) => {
      f.addEventListener('click', () => {
        events.send(ID, `${ID} Click`, `${ID} - Add Product From Banner`);
      });
    });

    
    // Add toggle events for QTY
    const qtyWraps = document.querySelectorAll('.BD009-menu .productTile__qtyToggles');
    ...
    const thisEl = document.querySelector('.BD009-menu');
    const openBtn = document.querySelector('.BD009-toggle');
    pageWrap.addEventListener('click', (e) => {
      if (openBtn != e.target) {
        // Is open
        if (!thisEl.contains(e.target)) {
          thisEl.classList.add('BD-hidden');
          document.body.classList.remove('BD009-fade');
        }
      }
    });
  ...
  const menuToggle = (el) => {
    ...
    document.body.classList.toggle('BD009-fade');
  };
  ...
  if (pass(qty)) {
    ...
    const menuWrap = document.querySelector('.BD009-menu');
    const close = document.querySelector('.BD009-menu .BD009-close');
    // Show menu
