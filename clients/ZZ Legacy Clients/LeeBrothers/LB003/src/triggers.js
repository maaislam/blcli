  ...
  function run() {
      ...
      $('.subcat_column-item').each(function() {
          ...
          if(variantSelect.length > 0 && $(this).find('.variant_options .options_not_available').length === 0) {
              ...
              var details = $(that).find('.options_not_available').text().trim().replace(',', '');
              var price = $(that).find('.firstLinePrice .price').text();
          ...
          $(that).find('.pd3-addto__button').click(function() {
              removeExistingMessage();
              var wrap = $(this).closest('.prod_cols');
              var CSRF = wrap.find('.add_to_cart_form input[name="CSRFToken"]').val();
              var mainCode = wrap.find('.add_to_cart_form input[name="productCodePost"]').val();
