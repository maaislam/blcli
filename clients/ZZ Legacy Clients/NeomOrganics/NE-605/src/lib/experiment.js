...
export default () => {
	...
	if (location.pathname === "/cart") {
		pollerLite(["form[action='/cart'] .cart-items"], () => {
			const entry = document.querySelector("form[action='/cart'] .cart-items");
			...
			claimGiftCta.addEventListener("click", (e) => {
				e.preventDefault();
				promoInput.value = "YESPLEASE";
				promoInputSubmit.click();
			});

			const ProgressEl = new Progress(
				root,
				`FREE 1 Wick Candle of your choice when you spend ${content[locale].currency}${content[locale].threshold}`,
				false,
				{
					url: "/collections/1wick-candles",
					text: "See our 1 Wick Candles",
				},
				"https://cdn.shopify.com/s/files/1/0104/8317/1391/products/Complete-Bliss-Calm-Relax-1-Wick_750x750.jpg?v=1605115527",
				0,
				content[locale].threshold,
				content[locale].currency,
				"away from a FREE GIFT",
				`Yippee! Get your FREE GIFT below. Only 1 Free Gift can be in your ${
	...
	if (progressEl) {
		...
		if (cta) {
			cta.addEventListener("click", () => {
				fireEvent("See our 1 Wick Candles CTA clicked");
			});
		}
	}
};
