    ...
    [].forEach.call(cols, (col, idx) => {
      podEl = `
        <div class="${shared.ID}-pod pod-${podPos} ${shared.VARIATION != 'control' ? 
          `column is-3-desktop is-4-tablet is-6-mobile` : ''}" id="x-block-${podPos}">
          </div>
      `;
  ...
  if(pod) {
    ...
    [].forEach.call(allPods, (pod) => {
      checkIntersection(pod).then(() => {
        // console.log(`Visible - View Ad Block - ${pod.getAttribute('id')}`);
      ...
      //switch(podId) {
      //  case 'V-day-block-1':
      //    pod.querySelector('a').setAttribute('href', '/products/the-energising-pair');
      //    break;
      //  case 'V-day-block-3':
      //    pod.querySelector('a').setAttribute('href', '/products/the-gift-of-zzzs');
      //    break;
      //  case 'V-day-block-2':
      //    pod.querySelector('a').setAttribute('href', '/products/love-is-a-moment-of-calm');
      //    break;
      //  case 'V-day-block-4':
      //    pod.querySelector('a').setAttribute('href', '/products/a-great-day-for-love');
      //    break;
      //}
    });
