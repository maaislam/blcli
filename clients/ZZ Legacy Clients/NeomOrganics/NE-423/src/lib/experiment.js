/**
 * NE-423 - Basket Christmas cross-sell
 *
...
// --- Comment out before adding to production
...
const getMarkup = () => {
  let listOfProds = [];
  // --- Get Basket Items
  const allBasketItems = document.querySelectorAll('.cart-items.is-row .js-item-wrapper');
  [].forEach.call(allBasketItems, (item) => {
    listOfProds.push(`https://www.neomorganics.com${item.querySelector('a').getAttribute('href')}`);
  });

  let itemsMarkup = '';
  let data = null;
  if (VARIATION == '1') {
    // --- Change data1 to window.data1 before adding to production
    data = data1[window.location.pathname];
  } 

  data.forEach(d => {
    if (listOfProds.indexOf(`${d.link}`) == -1) {
      itemsMarkup += `
...
export default () => {
  ...
  const mainSection = document.querySelectorAll('.cart-items.is-row .js-item-wrapper')[document.querySelectorAll('.cart-items.is-row .js-item-wrapper').length - 1];
  if(mainSection) {
