...
const startExperiment = () => {
  ...
  Go the extra mile and build them a gift that’s tailored to suit their wellbeing. <span class="${ID}-bold">Choose three or more products to get 20% off.</span>`;
  ...
  pollerLite(['#shopify-section-gift-builder .gift-builder__hero'], () => {
    ...
    if (giftRefresh) {
      ...
      if (totalGiftQuantity > 2) {
        ...
        miniBagLinePrices.forEach((linePrice) => {
          const originalPrice = (parseFloat(linePrice.textContent.split('£')[1]).toFixed(2) * 1.25).toFixed(2);
      ...
      // localStorage.removeItem("applyDiscountMiniCart");
    ...
    const newProductSummaryContainer = `
     <div class="${ID}-product-summary-container-desktop ${ID}-hide">
          ...
          <div class="${ID}-product-summary-callout ${ID}-mobile-order">
            <h3>Add 2 more products to get 20% off</h3>
          </div>
    ...
    function performCalculations() {
      ...
      if (numberOfProducts > 2) {
        ...
        savings = (subTotal * 0.2).toFixed(2);
      } else {
        total = subTotal;
      }
    }
    ...
    const updateProductSummary = (selectThisGift) => {
      ...
      if (numberOfProducts < 3) {
        ...
        productSummaryCalloutDOM.innerHTML = `Add ${3 - numberOfProducts} more ${productText} to get 20% off`;
      } else {
        productSummaryCalloutDOM.innerHTML = `Congratulations, You've got 20% off`;
      }
      //Update total Price
      ...
      productArray.forEach((product, index) => {
        ...
        <div class="${ID}-product-summary-lightbox-desktop-product">
          ...
          <div class="${ID}-product-summary-lightbox-desktop-product-info">
            ...
            ${
              numberOfProducts > 2
                ? `£${(product.price * 0.8).toFixed(2)} <span class="${ID}-line-through" >£${product.price.toFixed(2)}</span>`
          ...
          ${
            numberOfProducts > 2
              ? `£${(product.price * product.quantity * 0.8).toFixed(2)}`
    ...
    const updateProductTotalLightbox = () => {
      const productSummaryLightboxDesktopFooterLeft = `
        ...
        numberOfProducts > 2 ? `Your savings (20% off) <span class="${ID}-margin-left">£${savings}</span>` : `<br>`
      }</h4>
