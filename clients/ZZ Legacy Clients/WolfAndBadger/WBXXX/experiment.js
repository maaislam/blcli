function triggers (options, cb) { // eslint-disable-line no-unused-vars
  ...
  var pt = window.universal_variable.page.type;

  if(pt === 'category' || pt === 'product' || pt === 'basket') {
...
function execution (options) { // eslint-disable-line no-unused-vars
  ...
  var labelText = 'Win £50 Wolf & Badger Vouchers';
  var worthValue = '£50';
  if(window.universal_variable.basket.currency == 'USD') {
    labelText = 'Win $50 Wolf & Badger Vouchers';
    worthValue = '$50';
  } else if(window.universal_variable.basket.currency == 'AUD') {
    labelText = 'Win A$50 Wolf & Badger Vouchers';
    worthValue = 'A$50';
  }
  ...
  UC.feedbackTab.init({
    ...
    content: [
      '<div>',
        '<img class="" src="https://www.wolfandbadger.com/staticmedia/i/logo.svg?fc7887b86423" />',
        '<p class="">',
          'Please answer this short survey so that we can improve our shopping experience. It takes just 3 minutes and you could <strong>win one of 3 x Wolf & Badger vouchers worth ' + worthValue + '</strong>',
        '</p>',
        '<p>',
          '<a class="btn btn-default wb-take-survey-button" target="_blank" href="https://goo.gl/forms/eQZtF8HTtqd9erZ02">',
          'Take quick survey',
          '</a>',
        '</p>',
      '</div>'
    ].join(''),
    customClass: 'wbxxxa-tab-init',
    dimBackground: true
  });
  
  $('.wb-take-survey-button').click(function() {
    UC.feedbackTab.destroy.all();
    window.sessionStorage.setItem('ucfbtab-closed', 1);
  });
}
