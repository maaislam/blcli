...
export default () => {
  ...
  const shouldBeFirst = (url) => {
    let first = "";
    if (window.location.pathname.indexOf("/women/clothing/") !== -1) {
      first = "/dresses/";
    }
    if (window.location.pathname.indexOf("/women/accessories/") !== -1) {
      first = "/bags/";
    }
    if (window.location.pathname.indexOf("/men/accessories/") !== -1) {
      first = "/bags/";
    }
    if (window.location.pathname.indexOf("/men/clothing/") !== -1) {
      first = "/jumpers-cardigans/";
    }
    console.log(first);
    return url.indexOf(first) !== -1;
  };
  ...
  const getCategories = () => {
    ...
    catLinks.forEach((cat) => {
      ...
      console.log(shouldBeFirst(catObj.url), catObj);
      // Make specific categories first in the list.
      if (shouldBeFirst(catObj.url)) formatted.unshift(catObj);
      else formatted.push(catObj);
    });
