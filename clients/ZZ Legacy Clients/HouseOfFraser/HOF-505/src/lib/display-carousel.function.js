      ...
      <div class="${ID}-recs-holder" id="${ID}-recs-holder">
        ...
        <div class="${ID}-recs-carousel-holder loading">
          ...
          <button class="${ID}-button ${ID}-button-prev">
            <svg width="16" height="28" viewBox="0 0 16 28" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M1 27L14.5 14L1 0.999999" stroke="#323232" stroke-width="2"/></svg>
          </button>
...
export const displayCarousel = () => {
  pollerLite(['#CartPanel'], () => {
    document.getElementById('CartPanel').insertAdjacentHTML('afterend', carouselHtml);


    let carouselRightArrow = document.querySelector(`.${ID}-button-next`);
    carouselRightArrow.addEventListener('click', (e) => {
      fireEvent(`Click - user has clicked on the right arrow in the variation carousel`);
    });

    let carouselLeftArrow = document.querySelector(`.${ID}-button-prev`);
    carouselLeftArrow.addEventListener('click', (e) => {
      fireEvent(`Click - user has clicked on the left arrow in the variation carousel`);
    });
