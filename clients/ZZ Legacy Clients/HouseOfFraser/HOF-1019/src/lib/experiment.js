...
const updateInfoPanel = () => {
  ...
  currNumDisplayed = currPageNo * 119;
  if (currNumDisplayed > totalProducts && totalProducts > 119) {
...
const startExperiment = (theVariation) => {
  ...
  if(theVariation == 1) {
    ...
    pollerLite(['#productlistcontainer', '.totalProducts', '.CurrentPageNumber'], () => {
      fireEvent(`Interaction - more than 119 products present, the infinite scroll is displayed`);
      totalProducts = document.querySelector('.totalProducts').innerText;
          ...
          <div id="${ID}-pagholder-bottom" class="${ID}-pagination-holder">
            ...
            <div class="${ID}-pagination">
              ...
              <div class="${ID}-pagination--details">
                <p id="${ID}-pagination--detailstext"><span id="${ID}-pagination--detailsnumber">${currPageNo * 119}</span> out of <span id="${ID}-pagination--detailstotal">${document.querySelector('.totalProducts').innerText}</span> products</p>
              </div>
    ...
    pollerLite(['#FiltersAndProductsWrapper', '.MaxPageNumber', '#productlistcontainer'], () => {
      ...
      if (totalPageNumber < 8) {
        ...
        toShowArray.forEach((i) => {
          ...
          let paginationButton = `
            <a href="${i == 1 ? baseURL : theUpdatedHref}" class="${ID}-pagination--link ${ID}-pagination--page ${i == currNumber ? `${ID}-pagination--active` : ``}" data-page="${i}">${i}</a>
          `;
...
export default () => {
  ...
  if(VARIATION == 1 && document.body.classList.contains('flanProdList')) {
    ...
    totalProducts = document.querySelector('.totalProducts').innerText;
    if (totalProducts > 119 && !document.querySelector(`.${ID}-pagination-holder`)) {
      startExperiment(VARIATION);
    } else {
      fireEvent(`Interaction - less than 119 products present, the infinite scroll is not displayed`, true);
    }
