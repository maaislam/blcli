    ...
    for (let j = 0; j < numOfProducts; j += 1) {
      ...
      if (i === 1) {
        if (currentP === 'current-product') {
          firstRow += `<td class="compareitemcell" style="border: none;">
            <span class="first-row-item" style="font-weight: 700;">
              Currently viewing
            </span>
          </td>`;
        } else {
        ...
        if (currentP === 'current-product') {
          pCtaBtns += `<td class="compareitemcell ${currentP}">
            <a class="btn btn--yellow" href="javascript: void(0)" onclick="document.querySelector('.${shared.ID}-lightbox__wrapper').classList.add('hide')">View</a>
          </td>`;
        } else {
          pCtaBtns += `<td class="compareitemcell ${currentP}">
