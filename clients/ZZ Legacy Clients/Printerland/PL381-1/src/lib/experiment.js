...
export default () => {
	...
	pollerLite([`.product__main_container`], () => {
		const domToRender = `
			<div class="${ID}-product_finder_bar">
				<h3 class="${ID}-product_finder_bar_title">
					<span>Need help looking for that perfect printer?</span>
					<img src="https://blcro.fra1.digitaloceanspaces.com/PL381%2FcloseIcon.svg" class="pl_bar_title-arrow" alt="Cross Icon">
				</h3>
				<div class="filter__wrapper">
					...
					<button class="nextcta">Next</button></div><div class="filter-tab-content" id="printer_feature" next-filter="printer_brands">
			...
			<div class="pl-tab-content-inner">
				...
				<p class="result-count-container"><span id="pl-result-count">1024</span> results found!</p>
				<button class="search_button">Search</button>	
			</div>
		</div>`;

		document.body.insertAdjacentHTML("afterbegin", domToRender);

		if (VARIATION == "1") {
			document.body.insertAdjacentHTML(
				"afterbegin",
				`<div class="pl-filter-overlay"></div>`
			);
			pollerLite([`#pnlAction`], () => {
				const ref = document.querySelector(
					`#pnlAction #pnlPrice .in-stock`
				);

				const secondaryRef = document.querySelector(
					`#pnlAction #pnlPrice .contracts__link`
				);

				const finalRef = document.querySelector(
					`#pnlAction .out-of-stock-container`
				);
				const searchFinder = `<div class="search-finder-container">
					<span>Not 100% sure? Let us help...</span>
					<img src="https://blcro.fra1.digitaloceanspaces.com/PL381%2Fright-arrow.svg" class="pl_bar_title-arrow" alt="Right Arrow" />
				</div>`;

				if (ref) {
					ref.insertAdjacentHTML("afterend", searchFinder);
				} else if (secondaryRef) {
					secondaryRef.insertAdjacentHTML(
						"beforebegin",
						searchFinder
					);
				} else if (finalRef) {
					finalRef.insertAdjacentHTML("beforeend", searchFinder);
				} else {
					document
						.querySelector("#pnlAction")
						.insertAdjacentHTML("beforeend", searchFinder);
				}

				document
					.querySelector(".search-finder-container")
					.addEventListener("click", () => {
						document
							.querySelector(`.${ID}-product_finder_bar`)
							.classList.add("active");
						document
							.querySelector(".pl-filter-overlay")
							.classList.add("active");
						fireEvent(
							`Click - User clicks on "Not 100% sure? Let us help..." CTA ans sees the product finder modal`
						);
					});
			});
		} else if (VARIATION == "2") {
			document.body.insertAdjacentHTML(
				"afterbegin",
				`<div class="pl-filter-switch">Product Finder</div>`
			);
			pollerLite([`.pl-filter-switch`], () => {
				document
					.querySelector(".pl-filter-switch")
					.addEventListener("click", () => {
						document
							.querySelector(`.${ID}-product_finder_bar`)
							.classList.add("active");
						...
						// 	.classList.add("active");
						fireEvent(
							`Click - User clicks on the sticky product finder CTA and sees the product finder modal`
						);
					});
			});
		}
	...
	pollerLite([`.${ID}-product_finder_bar`], () => {
		...
		const productFinderBarTitleClose = document.querySelector(
			`.${ID}-product_finder_bar h3.${ID}-product_finder_bar_title img`
		);
		document
			.querySelector(".pl-filter-overlay")
			?.addEventListener("click", () => {
		...
		productFinderBarTitleClose.addEventListener("click", () => {
			productFinderBar.classList.toggle("active");
			document
				.querySelector(".pl-filter-overlay")
				?.classList.toggle("active");
