	...
	pollerLite([`.page__main_body`], () => {
		...
		if (!document.querySelector(`.${ID}-price-promise-bar`)) {
			if (VARIATION == 1) {
				...
				pollerLite([`#pnlAction #pnlPrice`], () => {
					const ref = document.querySelector(`#pnlAction #pnlPrice .in-stock`);
					...
					if (ref) {
						ref.insertAdjacentHTML("afterend", pricePromiseBarMobile);
						...
						finalRef.insertAdjacentHTML("beforeend", pricePromiseBarMobile);
					}
				});
			...
			if (VARIATION == 2) {
				...
				const referenceDom = document.querySelector(
					`.page__main_body .product__main_container .title_container`
				);

				const refMobile = document.querySelector(
					`.page__main_body .product__main_container`
				);

				if (referenceDom) {
					referenceDom.insertAdjacentHTML("beforebegin", pricePromiseBar);
				}

				if (refMobile) {
					refMobile.insertAdjacentHTML("beforebegin", pricePromiseBarMobile);
				}
				fireEvent(`Visible - User sees the price promise bar`);
			}
			...
			document.body.insertAdjacentHTML("beforeend", pricePromiseModal);
			...
			pollerLite(
				...
				() => {
					...
					pricePromiseModal.style.maxHeight = `${windowHeight - 60}px`;
					...
					pricePromiseModalOverlay.addEventListener(
						...
						() => {
							...
							pricePromiseModalOverlay.classList.remove("active");
							fireEvent(`Click - User closes the price promise modal`);
						},
						false
					);
					...
					if (VARIATION == 1) {
						const pricePromiseBar = document.querySelector(
							`.${ID}-price-promise-bar`
						);
						pricePromiseBar.addEventListener(
							"click",
							() => {
								pricePromiseModal.classList.add("active");
								pricePromiseModalOverlay.classList.add("active");
								fireEvent(`Click - User opens the price promise modal`);
							},
							false
						);
					} else if (VARIATION == 2) {
