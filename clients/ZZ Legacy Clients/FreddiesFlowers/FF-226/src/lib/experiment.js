...
const handleRedirect = () => {
  ...
  window.ga(trackerName + '.send', 'event', 'Experimentation', 'FreddiesFlowers - FF-226', 'Code Redirect', {
  ...
  const toRedirect = !settings.isNewSite && !settings.isOldSite && !settings.isExcludedPage;
    if (toRedirect) {
        ...
        if (!settings.isExcludedPage) {
          ...
          pollerLite(['.carousel[type="carousel"]', 'div[type="trustpilot"]'], () => {
            let carousel = document.querySelector('.carousel[type="carousel"]');
            let carouselParent = carousel.parentElement;
  
            let trustpilot = document.querySelector('div[type="trustpilot"]');
            let trustpilotParent = trustpilot.parentElement;
  
            trustpilotParent.insertAdjacentElement('afterend', carouselParent);
          })
