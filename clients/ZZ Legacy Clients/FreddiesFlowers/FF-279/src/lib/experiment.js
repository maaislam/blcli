...
const handleRedirect = () => {
  ...
  localStorage.setItem(`${ID}-has-redirected`, true);

  window.ga(trackerName + '.send', 'event', 'Experimentation', 'FreddiesFlowers - FF-279', 'Code Redirect', {
...
const handleExperiment = () => {
  const toRedirect = !localStorage.getItem(`${ID}-has-redirected`) && !settings.isExcludedPage;
    if (toRedirect && shared.VARIATION == 1) {
        handleRedirect();
    } else {
        localStorage.setItem(`${ID}-has-redirected`, false);
