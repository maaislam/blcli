var IT009 = (function() {
	...
	function run(){
		...
		if($('.right-off-canvas-menu .block-cart .actions-top').length > 0){
			$('.right-off-canvas-menu .block-cart .summary + .actions .button.btn-bag').hide();
			$('.right-off-canvas-menu .block-cart .summary').insertBefore('.right-off-canvas-menu .block-cart .actions-top');
			$('.right-off-canvas-menu .block-cart .subtotal .label').text('Basket Sub-Total:');
			$('#cart-sidebar').before([
			...
			$('.right-off-canvas-menu .block-cart .summary').appendTo('.right-off-canvas-menu .block-cart');
			$('.right-off-canvas-menu .block-cart .actions').appendTo('.right-off-canvas-menu .block-cart');
			$('.right-off-canvas-menu .block-cart').append([
				'<div class="IT_free_delivery">',
					'<div>',
						'<div class="IT_progress_bar"><span></span></div>',
						'<div class="IT_progress_img"><img src="" /></div>',
				...
				if($('.alert-box.success-msg').length == 0){
					...
					var interval = setInterval(function(){
						if($('.alert-box.success-msg').length > 0){
							...
							if($('.right-off-canvas-menu .block-cart .actions-top').length > 0){
								// Check if the user has enough items to have 2 sets of checkout buttons
								// 		Then append new cart items and move around to match design
								$('.right-off-canvas-menu .block-cart .summary + .actions .button.btn-bag').hide();
								$('.right-off-canvas-menu .block-cart .summary').insertBefore('.right-off-canvas-menu .block-cart .actions-top');
								$('.right-off-canvas-menu .block-cart .subtotal .label').text('Basket Sub-Total:');
								$('#cart-sidebar').before([
							...
							if(parsePrice < 60){
								priceTillFree = priceTillFree - parsePrice;
								var n = new UC.Money.Format(priceTillFree);
								n.setCurrency('GBP');
								priceUpdate.text(n.formatMoney(2, '.', ',', true));
								percentageNum = (parsePrice / 60) * 100;
						...
						else if($('.alert-box.error-msg').length > 0){
							// If an error was thrown stop checking for success message and clear interval
							clearInterval(interval);
						}
					}, 50); 
				}
