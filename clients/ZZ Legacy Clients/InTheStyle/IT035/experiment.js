var IT035V1 = (function() {
	...
	   		}
		...
		var cacheDom = (function() {
			...
			// Add cancel class to stop the mini basket from sliding out
		...
		var addToBag = {
			// Create add to bag notification markup
			contentBuilder: function(){
				...
				// Cache wrapping element for later use
		...
		var elementBindings = {
			...
			addToBasketClick: function(){
				cacheDom.basketBtn.on('click', function(){
					if(cacheDom.sizeSelector.find('label.selected').length > 0){
						...
						// poll for the move-left class thats added when you add something to the basket
						UC.poller([
						...
						// Have notification fade in then fade out after 3 seconds
			...
			// Fade the notification out when the close button is clicked
			...
			// When the user clicks on the bag icon in the nav, slide out the mini basket
