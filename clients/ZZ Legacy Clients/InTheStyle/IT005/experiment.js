	...
	function run(){
		...
		if(urlTest.test(url)){
			if(/[?&]/.test(url)){
				...
				window.location.search = '?dir=desc&order=price';
			}
		}
		...
		$('a').each(function(){
			...
			if(elHref == '#' || elHref === ''){
				...
				el.attr('href', elHref + '?dir=desc&order=price');
			}
		});
	}
	...
	function homePage(){
		$('.slick-slide').each(function(){
			var el = $(this),
				...
				newAttr = thisAttr.replace("');", "?dir=desc&order=price');");
