...
var _IT028 = (function() {
    ...
    function activate() {
        ...
        var $newProductTable = $('<div class="IT028_products"></div>');
        ...
        $cart.children('tbody').children('tr').each(function() {
            // components
            var $el = $(this);
            ...
            var $newRow = $([
                '<div class="IT028_prod">',
                    '<div class="IT028_prod__left">',
                        '<div class="IT028_prod__img">',
                            '<img src="' + productImg + '" />',
                        '</div>',
                    '</div>',
                    '<div class="IT028_prod__right">',
                        '<div class="IT028_prod__title">',
                            '<a href="' + link + '">',
                                '<h2>' + productName + '</h2>',
                            '</a>',
                        '</div>',
                        '<div class="IT028_prod__price">' + productPrice + '</div>',
                        '<div class="IT028_prod__size">Size: ' + productSize + '</div>',
                        '<div class="IT028_btn IT028_change-size-btn">Change size</div>',
                        '<select class="IT028_prod__size--change"><option selected="selected">Change size</option></select>',
                        '<div class="IT028_prod__qty">',
                            '<div class="IT028_prod__qty-label">QTY: </div>',
                            '<input class="input-text qty" maxlength="12" type="number">',
                            '<div class="IT028_prod__qty-update">Update</div>',
                        '</div>',
                        '<i class="IT028_prod__remove fa fa-close"></i>',
                    '</div>',
                '</div>'
            ].join(''));
            ...
            var removeProduct = function() {
                ...
                $remove.find('i').trigger('mousedown');
                $remove.find('i').trigger('touchstart');
                $remove.find('i').trigger('touchend');  
            };
            
            $newRow.find('.IT028_prod__remove').on('click touchstart', removeProduct);

            // quantity
            var quantityEvents = (function() {
                var $newQty = $newRow.find('.IT028_prod__qty');
                var $input = $newQty.children('input');
                var $update = $newQty.children('.IT028_prod__qty-update');
            ...
            var addToCartAjax = function(data, sizeData) {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        $('body').prepend('AJAX SUCCESS - CLICKING REMOVE');
                        // Remove old product and trigger refresh
                        removeProduct();

                        // Try removing product again after a timeout (removing instantly after AJAX doesn't work on iOS)
                        setTimeout(removeProduct, 1200);
                        //setTimeout(removeProduct, 2500);
                        window.ga('send', 'event', 'IT028', 'change', 'user changed size', {nonInteraction: true});
                    },
                    error: function() {
                        window.ga('send', 'event', 'IT028', 'change', 'error changing size', {nonInteraction: true});
            ...
            var getSizes = function(productUrl) {
                var $el = $newRow.find('.IT028_prod__size--change');
            ...
            var changeSize = (function() {
                $newRow.find('.IT028_prod__size--change').hide();
                ...
                if ($qty.val() > 1) {
                    $newRow.find('.IT028_change-size-btn').hide();
                    return false;
                }

                var $el = $newRow.find('.IT028_change-size-btn');

                $el.click(function(e) {         
                    if (!blockEvents) {
                        blockEvents = true;
                        window.ga('send', 'event', 'IT028', 'click', 'user clicked change size', {nonInteraction: true});
                        setTimeout(function() {
                            blockEvents = false;
                        }, 1000);
                    }
            ...
            */
            function createCustomSelect($el) {
                ...
                var $btn = $newRow.find('.IT028_change-size-btn');

                var buildSrc = function() {
                    var $src = $([
                        '<div class="IT028_change-size-modal">',
                            '<div class="IT028_modal-title">Change Size</div>',
                            '<ul class="IT028_custom-change-size"></ul>',
                        '</div>'
                    ].join(''));
                    var $ul = $src.find('ul');
                    ...
                    $options.each(function(i) {
                        ...
                        var $li = $('<li class="IT028_opt">' + $opt.text() + '</li>');

                        if (isDisabled) {
                            $li.addClass('IT028_opt--disabled');
                        }
            ...
            // GA Events
            $newRow.find('.IT028_prod__title > a').one('click', function() {
                window.ga('send', 'event', 'IT028', 'click', 'clicked product title', {nonInteraction: true});
            });
            
            $newRow.find('.IT028_prod__img > a').one('click', function() {
                window.ga('send', 'event', 'IT028', 'click', 'clicked product image', {nonInteraction: true});
            });
        ...
        $deliveryOpts.find('tbody > tr').each(function() {
            ...
            var $newText = $([
                '<div class="IT028_delivery-opt">',
                    '<span class="IT028_del-title">' + title + '</span>',
                    '<span class="IT028_del-price">' + price + '</span>',
                    '<span class="IT028_del-time">' + time + '</span>',
                '</div>'
            ].join(''));
            ...
            $input.on('change', function() {
                window.ga('send', 'event', 'IT028', 'user-chose-delivery-method', '', {nonInteraction: true});
                $('#basket-shipping-options-form').submit();
            });
        });

        // ----------------------------------------------------------
        // Based on IT002, which modifies delivery methods, we are
        // re-creating their display but in a custom-built dropdown -- 
        // The reason we don't use a <select> is because iOS truncates
        // the display of the text in the selector carousel it shows
        // ----------------------------------------------------------
        var deliveryOptsTable = $deliveryOpts.find('table:first'),
            deliveryOptsTableWrapper = deliveryOptsTable.wrap('<div class="IT028_delivery-table-wrapper">').parent();

        deliveryOptsTableWrapper.prepend('<h3>Choose your Delivery Method <span class="IT028_close"><i class="fa fa-times"></i></span></h3>');
        deliveryOptsTableWrapper.find('.IT028_close').on('click', function() {
            deliveryOptsTableWrapper.removeClass('IT028_active');
        });

        var activeOpt = $deliveryOpts.find('input.shipping_radio:checked'),
            activeOptRow = activeOpt.parents('tr:first'),
            activeTextDiv = activeOptRow.find('.IT028_delivery-opt');

        $deliveryOpts.prepend([
            '<div class="IT028_delivery-selector">',
                activeTextDiv.html(),
                '<a>Change delivery</a>',
            '</div>'
        ].join(''));

        $('.IT028_delivery-selector').on('click', function() {
            deliveryOptsTableWrapper.addClass('IT028_active');

            window.ga('send', 'event', 'IT028', 'user-clicked-on-delivery-selector', '', {nonInteraction: true});
        });
        
        // ----------------------------------------------------------
        // Discount field
        // ----------------------------------------------------------
        $('#shopping-cart-totals-table .IT028_total-field').each(function() {
            var tdLeft = $(this).find('td.a-left:first');
            if(tdLeft.text().trim().match(/^Discount/i)) {
                $(this).addClass('IT028_discount-row');
            }
        });
        ...
        // Discount message
        // ----------------------------------------------------------
        var discountAmount = $('.IT028_discount-row td.a-right .price').text().trim().replace(/-/, '');
        if(discountAmount) {
            $('.btn-proceed-checkout:last').after([
                '<p class="IT028_discount-identifying-message">',
                    'Congratulations Honey. You have just saved ',
                    discountAmount,
                    '. What a deal.',
                '</p>'
            ].join(''));
        ...
        // ----------------------------------------------------------
        var price = '';
        $('#shopping-cart-totals-table tfoot tr').each(function() {
            var tdLeft = $(this).find('.a-left:first'),
                tdRight = $(this).find('.a-right:first');

            if(tdLeft.text().trim().match(/Grand Total/i)) {
                price = tdRight.find('.price').text().trim();

            }

        });
        if(price) {
            $('.page-title.title-buttons h1').append([
                '<div class="IT028_title-total">',
                    '<span>Total</span>',
                    '<strong>' + price + '</strong>',
                '</div>'
            ].join(''));
        ...
        $.each($footerLinksGroups, function(){
            $(this).wrapAll('<div class="IT028_footer-link-group"></div>');
        });
    }
