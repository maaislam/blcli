...
var _PP019 = (function($) {
    ...
    var run = function() {
        $('body').addClass('PP019');
        // Ask user if they would like help or not
        // Send GA event on selection

        // If yes
        function getTooltipText(option) {
            var text;
            switch (option) {
                case 'name this print job':
                    text = 'Give your print job a name';
                    break;
                default:
        ...
        $('.feature:not(".hide"), .nome_lavorazione').each(function(i) {
            var $this = $(this);
            var label = $(this).find('.panel-heading label').text().trim().toLowerCase();
            var tooltipText = getTooltipText(label);
            
            $this.wrap('<div data-intro="' + tooltipText + '"></div>');
        });
