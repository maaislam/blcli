...
export default () => {
  ...
  const giftWrappingPrice = () => {
    ...
    const allOrderSummaries = document.querySelectorAll('.order-summary__row');
    for (let index = 0; index < allOrderSummaries.length; index += 1) {
      const element = allOrderSummaries[index];
      if (element.querySelector('.order-summary__item')) {
        if (element.querySelector('.order-summary__item').textContent === 'gift packaging') {
          newGiftWrap.classList.add(`${id}-giftWrappingPrice_show`);
          newGiftWrap.querySelector(`.${id}-gift_price span`).textContent = element.querySelector('.order-summary__item-value').textContent;
  ...
  pollerLite([`.${id}-giftwrap`], () => {
    if (document.querySelector('.HS024-subtotal').textContent !== '') {
      giftWrappingPrice();
    }
  });
};
