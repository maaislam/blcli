...
let stageData = [
  {stageHeader: "Who are you shopping for?", stageImageURL: "https://blcro.fra1.digitaloceanspaces.com/FLAN-602/stage0.jpg"},
  {stageHeader: "What do you have in mind? ", stageImageURL: "https://blcro.fra1.digitaloceanspaces.com/FLAN-602/stage1.jpg"},
  {stageHeader: "What is your price range?", stageImageURL: "https://blcro.fra1.digitaloceanspaces.com/FLAN-602/stage2.jpg"},
  {stageHeader: "Your results", stageImageURL: "https://blcro.fra1.digitaloceanspaces.com/FLAN-602/stage3.jpg"},
];
...
const checkProgress = () => {
  ...
  if(stageNumber == 1) {
    let selectedGender = localStorage.getItem(`${ID}-gender-option`).replace('option-', '');
    let currImage = stageData[stageNumber].stageImageURL;
    currImage = currImage.replace('.jpg', '-'+selectedGender+'.jpg');
    quizHeader.style.backgroundImage = `url('${currImage}')`;
  } else {
        ...
        [].slice.call(stage2Buttons).forEach((button) => {
          ...
          button.addEventListener('click', (e) => {
            ...
            let theData = {
              ...
              mobileColumns: 2,
              clearFilters: false,
              pathName: 'https://www.flannels.com/'+stageURL,
              searchTermCategory: '',
          ...
          let currentGetProductsXhr = $.ajax({
                ...
                success: function(returnedData) {
                  if(returnedData) {
                    ...
                    if(returnedData.numberOfProducts <= 4 && window.outerWidth > 767) {
                      document.querySelector(`.${ID}-quiz--stage3`).classList.add('carousel-hidden');
                    } else {
                      document.querySelector(`.${ID}-quiz--stage3`).classList.remove('carousel-hidden');
                    }

                    if(returnedData.numberOfProducts == 0) {
                      document.querySelector(`.${ID}-quiz--stage3`).classList.add('no-prods-found');
                    } else {
                      document.querySelector(`.${ID}-quiz--stage3`).classList.remove('no-prods-found');
                    }
                          ...
                          return `
                            <div class="${ID}-product swiper-slide">
                              ...
                              <div class="${ID}-product--desc">
                                ...
                                <a href="${product.url}" class="${ID}-quiz--button primary">View Product</a>
                              </div>
                    ...
                    `;
                    stage3.querySelector(`#${ID}-quiz--itemscarousel .swiper-wrapper`).innerHTML = "";
        
                    stage3.querySelector(`#${ID}-quiz--itemscarousel .swiper-wrapper`).insertAdjacentHTML('afterbegin', productDisplayHTML);

                    catLink.href = fullURL;
                    catLink.querySelector('span').innerText = returnedData.numberOfProducts;
        
                    setTimeout(() => {
                      
                      let allReturnedProducts = document.querySelectorAll(`.${ID}-quiz--products .${ID}-product .primary`);
                      [].slice.call(allReturnedProducts).forEach((prodLink) => {
                        prodLink.addEventListener('click', (e) => {
                          fireEvent(`Click - user has clicked on product: ${prodLink.href}`);
                        })
                      });
                      ...
                      setTimeout(() => {
                        ...
                        if(products.length == 0) {
                          stage3.classList.add('no-products');
                        }
          
                        stage3.classList.remove('calculating');
...
const buildQuiz = () => {
  ...
  let quizHTML = `
    ...
    <div class="${ID}-quiz">
      ...
      <div class="${ID}-quiz--header" style="background-image: url('https://blcro.fra1.digitaloceanspaces.com/FLAN-602/stage0.jpg')">
      ...
      <div class="${ID}-quiz--body">
        ...
        <div class="${ID}-quiz--stage ${ID}-quiz--stage0 ${stageNumber == 0 ? 'active' : ''}">
          ...
          <button class="${ID}-quiz--button ${ID}-gender-option option-him" data-image="" data-option="option-her">WOMEN</button>
          <button class="${ID}-quiz--button ${ID}-gender-option option-her" data-option="option-him">MEN</button>
          <button class="${ID}-quiz--button ${ID}-gender-option option-junior" data-option="option-junior-girls">GIRLS</button>
          <button class="${ID}-quiz--button ${ID}-gender-option option-junior" data-option="option-junior-boys">BOYS</button>
        ...
        <div class="${ID}-quiz--stage ${ID}-quiz--stage1 ${stageNumber == 1 ? 'active' : ''}">
          ...
          <div class="${ID}-option-junior-girls">
            <button class="${ID}-quiz--button ${ID}-category-option option-clothing" data-stageurl="/kids/girls/clothing" data-catcode="FLAN_TOPKIDSGIRLSCL" data-catoption="">CLOTHING</button>
            <button class="${ID}-quiz--button ${ID}-category-option option-fragrance" data-stageurl="/kids/girls/shoes/" data-catcode="FLAN_TOPKIDSGIRLSSHO" data-catoption="">FOOTWEAR</button>
            <button class="${ID}-quiz--button ${ID}-category-option option-accessories" data-stageurl="/kids/girls/accessories" data-catcode="FLAN_TOPKIDSGIRLSACC" data-catoption="">ACCESSORIES</button>

          </div>

          <div class="${ID}-option-junior-boys">
            <button class="${ID}-quiz--button ${ID}-category-option option-clothing" data-stageurl="/kids/boys/clothing" data-catcode="FLAN_TOPKIDSBOYSCL" data-catoption="">CLOTHING</button>
            <button class="${ID}-quiz--button ${ID}-category-option option-fragrance" data-stageurl="/kids/boys/shoes" data-catcode="FLAN_TOPKIDSBOYSSHO" data-catoption="">FOOTWEAR</button>
            <button class="${ID}-quiz--button ${ID}-category-option option-accessories" data-stageurl="/kids/boys/accessories" data-catcode="FLAN_TOPKIDSBOYSACC" data-catoption="">ACCESSORIES</button>
        ...
        <div class="${ID}-quiz--stage ${ID}-quiz--stage3 ${stageNumber == 3 ? 'active' : ''} calculating">
          ...
          <span class="no-products-found">No Products Found</span>

          <p> Calculating your results...</p>

          <div class="${ID}-quiz--stage3results">
          <div class="${ID}-button ${ID}-button-prev"> <svg width="20" height="11" viewBox="0 0 20 11" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M19 1L10 9L1 1" stroke="#EAFF00" stroke-width="2"/></svg></div>
          <div class="${ID}-button ${ID}-button-next"> <svg width="20" height="11" viewBox="0 0 20 11" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M19 1L10 9L1 1" stroke="#EAFF00" stroke-width="2"/></svg></div>
            <div id="${ID}-quiz--itemscarousel" class="${ID}-quiz--products swiper-container">
            ...
            <div class="${ID}-quiz--categorylink">
              ...
              <a target="_blank" class="${ID}-quiz--button" href=""> See all matches (<span id="${ID}-numprods">0</span>) </a>
  ...
  backwardsButton.addEventListener('click', (e) => {
    ...
    stageNumber --;
    moveBackwardsStage();
    fireEvent(`Click - user has clicked the back button to go to stage ${stageNumber}`);
  });
