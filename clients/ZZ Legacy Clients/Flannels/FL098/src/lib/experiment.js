/**
 * FL098 - Brands differentiation on mobile filter
 *
...
export default () => {
  ...
  if (VARIATION == 3) {
    events.send('FL098', 'FL098 Control', 'Control is active');
    return false;
  } else {
    events.send('FL098', `FL098 Variation ${VARIATION}`, 'FL098 Variation is active');
  }
  ...
  if (VARIATION == 2) { // Show top 6 Brands instead
    ...
    searchInput ? searchInput.addEventListener('click', () => {
      const addedFilters = document.querySelectorAll('.FL098-filter');
