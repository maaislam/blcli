...
const RunCheckoutLaunch = () => {
  ...
  const Exp = {
    ...
    cache: (() => {
      ...
      const storedJSON = JSON.parse(decodeURIComponent(localStorage.getItem('FL013_checkoutGuest')));
      let checkoutJSON = {
        email: '',
      };
    ...
    components: {
      loadStorage() {
        Exp.cache.input.value = Exp.cache.checkoutJSON.email;
        const { settings } = Exp;
        events.send(settings.ID, 'Load', 'Loaded in stored data user entered previously');
      },
      bind() {
        Exp.cache.input.addEventListener('blur', () => {
          ...
          if (val) {
            ...
            localStorage.setItem('FL013_checkoutGuest', encodeURIComponent(JSON.stringify(Exp.cache.checkoutJSON)));
          }
        });
      },
    },
...
const RunDeliveryChoices = () => {
  ...
  const Exp = {
    ...
    cache: (() => {
      ...
      const storedJSON = JSON.parse(decodeURIComponent(localStorage.getItem('FL013_deliveryForm')));
    ...
    components: {
      loadStorage() {
        Exp.cache.firstName.value = Exp.cache.checkoutJSON.firstName;
        Exp.cache.lastName.value = Exp.cache.checkoutJSON.lastName;

        if (Exp.cache.checkoutJSON.country !== '') {
          Exp.cache.country.value = Exp.cache.checkoutJSON.country;
        }

        if (Exp.cache.checkoutJSON.addressLine1 !== '') {
          Exp.cache.addressLine1.value = Exp.cache.checkoutJSON.addressLine1;
        }

        Exp.cache.addressLine2.value = Exp.cache.checkoutJSON.addressLine2;
        Exp.cache.town.value = Exp.cache.checkoutJSON.town;
        Exp.cache.countyState.value = Exp.cache.checkoutJSON.countyState;
        Exp.cache.postcode.value = Exp.cache.checkoutJSON.postcode;
        Exp.cache.mobileNumber.value = Exp.cache.checkoutJSON.mobileNumber;

        if (Exp.cache.checkoutJSON.billingAdd === true) {
          Exp.cache.billingAdd.checked = true;
        } else if (Exp.cache.checkoutJSON.billingAdd === false) {
          Exp.cache.billingAdd.checked = false;
        }

        const { settings } = Exp;
        events.send(settings.ID, 'Load', 'Loaded in stored data user entered previously Delivery Options');
      },
      bind() {
        Exp.cache.firstName.addEventListener('blur', () => {
          ...
          localStorage.setItem('FL013_deliveryForm', encodeURIComponent(JSON.stringify(Exp.cache.checkoutJSON)));
        });
...
const RunDeliveryOptions = () => {
  ...
  const Exp = {
    ...
    cache: (() => {
      ...
      const storedJSON = JSON.parse(decodeURIComponent(localStorage.getItem('FL013_pickupOption')));
      let checkoutJSON = {
        postcode: '',
      };
    ...
    components: {
      loadStorage() {
        Exp.cache.postcode.value = Exp.cache.checkoutJSON.postcode;
        const { settings } = Exp;
        events.send(settings.ID, 'Load', 'Loaded in stored data user entered previously Collection');
      },
      bind() {
        ...
        Exp.cache.postcode.addEventListener('blur', () => {
          ...
          if (val !== '') {
            Exp.cache.checkoutJSON.postcode = val;
            localStorage.setItem('FL013_pickupOption', encodeURIComponent(JSON.stringify(Exp.cache.checkoutJSON)));
          }
        });
      },
    },
...
const RunHomeDelivery = () => {
  ...
  const Exp = {
    ...
    cache: (() => {
      ...
      const billingAdd = bodyVar.querySelector('#chkUseAsBillingAddress');
      const mobileNumber = bodyVar.querySelector('#HomeDeliveryWrapper .TelephoneLine .form-control');
      const storedJSON = JSON.parse(decodeURIComponent(localStorage.getItem('FL013_deliveryHome')));
    ...
    components: {
      loadStorage() {
        ...
        const { settings } = Exp;
        events.send(settings.ID, 'Load', 'Loaded in stored data user entered previously Home Delivery');
      },
      bind() {
        Exp.cache.firstName.addEventListener('blur', () => {
          const val = Exp.cache.firstName.value;
          ...
          Exp.cache.checkoutJSON.firstName = val;
          localStorage.setItem('FL013_deliveryHome', encodeURIComponent(JSON.stringify(Exp.cache.checkoutJSON)));
        });
...
const RunDeliveryType = () => {
  ...
  const Exp = {
    ...
    cache: (() => {
      ...
      const storedJSON = JSON.parse(decodeURIComponent(localStorage.getItem('FL013_deliveryType')));
      let checkoutJSON = {
        standardDel: false,
        nextdayDel: false,
      };
    ...
    components: {
      loadStorage() {
        if (Exp.cache.checkoutJSON.standardDel === true) {
          Exp.cache.standardDel.querySelector('label').click();
        } else if (Exp.cache.checkoutJSON.nextdayDel === true) {
          Exp.cache.nextdayDel.querySelector('label').click();
        }
        const { settings } = Exp;
        events.send(settings.ID, 'Load', 'Loaded in stored data user entered previously Delivery Type');
      },
      bind() {
        Exp.cache.standardDel.addEventListener('click', () => {
          Exp.cache.checkoutJSON.standardDel = true;
          localStorage.setItem('FL013_deliveryType', encodeURIComponent(JSON.stringify(Exp.cache.checkoutJSON)));
        });
...
const RunVoucher = () => {
  ...
  const Exp = {
    ...
    cache: (() => {
      ...
      const storedJSON = JSON.parse(decodeURIComponent(localStorage.getItem('FL013_giftcard')));
      let checkoutJSON = {
        cardnumber: '',
        pincode: '',
      };
    ...
    components: {
      loadStorage() {
        Exp.cache.cardnumber.value = Exp.cache.checkoutJSON.cardnumber;
        Exp.cache.pincode.value = Exp.cache.checkoutJSON.pincode;
        const { settings } = Exp;
        events.send(settings.ID, 'Load', 'Loaded in stored data user entered previously Voucher');
      },
      bind() {
        Exp.cache.cardnumber.addEventListener('blur', () => {
          const val = Exp.cache.cardnumber.value;
          Exp.cache.checkoutJSON.cardnumber = val;
          localStorage.setItem('FL013_giftcard', encodeURIComponent(JSON.stringify(Exp.cache.checkoutJSON)));
        });
