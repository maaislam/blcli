...
const activate = () => {
  ...
  if (settings.VARIATION === '3') {
    ...
    document.body.classList.add('FL041-v3');
  }
  ...
  pollerLite(['.FL041-quickview a'], () => {
    setTimeout(() => {
        // Cache all quick view links
      let quickViewLinks = document.querySelectorAll('.FL041-quickview a');
      console.log('quick link ', quickViewLinks);
    ...
    observer.connect(productContainer, () => {
      if (!document.querySelector('.FL041-quickview')) {

        // Re cache
        products = document.querySelectorAll('#productlistcontainer ul.s-productscontainer2 li');
        addQuickView(products);
        // Re cache links
        const quickViewLinks = document.querySelectorAll('.FL041-quickview a');
        addFunctionality(quickViewLinks);
      }
    });
  })
