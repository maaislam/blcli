...
function showErrorMessage(messageToShow, ref) {
  if (messageToShow && ref) {
    ref.insertAdjacentHTML('beforeend', `
      <div class="FL041-error SelectSizePopover popover bottom in">
        <div class="arrow"></div>
...
function addQuickView(products) {
  if (products.length) {
    Array.from(products).map((product) => {
      ...
      if (productRef && link) {
        if (!product.querySelector('.FL041-quickview')) {
      
          productRef.insertAdjacentHTML('afterend', `
            <div class="FL041-quickview">
              <a href="${link}"><span class="FL041-qv-icon"></span> QUICK VIEW</a>
            </div>
          `);
        }
      }
      ...
      if (request.status >= 200 && request.status < 400) {
        ...
        if (data) {
          ...
          images = [].slice.call(images).map((img) => {
            ...
            let url = '';
            // console
            let src = imgRent.getAttribute('srczoom');
            if (!src) {
              src = imgRent.getAttribute('href');
            }
          ...
          // console.log(JSON.parse(productObj.variants[0].dataset.variants));
          cb(productObj);
        }
...
function popup(elObj, product) {
  if (elObj && product) {
    ...
    slider.classList.add('FL041-slider');
    ...
    if (ref) {
      ref.insertAdjacentHTML('beforebegin', `
        <div data-svid="${vsID}" class="FL041-popup">
          <div class="FL041-popup--close">
            <div>
              <span></span>
              <span></span>
            </div>
          </div>
          <div class="FL041-popup--title">
            <h2>${elObj.title.outerHTML}</h2>
            <p class="price">${elObj.price.outerHTML}</p>

            <a href="${elObj.link}" class="all-details">VIEW FULL DETAILS</a>
          </div>

          <div class="FL041-img--slider">
            <div class="FL041-prev"><span></span></div>
            ${slider.outerHTML} 
            <div class="FL041-next"><span></span></div>
          </div>

          <div class="FL041-info">
            <h3>${elObj.code.outerHTML}</h3>
            <p>${excerpt}... <a href="${elObj.link}">read more.</a></p>

            <div class="FL041-options" id="productDetail">
      ...
      if(elObj && elObj.chosenColCode) {
        const select = document.querySelector('.FL041-info .FL041-options select#colourDdl');
    ...
    pollerLite([() => {
      ...
      const $slider = $('.FL041-popup .FL041-slider');
      ...
      if ($slider) {
        $.loadScript('https://cdnjs.cloudflare.com/ajax/libs/slick-carousel/1.9.0/slick.min.js', () => {
          ...
          $($slider).slick({
            dots: true,
            arrows: true,
            prevArrow: $('.FL041-prev'),
            nextArrow: $('.FL041-next'),
          });
      
        });
      }
      const sliderWrap = document.querySelector('.FL041-popup .FL041-img--slider');
      sliderWrap.addEventListener('click', (e) => {
        if (e.target.classList.contains('FL041-prev') || e.target.classList.contains('FL041-next')) {
          events.send(settings.ID, 'Click', 'Navigated through images');
        }
      });

      const popupSelect = document.querySelector('.FL041-popup select.SizeDropDown');
...
function closePopup(event, thisPopup) {
  if (event.currentTarget) {
    // const thisPopup = document.querySelector('.FL041-popup');
...
function getProduct() {
  let reuturnedID;
  const currentID = document.querySelector('.FL041-popup').parentElement.getAttribute('li-productid');
...
function changeVariation(e) {
  ...
  const sizeSelect = document.querySelector('.FL041-popup .swapSize select.SizeDropDown');
  if (sizeSelect) {
  ...
  const imageSlider = document.querySelector('.FL041-popup .FL041-img--slider .FL041-slider');
  ...
  const $newSlider = $('.FL041-popup .FL041-slider');
  if ($newSlider) {
    $.loadScript('https://cdnjs.cloudflare.com/ajax/libs/slick-carousel/1.9.0/slick.min.js', () => {
      $newSlider.slick({
        dots: true,
    ...
    for (let i = 0; cachedLinks.length > i; i += 1) {
      ...
      cachedLinks[i].addEventListener('click', (e) => {
        ...
        fetchProductDetails(thisLink, (obj) => {
          ...
          // Disable body scroll
          document.body.classList.add('FL041-noscroll');

          const close = document.querySelector('.FL041-popup .FL041-popup--close');
          if (close) {
            close.addEventListener('click', (e) => {
              closePopup(e, thisPopup);
              // Enable body scroll
              document.body.classList.remove('FL041-noscroll');
          ...
          const atbCta = document.querySelector('.FL041-popup .ImgButWrap a');
          if (atbCta) {
            atbCta.addEventListener('click', (atbEvent) => {
              events.send(settings.ID, 'Click', 'Add to bag');
              // Get current size
              const sizeSelect = document.querySelector('.FL041-popup #productVariantAndPrice select.SizeDropDown');
              ...
              if (!chosenSize) {
                ...
                const errorMessage = thisPopup.querySelector('.FL041-error');
                if (errorMessage) {
                  setTimeout(() => {
              ...
              // Get current colour
              const colSelect = document.querySelector('.FL041-popup #divColour select');
              const chosenCol = colSelect.options[colSelect.selectedIndex].value;
              ...
              addToBag(thisChosenProductId, ((result) => {
                ...
                const successMessage = document.querySelector('.FL041-success');
                if (successMessage) {
                  setTimeout(() => {
          ...
          if (changeSizeSelect) {
            const noSizeOptions = changeSizeSelect.querySelectorAll('option.greyOut');
            if (noSizeOptions.length) {
              for (let j = 0; noSizeOptions.length > j; j += 1) {
          ...
          // More info event
          const moreInfoLink = document.querySelector('.FL041 .FL041-popup .FL041-popup--title a.all-details');
          if (moreInfoLink) {
