...
import getMaleTab from './lib/maleTab';
import getFemaleTab from './lib/femaleTab';
...
const opts = typeof options === 'object' ? options : localOptions;
const { data } = opts;
const maleTab = getMaleTab(data);
const femaleTab = getFemaleTab(data);
  ...
  const Exp = {
    ...
    components: {
      ...
      contentBuilder() {
        ...
        const helpNav = Exp.cache.bodyVar.querySelectorAll('.item-level-1.nav-help')[1];
        helpNav.parentElement.removeChild(helpNav);
      },
      contentBuilderMB() {
