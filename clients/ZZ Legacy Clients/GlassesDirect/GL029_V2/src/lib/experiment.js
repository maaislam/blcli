...
const basketPage = () => {
  ...
  const Exp = {
    cache: (() => {
      ...
      // Used to target the correct product
      ...
      const POSTRequestData = {
        ...
        Vision: [`csrfmiddlewaretoken=${CSRFToken}&vision_type_name=${eligibleProduct.querySelector('.product-detail--vision .detail-value').textContent.trim()}`, `https://www.glassesdirect.co.uk/basket/personalise/${productCount}/vision/`],
      ...
      // Product link to redirect to on request failures: getting SKU, adding to basket
      // Currently set eligible product, will update if carousel product is used
      // eslint-disable-next-line
      let productRedirectURL = `https://www.glassesdirect.co.uk/${eligibleProduct.querySelector('.product-name').getAttribute('href')}`;
      return {
        docVar,
        bodyVar,
        eligibleProduct,
    ...
    services: {
      /**
       * @desc Descibe function code
       * @param {Element} element - Detail all parameters, separately
       * @returns {Element || Boolean} - Return type if any
       */
      /**
       ...
       */
      createRequestObject() {
        // Replace any instance of 'transitions' to transition
        const tintType = Exp.cache.eligibleProduct.querySelector('.product-detail--tint .detail-value').textContent.trim().replace(/transitions/gi, 'transition');
        ...
        if (tintType.toUpperCase() === 'BLUEREFLECT') {
          ...
          // Otherwise, parse tint type
        } else if (tintType.toUpperCase() !== 'NO TINTS') {
          Exp.cache.POSTRequestData.Tint[0] += this.parseProductDetail(tintType);
        }
        // Package type
        Exp.cache.POSTRequestData.Package[0] += this.parseProductDetail(Exp.cache.eligibleProduct.querySelector('.product-detail--package .detail-value').textContent.trim());
    ...
    render: {
      ...
      productCarousel() {
        ...
        GD029SlickParent.querySelector('.slick-track').addEventListener('click', (e) => {
          ...
          if (e.target.classList.contains('GD029_Carousel_Add')) {
            ...
            Exp.cache.productRedirectURL = `https://www.glassesdirect.co.uk/${productContainer.querySelector('.GD029_Link').getAttribute('href')}`;
            ...
            // Product has SKU, start post requests
      ...
      requestModal() {
        ...
        Exp.cache.bodyVar.insertAdjacentHTML('beforeend', `
          ...
          `);
        // Store References if a related product is added
    ...
    bindExperimentEvents: {
      // Click the bottom complete purchase button
      continueCheckout() {
        Exp.cache.completePurchaseButton.click();
      },
    },
    handleSecondProduct: {
      /**
       * @desc Redirects to a link provided
       * @param {String} productLink - Expects a string link of hte page to redirect to
       */
      redirectProductPage(productLink) {
      ...
      * arguments passed are the route for posting and data to POST
      */
      ...
      /**
       * @desc Handles sending data to skip the lens flow, recieved the route to post the data to
       ...
       * or to end the request which reloads the page, on any error realod the page as the product
