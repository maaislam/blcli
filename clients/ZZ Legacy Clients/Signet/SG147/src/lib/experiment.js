...
export default () => {
  ...
  if(VARIATION !== 'control') {
    ...
    // 2 image with CTA's and offer carousel
    if(VARIATION === '1') {
    ...
    // changing grid tiles but adding categories below
    if(VARIATION === '2') {
      changeInnerTiles();

      // add new category block 
      const categories = document.createElement('div');
      categories.className = `${ID}-categories`;
      categories.innerHTML = `
      <div class="${ID}-middle ${ID}-ctaWrap">
        <h3>Shop Our Categories</h3>
        <div class="${ID}-buttons"></div>
      </div>`;
      document.querySelector('.home-tile-grid').insertAdjacentElement('afterend', categories);
