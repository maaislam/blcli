 ...
 */
import { setup, getSiteFromHostname, cookieOpt } from './services';
...
export default () => {
  if(VARIATION == '1') {
    ...
    pollerLite(['.paysum__paysumrow'], () => {
      ...
      const payLogos = document.querySelectorAll('.container__finance-display-section .c-card-list li');
      if(payLogos) {
        for (let index = 0; index < payLogos.length; index += 1) {
          const element = payLogos[index];
          if(element.querySelector('img').getAttribute('alt') === 'We accept Clearpay') {
            element.style.display = 'none';
          }
        }
      }
