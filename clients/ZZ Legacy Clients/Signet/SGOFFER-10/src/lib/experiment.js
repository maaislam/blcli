...
export default () => {
  ...
  if (window.location.href.indexOf("webstore/l/") > -1) {
    document.documentElement.classList.add(`${ID}-plp`);
      ...
      const createBadge = (el) => {
        ...
        badge.innerHTML = `<span>${window.products.offerPercentage} OFF*</span>`;
        el.appendChild(badge);
        if(window.products.discountAppliedAtBasket === true) {
      ...
      const checkProducts = () => {
        ...
        for (let index = 0; index < allProducts.length; index += 1) {
          ...
          if (window.products.includedSKUS.indexOf(elSku) > -1) {
            createBadge(element);
          }
        }
      }
  ...
  // IF PDP
  if(window.digitalData.page.pageInfo.pageType === 'PDP' && !document.querySelector(`.${ID}-offer-message`)) {
    ...
    const addOfferMessage = () => {
      ...
      <h3>OFFER</h3>
      <p>${window.products.PDPmessage}</p>`;
