...
export default () => {
  ...
  if(VARIATION !== 'control') {
    ...
    let date;
    if(window.SGData && window.SGData.countdownDate) {
      date = window.SGData.countdownDate;
    } else {
      date = "May 01, 2023 15:37:25";
    }

    let text;
    if(window.SGData && window.SGData.countdownText) {
      text = window.SGData.countdownText;
    } else {
    ...
    const addBanner = () => {
      ...
      <div class="${ID}-deliveryCountdown">
        <div class="${ID}-container">
          <div class="${ID}-countdown-text">
            <p><strong>${text}</strong><em> ends in...</em></p>
          </div>
      ...
      document.querySelector('.main-header').insertAdjacentHTML('afterend', deliveryBanner);
    }
