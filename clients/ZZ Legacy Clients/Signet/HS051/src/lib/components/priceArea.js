...
export default () => {
    ...
    // move buttons 
   orderSummary.querySelector('.order-summary').appendChild(document.querySelector('#lower-button-group'));
    ...
    const voucherBox = document.querySelector('#promo-code-content');
    if(voucherBox.querySelector('.cta--secondary')) {
        voucherBox.querySelector('.cta--secondary').textContent = 'Apply';
        if(window.innerWidth > 767) {
            document.querySelector('.basket-table').appendChild(voucherBox);
        } else {
            orderSummary.insertAdjacentElement('afterbegin', voucherBox);
        }
    }
    ...
    const totalSection = () => {
        ...
        if(errorVoucher) {
            if(errorVoucher.textContent !== '' && document.querySelector('#couponCodeRemove')) {
