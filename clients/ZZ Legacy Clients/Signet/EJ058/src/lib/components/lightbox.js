...
export default class Lightbox {
  ...
  render() {
    ...
    if(!sessionStorage.getItem(`${cache.ID}-lightboxShown`, 1)) {
  ...
  open() {
    ...
    events.send(`${cache.ID} v${shared.VARIATION}` , 'Opened', `${cache.ID} pop up seen`);
  ...
  close() {
    ...
    sessionStorage.setItem(`${cache.ID}-lightboxShown`, 1);
