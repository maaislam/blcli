...
export default () => {
  ...
  if(window.digitalData.page.pageInfo.pageType === 'Landing') {
    //if(localStorage.getItem(`${ID}-citizen`)){
        pollerLite(['.home-tile-grid'], () => {
          if(VARIATION === '1') {
            fireEvent('Citizen stored - banners shown');
            document.documentElement.classList.add(`${ID}-citizen`);
            new HomePageGrid();
          } else {
            fireEvent('Citizen stored - control');
          }
       }); 
     //} 
  }
