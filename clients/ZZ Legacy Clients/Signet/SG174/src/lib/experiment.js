...
export default () => {
  ...
  //     localStorage.setItem(`${ID}-bulova`, 1);
  //   } else {
  //     localStorage.removeItem(`${ID}-bulova`);
  //   }
  // }

  // // store on PLP & PDP
  // if(window.digitalData.page.pageInfo.pageType === 'PLP') {
  //   if(window.location.href.toLowerCase().indexOf('bulova') > -1) {
  //     localStorage.setItem(`${ID}-bulova`, 1);
  //   } else {
  //     localStorage.removeItem(`${ID}-bulova`);
  //   }
  ...
  if(window.digitalData.page.pageInfo.pageType === 'Landing') {
    //if(localStorage.getItem(`${ID}-bulova`)){
        pollerLite(['.home-tile-grid'], () => {
          if(VARIATION === '1') {
            fireEvent('bulova stored - banners shown');
            document.documentElement.classList.add(`${ID}-bulova`);
            new HomePageGrid();
          } else {
            fireEvent('bulova stored - control');
          }
       }); 
    //} 
  }
