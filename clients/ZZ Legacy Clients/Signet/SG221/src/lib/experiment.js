...
export default () => {
  ...
  if(VARIATION !== 'control') {
    ...
    const addBadge = () => {
      ...
      badge.innerHTML = `<span>30% OFF</span>`;
    ...
    const addOfferMessage = () => {
      ...
      <h3>Save 30%</h3>
      <p>Save 30% on this product. Use Code JEWEL30 at basket.</p>`
      
      // Offer ends in: <span class="${ID}-countdown"></span></p>`;
    ...
    const countdown = () => {
      var countDownDate = new Date("Nov 25, 2022 23:59:59").getTime();
    ...
    const changeFinance = () => {
      ...
      if(price >= 430) {
        ...
        From <span class="bf-finance">£${monthlyPayment}</span> p/m with 0% interest free credit when saving 30% with code JEWEL30. Usually <span class="normal-finance">${financePrice[0]}</span> p/m.
