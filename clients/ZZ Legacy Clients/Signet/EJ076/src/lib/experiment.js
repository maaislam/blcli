...
export default () => {
  ...
  const createBrandBar = () => {
    ...
    if(window.location.href.indexOf('watches.do') > -1) {
      ...
      pollerLite(['.EJ050-heroBanner'], () => {
        if(VARIATION !== '3') { 
          document.querySelector(`.EJ050-heroBanner`).insertAdjacentElement('afterend', brandBar);
        } else if(VARIATION === '3') {
          document.querySelector('.EJ050-heroBanner').insertAdjacentElement('beforebegin', brandBar);
        }    
      });
    }
