...
export default () => {
  ...
  if(VARIATION !== 'control') {
    ...
    const newImage = (src) => {
      const newImage = `<div class="${ID}-mainImg"><div class="${ID}-container"><img src="${src}"/></div></div>`;
      return newImage;
    }

    const firstImage = document.querySelector('.swiper-container.product-gallery .product-gallery__image-container img');

    if(firstImage) {
      const firstImageSrc = firstImage.getAttribute('src');
      document.querySelector('.product-gallery__main ').insertAdjacentHTML('afterbegin', newImage(firstImageSrc));
    }
