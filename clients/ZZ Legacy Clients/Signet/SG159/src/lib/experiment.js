...
export default () => {
  ...
  const checkCS = setInterval(function () {
    if (window._uxa) {
  ...
  if(window.digitalData.page.pageInfo.pageType === 'PDP') {
    if(brands.indexOf(window.digitalData.product[0].productInfo.brand) > -1) {
      localStorage.setItem(`${ID}-fossil`, 1);
    } else {
      localStorage.removeItem(`${ID}-fossil`);
    }
  }

  // store on PLP & PDP
  if(window.digitalData.page.pageInfo.pageType === 'PLP') {
    if(window.location.href.indexOf('fossil') > -1) {
      localStorage.setItem(`${ID}-fossil`, 1);
    } else {
      localStorage.removeItem(`${ID}-fossil`);
    }
  ...
  if(window.digitalData.page.pageInfo.pageType === 'Landing') {
    if(localStorage.getItem(`${ID}-fossil`)){
        pollerLite(['.home-tile-grid'], () => {
          if(VARIATION === '1') {
            fireEvent('Fossil stored - banners shown');
            document.documentElement.classList.add(`${ID}-fossil`);
            new HomePageGrid();
          } else {
            fireEvent('Fossil stored - control');
          }
        }); 
      } 
  }
