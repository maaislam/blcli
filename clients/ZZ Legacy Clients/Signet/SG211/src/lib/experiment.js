...
export default () => {
  ...
  if(VARIATION !== 'control') {
    ...
    document.querySelector('#js-product-gallery').swiper.removeSlide([1]);
    document.querySelector('#js-product-gallery').swiper.destroy(true, true);

    const newSwiper = new Swiper('.product-gallery', {
      pagination: {
        el: '.swiper-pagination',
        clickable: true,
      },
      navigation: {
        nextEl: '#js-product-gallery .swiper-button-next-first',
        prevEl: '#js-product-gallery .swiper-button-prev-first',
        clickable: true,
      },
    });
