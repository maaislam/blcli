...
export default class HelpBox {
  ...
  bindEvents() {
    ...
    component.querySelector(`.${ID}-book`).addEventListener('click', (e) => {
      e.preventDefault();
      events.send('EJ008 V1', 'click', 'Clicked Book Instore', {
        nonInteraction: true
      });
      window.open('https://booking.ernestjones.co.uk/#/map', '_blank');
    });
    component.querySelector(`.${ID}-store`).addEventListener('click', (e) => {
      e.preventDefault();
      events.send('EJ008 V1', 'click', 'Clicked Find Instore', {
        nonInteraction: true
      });
      window.open('https://www.ernestjones.co.uk/webstore/secure/storeLocator.sdo?icid=ej-fn-ess-store', '_blank');
    });
  }
