...
export default () => {
  ...
  if(VARIATION !== 'control') {
    ...
    if(window.GWPData && window.GWPData.GWPimage) {
      const giftBlock = 
      `<div class="${ID}-gwp-block">
        <div class="${ID}-container">
          <div class="${ID}-image">
            <a href="${window.GWPData.GWPlink}">
              <img src="${window.GWPData.GWPimage}"/>
            </a>
          </div>
        </div>
      </div>`;

      document.querySelector('.product-detail__add-to-cart').insertAdjacentHTML('afterend', giftBlock);
    }
