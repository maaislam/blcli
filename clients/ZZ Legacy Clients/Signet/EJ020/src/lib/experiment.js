...
const activate = () => {
  ...
  if (settings.VARIATION === '1') {
    const productChanges = () => {
      ...
      for (let index = 0; index < allProducts.length; index += 1) {
        ...
        if (wasPrice) {
          ...
          const newWasPrice = document.createElement('div');
          newWasPrice.classList.add(`${ID}-wasPrice`);
          newWasPrice.innerHTML = `<span>${wasPrice.querySelector('.product-tile__savings-price:last-child').textContent}</span>`;
          currentPrice.insertAdjacentElement('beforebegin', newWasPrice);
        }
      }
    };
    productChanges();
    ...
    observer.connect([document.querySelector('.product-tile-list.js-infinite-scroll')], () => {
      const oldPrice = document.querySelectorAll(`.${ID}-wasPrice`);
      if (oldPrice) {
        for (let index = 0; index < oldPrice.length; index += 1) {
          const element = oldPrice[index];
          element.remove();
        }
      }
      productChanges();
      ...
      for (let index = 0; index < allProducts.length; index += 1) {
        ...
        if (enlargeButton) {
          enlargeButton.addEventListener('click', (e) => {
            ...
            events.send('EJ020', 'click', 'Clicked Enlarge Image', {
              nonInteraction: true,
            });
