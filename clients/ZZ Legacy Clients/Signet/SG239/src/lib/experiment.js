...
export default () => {
  ...
  setTimeout(function () {
    var campaignName = `${ID} - V${VARIATION}`;
    var variationName = 'Conditions Met';
    ...
    ga('create', 'UA-31570-7', {
      'cookieDomain': 'none'
    });
    ga('send', 'event', 'Experimentation', '' + campaignName + '', '' + campaignName + ' ' + variationName, {
      nonInteraction: true
    });
    }, 1000);
  ...
  if(VARIATION !== 'control') {
    ...
    pollerLite(['.SG239-productPrice', '.product-price-offer'], () => {
      var priceOffer = document.querySelector('.product-price-offer');
      document.querySelector('.SG239-productPrice').insertAdjacentElement('beforebegin', priceOffer);
      priceOffer.classList.add('SG239');
    });
