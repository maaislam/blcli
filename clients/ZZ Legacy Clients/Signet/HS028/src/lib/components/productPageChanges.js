...
export default () => {
  ...
  const changeFinanceMessage = () => {
    ...
    if (financeMessage) {
      const price = financeMessage.textContent.trim().match(/[\d.]+/gm);
  ...
  const makeSalePriceRed = () => {
    ...
    if (wasPrice) {
      productPrice.style = 'color: #a41825;';
    }
  };
  makeSalePriceRed();
};
