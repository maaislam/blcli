...
export default () => {
    ...
    Object.keys(articles).forEach((i) => {
        ...
        article.classList.add(`${ID}-article`);
        article.innerHTML = 
        `<div class="${ID}-colLeft">
            <div class="${ID}-image" style="background-image:url(${data.image})"></div>
        </div>
        <div class="${ID}-colRight">
            <h3 class="${ID}-heading">${[i][0]}</h3>
            <p class="${ID}-paragraph">${data.description}</p>
            <a class="${ID}-contentLink" target="_blank" href="${data.link}">Learn More</a>
        </div>`;

        document.querySelector(`.${ID}-articles .${ID}-container`).appendChild(article);
    });

    // add attributes for scroll effects
    if(window.innerWidth > 767) {
        const article1 = document.querySelectorAll(`.${ID}-article`)[0];
        const article2 = document.querySelectorAll(`.${ID}-article`)[1];
        article1.setAttribute('data-aos', "fade-right");
        article2.setAttribute('data-aos', "fade-left");
    }
}