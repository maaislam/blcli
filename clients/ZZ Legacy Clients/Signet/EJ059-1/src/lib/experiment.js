...
export default () => {
  ...
  if(shared.VARIATION === '1') {
    ...
    } else if(brand === 'Bremont') {
      document.body.classList.add(`${shared.ID}-bre`);
    } else {
    ...
    const video = document.querySelector(`.${shared.ID}-video video`);
      if(video) {
          video.onplaying = function() {
            events.send(`EJ059 v${shared.VARIATION}`, 'click', 'play video', { sendOnce: true });
          }
      };
    ...
    for (let index = 0; index < otherWatch.length; index += 1) {
      ...
      if(element) {
        element.addEventListener('click', () => {
          events.send(`EJ059 v${shared.VARIATION} brand: ${brand}`, 'click', 'clicked product rec');
        });
      }
    }  
  }
