...
const PU006 = (() => {
    const activate = () => {
        const $ = window.jQuery,
             $body = $('body');
		$body.addClass('PU006v2');

		if(!utils.getCookie('PU6v2_boxShown')){
			//detect when use gets to half way down the page then show box	
			setTimeout(function(){
				showdoctorsBox();
			},3000); 
		}
		...
		const doctorsBox = () => {
			...
			$.getScript('//cdnjs.cloudflare.com/ajax/libs/slick-carousel/1.8.1/slick.min.js', function() { 
				...
				$slider.on('afterChange', function(event, slick, currentSlide){   
				  ...
				  $('.slick-next.slick-arrow').click(function(){
					utils.events.send('PU006 V2', 'Next click', 'clicked on next slide on module', {
						sendOnce: true
					});
				  });

				  //on cta click
				  $('.PU6-book').click(function(){
					utils.events.send('PU006 V2', 'Book click', 'clicked on book an appointment', {
						sendOnce: true
					});
			...
			$('.PU6_box_wrap a').click(function(){
				utils.events.send('PU006 V2', 'Link click', 'clicked on see all GPs', {
					sendOnce: true
				});
			});
		...
		const showdoctorsBox = () => {
			...
			utils.events.send('PU006 V2', 'Box Shown', 'Module box shown', {
				sendOnce: true
			});
			utils.setCookie('PU6v2_boxShown',1);
		}
    ...
    const triggers = ((options) => {
        UC.poller([
            ...
            () => {
             ...
             $ = window.jQuery;
             utils.fullStory('PU006', 'Variation 2');
             activate();
            });
    })();
