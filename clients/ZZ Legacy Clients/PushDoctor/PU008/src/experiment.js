...
import { videoArea } from './lib/PU008-content.js';
          ...
          videobox_wrapper.remove();
			...
			const contentBlocks = () => {
              ...
              $thisTitle.addClass('PU8-contentOpen');
					...
					//DIFFERENCE BETWEEN V1 & V2 - PRICING IS SHOWN
					pricingBlock.find('.PU8-copyTitle').addClass('PU8-contentOpen');
          pricingBlock.find('.PU8-content').addClass('PU8-content_showing');
        ...
        uspAnchors = $('.PU8-anchor');
			...
			const events = () => {
				const bookNow = $('.PU8-book'),
					learnMore = $('.PU8-anchor:first'),
					getAppLink = $('.PU8-applink'),
					meetDoctorsLink = $('.PU8-doctorLink'),
					videoPlay = $('.PU8-video_container'),
					whatWeTreatLink = $('.PU8-link.PU8-sectionHeader a'),
					pricingLink = $('.PU8-pricingLink'),
					accordionTabs = $('.PU8-copyBlock'),
					treatmentInnerLink = $('.PU8-treatment_links'),
					childLink = $('.PU8-childLink');

				

				learnMore.click(function(){
					utils.events.send('PU008', 'Learn more click','PU008 clicked Need a prescription? Learn More link', {
                        sendOnce: true
                    });
				});
				bookNow.click(function(){
					utils.events.send('PU008', 'Book appointment click','PU008 clicked book an appointment', {
                        sendOnce: true
                    });
				});
				getAppLink.click(function(){
					utils.events.send('PU008', 'get app click','PU008 clicked get the app link', {
                        sendOnce: true
                    });
				});
				meetDoctorsLink.click(function(){
					utils.events.send('PU008', 'meet doctors click','PU008 clicked meet our doctors anchor', {
                        sendOnce: true
                    });
				});
				whatWeTreatLink.click(function(){
					utils.events.send('PU008', 'what we treat click','PU008 clicked on what we treat link', {
                        sendOnce: true
                    });
				});
				pricingLink.click(function(){
					utils.events.send('PU008', 'learn more about pricing click','PU008 clicked on find out about pricing link', {
                        sendOnce: true
                    });
				});
				accordionTabs.click(function(){
					utils.events.send('PU008', 'Opened the tabs','PU008 opened the Concertina tabs', {
                        sendOnce: true
                    });
				});
				treatmentInnerLink.click(function(){
					utils.events.send('PU008', 'what we treat click','PU008 Inner what we treat links clicked in the tabs', {
                        sendOnce: true
                    });
				});
				childLink.click(function(){
					utils.events.send('PU008', 'child consultations','PU008 learn more about child consultations link', {
                        sendOnce: true
                    });
				});
			}
...
// Audience conditions
	const triggers = ((options) => {
		UC.poller([
			...
			$ = window.jQuery;
			utils.fullStory('PU008', 'Variation 1');
			activate();
		});
	})();
