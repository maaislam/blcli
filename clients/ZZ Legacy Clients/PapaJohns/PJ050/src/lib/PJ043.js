...
const PJ043 = {
  ...
  init() {
    // Setup
    const { settings, services, components } = PJ043;
    services.tracking();
  ...
  components: {
    ...
    changeButtonText: () => {
      ...
      for (let index = 0; index < buttons.length; index += 1) {
        ...
        if (element.textContent.trim() !== 'Collection Only' && element.textContent.trim() !== 'Delivery Only') {
