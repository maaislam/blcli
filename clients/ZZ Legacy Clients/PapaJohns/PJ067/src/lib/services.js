...
function generateOfferBlock(type, offerInfo){
  const reg = /[+-]?\d+(\.\d+)?/g;
      ...
      <div class="${ID}_offer">
        <div class="${ID}_offer__content">
          <div class="${ID}_offer__header">
            <h4 class="${ID}_offer__title">${offerInfo.discountCellText}</h4>
          </div>
          <!--End header-->
          <div class="${ID}_offer__body">
            <div class="${ID}_offer__sliderWrap">
              <h5 class="${ID}_offer__title"></h5>
              <span class="${ID}_offer__price ${ID}_offer__price--target">£${(total + offerInfo.outstandingValue).toFixed(2)}</span>
              <div class="${ID}_offer__slider">
                <span class="${ID}_offer__bar"></span>
              </div>
              <span class="${ID}_offer__price"><i class="fa fa-star"></i></span>
            </div>
          </div>
          <!--End body-->
        </div>
...
function modifycounter(offerInfo) {
  const outstanding = offerInfo.outstandingValue || 0;
  /*
  ...
  */
  ...
  if(!outstanding || outstanding <= 0){
    ...
    document.querySelector(`.${ID}_offer__sliderWrap h5.${ID}_offer__title`).innerHTML = offerInfo.outstandingMessage;
    ...
    document.querySelector(`.${ID}_offer__sliderWrap .${ID}_offer__title`).innerHTML = offerInfo.outstandingMessage;

    target.setAttribute('style', `width:${(100 * total / parseFloat(total + outstanding)).toFixed(0)}%`);

    target.parentElement.setAttribute('data-offer', (100 * (total) / parseFloat(total + outstanding)).toFixed(0));

    document.querySelector(`.${ID}_offer__sliderWrap`).classList.remove(`${ID}_offer__sliderWrap--completed`);

    const targetPrice = document.querySelector(`.${ID}_offer__price--target`);
    if(targetPrice) {
      targetPrice.innerHTML = `£${(total + offerInfo.outstandingValue).toFixed(2)}`;
    }
  }
}
