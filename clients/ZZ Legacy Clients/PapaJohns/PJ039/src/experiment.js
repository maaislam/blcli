...
const Experiment = {
  ...
  init() {
    ...
    /* eslint-disable */
      ...
      window.prm.add_pageLoaded(function (sender, error) {
        try {
          if(!document.querySelector('.PJ039-category')){
            components.AddOfferTitle();
            components.createOfferGroups();
            components.loopThroughAttributes();
            components.assignTheTopOffers();
  ...
  components: {
    ...
    AddOfferTitle: function AddOfferTitle() {
      ...
      offerTitle.classList.add('PJ039-offer_title');
    ...
    createOfferGroups: function createOfferGroups() {
      ...
      groupedOffers.classList.add('PJ039-offersWrap');
      groupedOffers.innerHTML = `
      <div id="PJ039-moneyoff_deal" class="PJ039-category"><h3>% Or £ Off deals</h3><hr/></div>
      <div id="PJ039-pizzaonly_deal" class="PJ039-category"><h3>Pizza Only</h3><hr/></div>
      <div id="PJ039-meal_deal" class="PJ039-category"><h3>Meal Deals</h3><hr/></div>
      <div id="PJ039-other_deals" class="PJ039-category"><h3>View more deals</h3><hr/><div class="PJ039-other"></div></div>`;
      ...
      for (let i = 0; i < allOffers.length; i += 1) {
        ...
        if (dealTypes) {
          if (dealTypes[0] === 'moneyOff') {
            element.classList.add('PJ039-moneyoff_deal');
          }
          if (dealTypes[0] === 'mealDeal') {
            element.classList.add('PJ039-meal_deal');
          }
          if (dealTypes[0] === 'pizzaOnly') {
            element.classList.add('PJ039-pizzaonly_deal');
          }
        }
      }
    },
    ...
    */
    assignTheTopOffers: function assignTheTopOffers() {
      ...
      for (let i = 0; i < allOffers.length; i += 1) {
        ...
        [].forEach.call(topOffers, (item) => {
          if (offerTitle === item) {
            element.classList.add('PJ039-top_offer');
          } else {
            element.classList.add('PJ039-otherDeal');
          }
        });
      }
    },
    ...
    moveOffers: function moveOffers() { // To here - sort css of offers
      ...
      const categories = document.querySelectorAll('.PJ039-category');
      for (let i = 0; i < menuItems.length; i += 1) {
        const element = menuItems[i];

        [].forEach.call(categories, (item) => {
          if (item.id === element.classList[2] && element.classList.contains('PJ039-top_offer')) {
            item.appendChild(element);
          } else if (item.id === element.classList[2] && element.classList.contains('PJ039-otherDeal')) {
            document.querySelector('.PJ039-other').appendChild(element);
          }
    ...
    showHideDeals: function showHideDeals() {
      const { settings } = Experiment;
      const viewMore = document.querySelector('#PJ039-other_deals');
      const moreDeals = document.querySelector('.PJ039-other');
      viewMore.querySelector('h3').addEventListener('click', () => {
        if (moreDeals.classList.contains('PJ039-others_showing')) {
          moreDeals.classList.remove('PJ039-others_showing');
          viewMore.querySelector('h3').classList.remove('PJ039-otherTitle_showing');
        } else {
          events.send(settings.ID, 'Clicked', 'View more deals', { sendOnce: true });
          moreDeals.classList.add('PJ039-others_showing');
          viewMore.querySelector('h3').classList.add('PJ039-otherTitle_showing');
        }
      });
    },
    /**
    ...
    fixedfilterBar: function fixedfilterBar() {
      ...
      filterBar.classList.add('PJ039-filter_bar');
      filterBar.innerHTML = `
      <p>Deal Type:</p>
      <div class="PJ039-filter PJ039-filter_active" cat-attr="PJ039-moneyoff_deal"><span>All</span></div>
      <div class="PJ039-filter" cat-attr="PJ039-moneyoff_deal"><span>% or £ Off</span></div>
      <div class="PJ039-filter" cat-attr="PJ039-pizzaonly_deal"><span>Pizza Only</span></div>
      <div class="PJ039-filter" cat-attr="PJ039-meal_deal"><span>Meal Deals</span></div>`;
    ...
    filterAnchor: function filterAnchor() {
      const allFilters = document.querySelectorAll('.PJ039-filter');
      const categories = document.querySelectorAll('.PJ039-category');
      ...
      for (let i = 0; i < allFilters.length; i += 1) {
        ...
        element.addEventListener('click', (e) => {
          ...
          for (let j = 0; j < allFilters.length; j += 1) {
            allFilters[j].classList.remove('PJ039-filter_active');
          }
          e.target.parentNode.classList.add('PJ039-filter_active');
