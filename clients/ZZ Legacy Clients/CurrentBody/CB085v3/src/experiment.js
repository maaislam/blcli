...
var _CB085 = (function() {
	...
	---------------------------------------*/
		...
		function desktopHeader(){
			...
			$desktopWrap.filter('.CB85-searchRow').html(
				`<div class="CB85-desktoplogo"><a href="https://www.currentbody.com/"><img src="${$topImages.cblogo}"/></a></div>
          ...
          ['ZAR','CB85-ZAR'],
			...
			function desktopUSPs(){
				...
				//Move usps to top row
				desktopUSPbar.find('.CB85-usp_bar').prependTo('.CB85-right');
				$('.CB85-usp').hover(function(){
					utils.events.send('CB085 V3', 'Usp hover', 'CB085 V3 USP Hover', {
                        sendOnce: true
                    });
				});
				$('.CB85-usp').click(function(){
					utils.events.send('CB085 V3', 'Usp click', 'CB085 V3 USP Click', {
                        sendOnce: true
                    });
				});
		    }
	...
	var _triggers = function(options) {
		utils.fullStory('CB085', 'Variation 3');
		UC.poller([
			'.header-container',
