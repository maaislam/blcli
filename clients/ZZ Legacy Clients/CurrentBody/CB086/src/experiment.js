...
var CB086 = (function() {
	...
	var UCPoller = (function(){
		...
		UC.poller([
			'.grid-section-products .bx-wrapper .product-grid > li',
			function () {
				if (window.jQuery) {
					return true;
				}
	...
	function init(){
		...
		// Slick slider plugin unminified 
		...
		var slideshowRebuild = {
			cacheSlideContent: function(){
				cacheDom.gridOuter.each(function(){
					var el = JQ(this),
						elBXWrapper = el.find('.bx-wrapper'),
						BXSlider = elBXWrapper.find('.col-xs-12.col-sm-6.col-md-4');

					elBXWrapper.after('<div class="product-grid CB086_bx-slider"></div>')
					BXSlider.attr('style', '');

					BXSlider.each(function(){
						JQ(this).appendTo(el.find('.CB086_bx-slider.product-grid'));
					})
		...
		var gridContentFunc  = function () {
			...
			var $products = JQ('.grid-section-products .CB086_bx-slider.product-grid > li');
			...
			$products.each(function (i) {
				if ((i + 1) % productsPerBanner === 0) {
					...
					var $hotspot = JQ([
						'<li class="col-xs-12 col-sm-6 col-md-4 CB086_banner">',
		...
		slideshowRebuild.cacheSlideContent();	
		gridContentFunc();
		slideshowRebuild.landingSliderInit();
