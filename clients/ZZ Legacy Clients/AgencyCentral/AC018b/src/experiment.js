...
import flickerPrevention from './lib/flickerPrevention.js';
		...
		const header = {
			...
			locationMarkup(){
				...
				if(doc.querySelector('.AC018_location-text-reveal p:first-child').innerText.length > 200) {
					let subText = doc.querySelector('.AC018_location-text-reveal p:first-child').innerHTML.substring(0, 200).trim();
					...
					if(doc.querySelector('.AC018_location-text-reveal p:first-child + p')){
						...
						let stringTotal = 200 - subText.length;
		...
		const search = {
			...
			moveElements(){
				...
				for(let i = resultsBody.length - 1; i > -1; i--){
					...
					let contactOptions = resultsCurrent.querySelector('.AC018_contact-options');
          ...
          // If the website option exists move it
					...
					if(resultJQ.find('.agency-address-line > .location').length > 0){
						resultJQ.find('.agency-address-line > .location').prependTo(resultJQ.find('.agency-address-line'));
					}
			...
			bindClick(){
				const searchBtn = $('#search-bar-expose-button'),
					navWrap = $('.AC018_search-refine');

				searchBtn.off('click');

				// Unbind current click to then add the new click
