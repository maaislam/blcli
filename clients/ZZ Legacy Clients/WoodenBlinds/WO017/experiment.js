...
const run = () => {
    ...
    validators.add('enter-measurements', (currentStep) => {
        ...
        if(!valid) {
            currentStep.addClass('wo17-step--invalid');
        }
    ...
    // ----------------------------------------------------------------
    // Additional validators, e.g. on key up snce Accordion only
    // runs validators when Accordion::decision() is called (on 
    // 'continue' button click)
    // ----------------------------------------------------------------
    const revalidateMeasurements = (elm) => {
        validators.execute('enter-measurements', [accordion.getCurrentStep()], false);
    };
    $('.wo17-step').on('keyup', '[name=wo17-width], [name=wo17-drop]', function() {
        revalidateMeasurements(this);
    });
    $('.wo17-step').on('change', '.wo17-extra-option select', function() {
        revalidateMeasurements(this);
    });
    ...
    const pollPrice = () => {
        const priceContainer = $('#price')
            , beforePrice = priceContainer.find('.sale-message').html().trim()
        ...
        if($('.flashsale .sale-message').length) {
            $('.wo17-was-price').html(beforePrice).find('br').remove();
        }
    ...
    // Helper when step radio changes update measurements help summary
    $('[name=wo17-recess]').change(function() {
        accordion.clearSummary('measurements-help-recess');

        const thisText = this.nextSibling.textContent.trim();
        accordion.updateSummary(thisText, 'measurements-help-recess');
    });
    ...
    $('.wo17-unit-selector-option').on('click', (e) => {
        ...
        helpers.sendEvent('did-toggle-units', 'to-' + chosenType);
    ...
    // Events
    $('.wo17-free-sample-button').on('click', function() {
        helpers.sendEvent('free-sample-button-clicked');
    });
    $('.wo17-add-to-cart-button').on('click', function() {
        helpers.sendEvent('clicked-add-to-cart');
    });
};
