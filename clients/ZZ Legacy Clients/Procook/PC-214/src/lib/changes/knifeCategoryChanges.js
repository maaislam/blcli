...
export default (url, allFilters) => {
    ...
    pollerLite([`#range`], () => {
      ...
      [].forEach.call(getRangeOptions, (opt) => {
        ...
        if (!!knifeRange[`${optText}`]) {
          opt.querySelector('label').insertAdjacentHTML('afterend', `<span class="${ID}-subtext">${knifeRange[`${optText}`]}</span>`);
        }
      });
    });
  }
  ...
  || url == '/shop/knives-scissors/procook-professional-x50') {
    ...
    if (url == '/shop/knives-scissors/chefs-knives') {
      document.querySelector(`.filter#${ID}-guarantee`).insertAdjacentElement('beforebegin', document.querySelector('.filter#size'));
    } else if (url == '/shop/knives-scissors/damascus-67'
    || url == '/shop/knives-scissors/procook-professional-x50') {
      pollerLite([`#set-size ul li`], () => {
        setTimeout(() => {
          document.querySelector(`.filter#${ID}-price`).insertAdjacentElement('beforebegin', document.querySelector('.filter#size'));
        }, 1000);
      });
    } 
