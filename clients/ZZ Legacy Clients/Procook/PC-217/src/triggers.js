    ...
    const observer = new MutationObserver(function(mutations) {
            mutations.forEach(function(mutation) {
                if (oldHref != document.location.href) {
                    ...
                    // document.documentElement.classList.remove('PC-217');
                    // document.querySelector('.PC-217-popular-filter-sizes__wrapper').parentElement.removeChild(document.querySelector('.PC-217-popular-filter-sizes__wrapper'));
                    pollerLite([
                      '#filterSize',
                      '.filterBoxDropDown ul li label.filterText',
                      '.template-category-list',
