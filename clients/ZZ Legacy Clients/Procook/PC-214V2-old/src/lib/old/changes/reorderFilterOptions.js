	...
	if (window.location.pathname.indexOf(`cookware`) > -1) {
		...
		pollerLite([`#${ID}-usage`], () => {
			...
			for (let i = 0; i < hobPriority.length; i += 1) {
				...
				let filterOpt = document.querySelector(
					`#${ID}-${hobP.replace(" ", "-").replace(" ", "-")}`
				);
				if (filterOpt) {
					document
						.querySelector(`.filter#usage .filterBoxDropDown ul`)
						.insertAdjacentElement("beforeend", filterOpt);
				}
			}
		});
	}
		...
		pollerLite([`.filter#range`], () => {
			...
			[].forEach.call(allRanges, (range) => {
				...
				for (let i = 0; i < knivesPriority.length; i += 1) {
					...
					let filterOpt = document.querySelector(
						`#${ID}-${rangeP.replace(" ", "-").replace(" ", "-")}`
					);
					if (filterOpt) {
						document
							.querySelector(`.filter#range .filterBoxDropDown ul`)
							.insertAdjacentElement("beforeend", filterOpt);
					}
				}
			});
