...
import { getUrlParameter, checkNonStickParameter } from "../helpers";
...
export default (url) => {
	pollerLite([`#${ID}-features`], () => {
		document
			.querySelector(`.filter#features .filterBoxTitle`)
			.insertAdjacentHTML("afterend", `<div class="filterBoxDropDown"><ul></ul></div>`);
		if (
			url == "/shop/cookware/induction" ||
			...
			for (let i = 0; i < getHobTypeOptions.length; i += 1) {
				...
				if (
					opt.querySelector("label").innerText.trim().toLowerCase().indexOf("dishwasher safe") > -1
				) {
        ...
        <span class="clear"></span>
			...
			const nonStickValue =
				"gourmetnonstick+aluminiumceramicnonstick+aluminiumgranitenonstick+aluminiumnonstick+stainlesssteelnonstick";
			...
			nonStickCheckbox.addEventListener("click", () => {
				if (nonStickCheckbox.checked) {
					if (window.location.href.indexOf("filter_products") > -1) {
						location.search =
							"filter_products=" + getUrlParameter("filter_products") + `+${nonStickValue}`;
