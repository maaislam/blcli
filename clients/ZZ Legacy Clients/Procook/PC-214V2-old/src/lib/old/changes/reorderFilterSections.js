...
export default (url) => {
	...
	for (let i = 0; i < priorityOrder.length; i += 1) {
		...
		if (document.querySelector(`.filter#${filterID}`)) {
			...
			document
				.querySelector(`.filterBoxOptions`)
				.insertAdjacentHTML("afterbegin", `<div class="filter" id="${ID}-${filterID}"></div>`);
          ...
          <div id="filter${newFilterID}" class="filterBox">
			...
			if (pFilter == "Set size") {
				if (url == "/shop/cookware/induction") {
					document
						.querySelector(`.filter#${ID}-size`)
						.insertAdjacentHTML("beforebegin", newFilterBox);
				} else if (url == "/shop/cookware/saucepans") {
					document
						.querySelector(`.filter#${ID}-usage`)
						.insertAdjacentHTML("beforebegin", newFilterBox);
				} else if (url == "/shop/cookware/stainless-steel") {
					document
						.querySelector(`.filter#${ID}-type`)
						.insertAdjacentHTML("beforebegin", newFilterBox);
					...
					pollerLite([`#${ID}-material`], () => {
						setTimeout(() => {
							document
								.querySelector(`.filterBoxOptions`)
								.insertAdjacentHTML("afterbegin", newFilterBox);
						}, 1000);
					});
				} else if (
					...
					pollerLite([`#${ID}-type`], () => {
						setTimeout(() => {
							document
								.querySelector(`.filter#${ID}-type`)
								.insertAdjacentHTML("afterend", newFilterBox);
						}, 1000);
					});
				...
				if (url == "/shop/cookware/induction") {
					...
					document
						.querySelector(`.filter#${ID}-material`)
						.insertAdjacentHTML("beforebegin", newFilterBox);
				} else if (url == "/shop/cookware/stainless-steel") {
					document
						.querySelector(`.filter#${ID}-usage`)
