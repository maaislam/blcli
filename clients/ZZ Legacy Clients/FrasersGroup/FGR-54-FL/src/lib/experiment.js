					...
					if (activeStep != 2) {
						...
						document
							...
							.addEventListener("click", (e) => {
								...
								if (
									target.closest("a.changeLink") &&
									target
										.closest("a.changeLink")
										.textContent.includes(
											"address manually"
										)
								) {
									fireEvent(
										"User clicked on Enter address manually"
									);
								} else if (
									target.closest("a.changeLink") &&
									target
									...
									fireEvent(
										"User clicked on change location"
									);
								}
							});
					}
					const input = document.querySelector(
						".sectionWrap .deliverySection.activeSection #frmAddress .formWrap .form-group input#line1"
					);
					if (
						input &&
						input
		...
		const checkCheckoutStepAndFireEvent = () => {
			if (
					...
					document
						...
						.addEventListener("click", (e) => {
							...
							if (
								...
								target
									.closest("a.changeLink")
									.textContent.includes("address manually")
							) {
								fireEvent(
									"User clicked on Enter address manually"
								);
				...
				const line2 = document.querySelector(
					".sectionWrap .deliverySection.activeSection #frmAddress .formWrap .form-group input#line2"
				);

				const towncity = document.querySelector(
					".sectionWrap .deliverySection.activeSection #frmAddress .formWrap .form-group input#towncity"
				);

				const postcode = document.querySelector(
					".sectionWrap .deliverySection.activeSection #frmAddress .formWrap .form-group input#postcode"
				);

				if (
					input &&
					input.getAttribute("placeholder").includes("postcode") &&
					!shownStatus
				) {
				...
				if (
					input &&
					input.getAttribute("placeholder").includes("postcode") &&
					!(postcode.value != "" || towncity.value != "")
				) {
					container.classList.add("hideItems");
					line2.closest(".form-group").classList.add("hide");
					input
						.closest(".form-group")
						.querySelector("label").textContent = "Address Search";
				} else if (input) {
					container.classList.remove("hideItems");
					line2.closest(".form-group").classList.remove("hide");
					input
						.closest(".form-group")
						.querySelector("label").textContent = "Address line 1";
					if (
						input.getAttribute("placeholder").includes("postcode")
					) {
						...
						input.parentNode.querySelector(
							"a.changeLink"
						).textContent = "Use Address Finder";
					}
				}
			} else if (
				document.querySelector(
