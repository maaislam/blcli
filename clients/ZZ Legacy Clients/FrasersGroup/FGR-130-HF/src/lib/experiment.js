...
export default () => {
	...
	if (location.pathname.includes("/checkoutsp")) {
		pollerLite([".sectionWrap .welcomeSection.activeSection"], () => {
			let myDetailsSubmitButton = document.querySelector(
				`.sectionWrap .welcomeSection.activeSection button[type="submit"]`
			);
			if (VARIATION == 1) {
				myDetailsSubmitButton.innerText = "Continue";
			} else if (VARIATION == 2) {
				myDetailsSubmitButton.innerText = "Checkout Securely";
			}
		});
	}
};
