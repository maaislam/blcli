...
(function() {
    ...
    ---------------------------------*/
            function rearrangePage() {
                var header = $('#header');
                header.append('<div class="pd2-headline"><span>The largest supplier of PPE & Workwear in the UK</span></div>');
...
(function() {
    ...
    function imageLightbox() {
        ...
        mainImage.click(function(){
            $lightbox.fadeIn(100, () => {
              const slickContainer = document.querySelector('.pd2-has-slick .swipe-wrap');
              // Refresh slick after fade in
              if (slickContainer) {
...
(function() {
    ...
    UC.poller([
        ...
        '.productDetailsContent',
        '.productDetailPanel',
        'input[name="CSRFToken"]', // CSRF Token
        '.productDetailsContent input[name="productCode"]', // Product code  - on load
        'input[name="productCodePost"]', // code for sending brand data - on load
        function() {
            return !!window.jQuery;
        }
    ], function() {
    ...
    function brandedLightbox() {
        ...
        var $brandingLightboxHTML = $(['<div class="pd2-brandinglightboxOverlay"></div>',
                                '<div class="pd2-brandinglightbox">',
                                    ...
                                    '<div class="pd2-brandightbox-innercontent">',
                                        ...
                                        '<label class="pd2-branding-quantity-select">Select your quantity:</label>',
                                        '<div class="pd2-size-selector__selector branding">',
                                            '<div class="pd2-size-selector__option">',
                                              '<div class="pd2-size-selector__selector">',
                                                  ...
                                                  '</div>',
                                                  '<div class="pd2-size-selector__input-wrap">',
                                                      '<input class="pd2-size-selector__quantity_branding" type="number" min="1" value="1" />',
                                                  '</div>',
                                                  '<div class="pd2-size-selector__plus">',
                                                      '+',
                                                  '</div>',
       ...
       $('.pd2-branding').click(function(){
        ...
        ---------------------------------*/
        $('.pd2-quantities .pd2-size-selector__option').each(function(){
            $sizeText = $(this).find('.pd2-size-selector__details span').text();
        ...
        var noChangeselectID = document.querySelector('input[name="productCodePost"]').getAttribute('value');
        var noChangesize = brandedForm.find("option:selected").text().replace(/\s+/g, "_").split(",")[0].replace("''",'').replace(/_/g, ' ');;
        const brandCSRFToken = document.querySelector('input[name="CSRFToken"]').getAttribute('value');
        const brandProductCode = document.querySelector('.productDetailsContent input[name="productCode"]').getAttribute('value');
        // Create valuese to be changed in functions
        // Quantity defaults to 1, can be changed
        let selectedValue = noChangeselectID;
        let productQuantity = 1;
        let brandRoute = `${window.location.pathname}?CSRFToken=${brandCSRFToken}&wantBranding=true&productCodePost=${noChangeselectID}&productUrl=&productBaseCode=${brandProductCode}&qty=${productQuantity}`;            
        // Set form input values onload
        brandedForm.find('input[name="productCode"]').val(noChangeselectID);
        brandedForm.find('input[name="productOption"]').val(noChangesize);
        brandedForm.attr("action", brandRoute);
        // Drop down selector functionality
        $(".pd2-productSelect").change(function() {
          selectedValue = $(this).val();
          var size = $(this).find("option:selected").text().replace(/\s+/g, "_").split(",")[0].replace("''",'').replace(/_/g, ' ');;
          brandedForm.find('input[name="productCode"]').val(selectedValue);
          brandedForm.find('input[name="productOption"]').val(size);
          updateBrandRoute();
        });

        // Lightbox quantity controls

        const brandingQinput = document.querySelector('.pd2-size-selector__quantity_branding');
        const brandingQPlus = document.querySelector('.pd2-size-selector__selector.branding .pd2-size-selector__plus');
        const brandingQMinus = document.querySelector('.pd2-size-selector__selector.branding .pd2-size-selector__minus');
        const brandingQHidden = document.querySelector('#wantBrandingForm input[name="qty"]');

        brandingQPlus.addEventListener('click', () => {
          let tempValue = parseInt(brandingQinput.value, 10);
          if (!isNaN(tempValue)) {
            tempValue += 1;
            // Update values and inputs
            brandingQinput.value = tempValue;
            brandingQHidden.value = tempValue;
            productQuantity = tempValue;
            updateBrandRoute();
          } 
        });

        brandingQMinus.addEventListener('click', () => {
          let tempValue = parseInt(brandingQinput.value, 10);
          if (!isNaN(tempValue)) {
            tempValue -= 1;
            if (tempValue > 0) {
        ...
        brandingQinput.addEventListener('change', (e) => {
          // Input restricted to numbers only, but can accept decimals
          ...
          // Update values and inputs
          brandingQHidden.value = setValue;
          e.target.value = setValue;
          productQuantity = setValue;
          updateBrandRoute();
        });

        // Function that updates the action of the form
        updateBrandRoute = () => {
          brandedForm.attr("action", `${window.location.pathname}?CSRFToken=${brandCSRFToken}&wantBranding=true&productCodePost=${selectedValue}&productUrl=&productBaseCode=${brandProductCode}&qty=${productQuantity}`);
        };
    };
