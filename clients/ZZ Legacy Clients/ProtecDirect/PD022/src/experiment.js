...
/**
 * {{PD022}} - {{Promote industry regulations}}
 */
const Run = () => {
  // const $ = window.jQuery;
  const Exp = {
    ...
    components: {
      setupHomepage() {
        poller([
          ...
          // Insert content
          Exp.cache.bodyVar.querySelector('.pd5-uspwrapper').insertAdjacentHTML('afterend', Exp.cache.homepageBasketMarkup);
          // Add styling class
      ...
      setupPLP() {
        poller([
          '.pd3-delivery-banner.clearfix',
        ], () => {
          Exp.cache.bodyVar.querySelector('.pd3-delivery-banner.clearfix').insertAdjacentHTML('afterend', Exp.cache.PLPMarkup);
      ...
      setupProductPage() {
        poller([
          'div[class="prod_review"]', '#breadcrumb > ul > li:nth-child(3) > a',
        ], () => {
          // Get the second level category and insert into text
          let secondCategory = Exp.cache.bodyVar.querySelector('#breadcrumb > ul > li:nth-child(3) > a').textContent.trim();
          secondCategory = secondCategory.substring(2);
          Exp.cache.bodyVar.querySelector('div[class="prod_review"]').insertAdjacentHTML('beforebegin', `
            <div class="PD022-Wrapper PD022-Product">
              <div class="PD022-Image-Wrap">
                <img class="PD022-Image" src="https://d191y0yd6d0jy4.cloudfront.net/idlkbp6e0rwdlrf.png" alt="Ribbon">
              </div>
              <div class="PD022-Text-Wrap">
                <span class="PD022-Text">You can trust that all ${secondCategory} sold by Protec Direct meet industry regulations and standards. Our buying guides and datasheets are available to give you more information.</span>
              </div>
           </div> 
          `);
        });
      ...
      setupBasket() {
        poller([
          '.span-24.basket > .span-20 > .span-20',
        ], () => {
          Exp.cache.bodyVar.querySelectorAll('.span-24.basket > .span-20 > .span-20')[1].insertAdjacentHTML('afterend', Exp.cache.homepageBasketMarkup);
          // Add styling class
