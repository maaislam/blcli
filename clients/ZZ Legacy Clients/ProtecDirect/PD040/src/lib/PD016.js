...
const PD016 = () => {
	...
	function init(){
      ...
      const discountAmt = bodyVar.querySelector('#variant-quant_disc span.saving');
      ...
      if(optionSelector == null){
		...
		const desktopQuantityDiscount = {
          ...
          let savingAmt = 0;
          if (cacheDom.discountAmt) {
            savingAmt = cacheDom.discountAmt.textContent.trim().match(/\W\d+\.\d+/);
          }

					cacheDom.totalsTextContainer.insertAdjacentHTML('afterend', `
          <span class="PD016_Discount_Threshold_Text">
            Buying in bulk? <br />
            <span class="PD016_Discount_Items_Text"> Add 
              <span class="PD016_Discount_Counter">`+ (cacheDom.discountThreshold - cacheDom.quantityInput.value) + `</span> 
              more items </span>
            <span class="PD016_Discount_Item_Text">Add 1 more item </span> to save ${savingAmt}!
          </span>
          `);
			...
			takeQuantityInput(){
                ...
                cacheDom.PD016IncVatText.textContent = cacheDom.newVatPrice;
							...
							//Adjust "another item" text 
								...
								} else if((cacheDom.discountThreshold - cacheDom.quantityInput.value) == 0){
