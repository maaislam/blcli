...
const newPopup = (() => {
    ...
    // Book now button
    const bookNowBtn = document.querySelectorAll('.search-content .result-item .buttons a.book-now');
    const bookBtnArr = [...bookNowBtn];
		
    document.body.classList.add('NH014'); 
		
		// For each product
		for (let i = 0; bookBtnArr.length > i; i++) {
			bookBtnArr[i].addEventListener("click", function(e) {

        runPopup(e.currentTarget.nextElementSibling.dataset['tourid'], e);
			});
    }
    ...
    // For each product
        ...
        runPopup(e.currentTarget.dataset['tourid'], e);
			});
		}
		

    const runPopup = (tourIdElement, e) => {

				const tourId = tourIdElement;
					...
					if (request.status >= 200 && request.status < 400) {
						...
						if (html) {
							...
							const highlights = (() => {
								...
								newHighlights = newHighlights[0].innerText;
								let nhSplit = newHighlights.split('\n');

							
								for (let i = 0; nhSplit.length > i; i++) {
									let jj = nhSplit[i].substring(2);
									
									let newLi = document.createElement('li');
									newLi.innerText = jj;
									highlightsUl.appendChild(newLi);
								}
		...
		document.onload = function() {
			window.location = 'http://www.nationalholidays.com/WebServices/SeatplanService.asmx/GetSeatplan';
		};
