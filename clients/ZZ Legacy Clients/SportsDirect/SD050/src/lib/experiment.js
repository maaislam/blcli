...
const activate = () => {
  ...
  if (VARIATION == 2) {
    events.send('SD050', 'SD050 Control', 'SD050 Control is active');
    return false;
  } else {
    events.send('SD050', 'SD050 Variation 1', 'SD050 Variation is active');
  }
  ...
  const notViewed = document.querySelectorAll('li:not(.SD050-viewed)');

  if (VARIATION === '2') {
    events.send(ID, 'Control', 'Control is active');
    if (notViewed.length) {
      [].slice.call(notViewed).forEach((prod) => {
  ...
  const displayYouViewed = () => {
    ...
    pollerLite(['.bv_stars_component_container', 'li.SD050-viewed'], () => {
      const prevViewed = document.querySelectorAll('li.SD050-viewed');
      if (prevViewed.length) {
        let maxHeight = 0;
        [].slice.call(prevViewed).forEach((prod) => {
  ...
  if (notViewed.length) {
    [].slice.call(notViewed).forEach((prod) => {
      prod.addEventListener('click', () => {
        if (prod.classList.contains('SD050-viewed')) {
