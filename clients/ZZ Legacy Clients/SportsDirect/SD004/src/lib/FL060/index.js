  ...
  (function() {
    ...
    var events = {
      ...
      send: function send(category, action, label, options) {
        ...
        if ((typeof options === 'undefined' ? 'undefined' : _typeof2(options)) === 'object' && options.sendOnce) {
    ...
    var createFeatureList = function createFeatureList(listArr) {
      if (listArr.length > 0) {
        ...
        ul.classList.add('SD004-featureList');
        listArr.map(function (feature) {
          ul.insertAdjacentHTML('beforeend', '\n        <li> <p>' + feature.trim() + '</p></li>\n      ');
        });
    ...
    var clearStorage = function clearStorage() {
      localStorage.removeItem('SD004-option');
    
      if (document.querySelector('.SD004-chosenOption')) {
        var existingEls = document.querySelectorAll('.SD004-chosenOption');
    ...
    var storeChosenOption = function storeChosenOption(event) {
      ...
      var html = '\n    <div class="SD004-chosenOption">\n      <p>' + obj.title + '</p>\n      <p><span>' + obj.subtitle + '</span></p>\n\n       </div>\n  ';
    
      localStorage.setItem('SD004-option', html);
    ...
    var activate = function activate() {
      ...
      var initialPage = function initialPage() {
        ...
        if (els.shippingGroupCont) {
          els.shippingGroupCont.insertAdjacentHTML('afterbegin', '\n        <p class="SD004-pleaseSelect"><strong>Please select one of the following</strong></p>\n      ');
        }
    
        if (window.localStorage.getItem('SD004-changedBilling')) {
          var firstOption = document.querySelector('.DeliveryOptions .deliveryGroupTypeLi .innerDeliSection');
          if (firstOption) {
            firstOption.click();
            window.localStorage.removeItem('SD004-changedBilling');
          }
        }
        ...
        addHTML(els.locationHead[1], '<span class="locationHead">Delivery to one of our 470 stores across UK<span>', false, 'beforeend');
        addHTML(els.locationHead[2], '<span class="locationHead">Collect from a locker / shop (UK or Europe only)<span>', false, 'beforeend');
    
        var homeFeatures = void 0;
        if (timeLeft) {
          homeFeatures = createFeatureList(['4 - 5 working days UK delivery', '' + timeLeft, 'Receive a 1 hour delivery window on the day of delivery via text', 'Options to leave with a neighbour or in a safe place if you aren\’t home', 'Your delivery will be automatically rearranged for another time if you miss it', 'Option to divert your package while it\’s en-route', '<strong>Excluding large or specialised delivery</strong>']);
        } else {
        ...
        var storeFeature = createFeatureList(['Free £5 voucher for use on your next purchase', '5 - 7 working days pick up time']);
        

        // Add in for 'Pickup Locations'
        var pickupLocationsRef = document.querySelector('.DeliveryOptions .deliveryGroupTypeLi[data-grouptype="3"] .innerDeliSection');
        if (pickupLocationsRef) {
          var pickupFeature = createFeatureList(['4 - 5 working days UK delivery']);
          addHTML(pickupLocationsRef, pickupFeature.outerHTML, true, 'beforeend');
        }
      ...
      var secondPage = function secondPage() {
        ...
        //   addressInfoDupe.classList.add('SD004-billingAddress');
        ...
        //   if (document.querySelector('.SD004-billingAddress')) {
        //     var elToRemove = document.querySelector('.SD004-billingAddress');
        //     if (elToRemove) {
        //       elToRemove.parentNode.removeChild(elToRemove);
        //     }
        //   }
    
        //   if (!localStorage.getItem('SD004-differentAdd')) {
        //     addHTML(els.deliveryAddress, addressInfoDupe.outerHTML, true);
    
        //   } else if (localStorage.getItem('SD004-differentAdd')) {
        //     var dupedDeliverAdd = addressInfoDupe.querySelector('.CurrentAddressText > span');
    
        //     if (dupedDeliverAdd) {
        //       dupedDeliverAdd.innerHTML = '';
        //       dupedDeliverAdd.insertAdjacentHTML('beforeend', '<p class="SD004-addedBilling">You can edit your billing address after the payment options</p>');
        //     }
        ...
        // if (!document.querySelector('h1.SD004-quickTitle')) {
        //   addHTML(els.shippingGroup, '<h1 class="SD004-quickTitle" id="dnn_ctr102498_Delivery_Header">How Quick Do You Want It?</h1>', true, 'afterbegin');
        // }
        ...
        //           if (!ref.querySelector('p.SD004-fromPrice')) {
        //             addHTML(ref, '<p class="SD004-fromPrice">from \xA3' + thisPrice + '</p>', true);
        //           }
        //         } else {
        //           var priceDiff = parseFloat(currentPrice - thisPrice);
        //           if (!ref.querySelector('p.SD004-fromPrice')) {
        //             addHTML(ref, '<p class="SD004-fromPrice">+ \xA3' + priceDiff.toString(10).replace('-', '') + '</p>', true);
        //           }
        //         }
        //       }
        //     }
        ...
        //     els.homeDelRadioBtns[_i4].classList.add('SD004-tick');
        //   }
        // }
        ...
        //     if (!addressBoxes[_i5].querySelector('.SD004-chosenOption')) {
        ...
        if (visitorLoginState && visitorLoginState === 'logged+in') {
          document.body.classList.add('SD004-loggedIn');
        }
    
        // var billingChangeLink = document.querySelector('.SD004-billingAddress .DifferentAddressLinkWrapper > a');
        // if (billingChangeLink) {
        //   billingChangeLink.addEventListener('click', function () {
        //     window.localStorage.setItem('SD004-changedBilling', 'true');
        //   });
        // }
      };
      ...
      var clickEvents = function () {
        ...
        if (els.changeLinks) {
          for (var _i6 = 0; els.changeLinks.length > _i6; _i6 += 1) {
            els.changeLinks[_i6].addEventListener('click', function () {
              var chosenOption = document.querySelector('.SD004-chosenOption');
              if (chosenOption) {
        ...
        if (els.billingChangeLink) {
          els.billingChangeLink.addEventListener('click', function () {
            localStorage.setItem('SD004-differentAdd', 'true');
            if (!document.querySelector('.SD004-billingMessage')) {
              addHTML(els.billingChangeLink, '<p class="SD004-billingMessage">You can add a new billing address after the payment options screen</p>', true);
            }
          });
        }
        ...
        if (els.differentAddressLinks) {
          for (var _i7 = 0; els.differentAddressLinks.length > _i7; _i7 += 1) {
            els.differentAddressLinks[_i7].addEventListener('click', function () {
              localStorage.removeItem('SD004-differentAdd');
            });
          }
        }
          ...
          (function () {
            ...
            var _loop = function _loop(_i8) {
              els.shippingGroupOptions[_i8].addEventListener('click', function () {
                ...
                var thisTick = els.shippingGroupOptions[_i8].querySelector('.SD004-tick');
                if (thisTick) {
                  thisTick.classList.add('SD004-tickActive');
                }
              });
            };
