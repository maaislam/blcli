...
function storeEmail(el) {
  if (el) {
    el.addEventListener('input', function(e) {
      ...
      // window.sessionStorage.setItem('SD002-email', emailVal || '');
...
function showError(el) {
  ...
  if(document.querySelector('.SD002-emailErr')) {
    document.querySelector('.SD002-emailErr').parentElement.removeChild(
      document.querySelector('.SD002-emailErr')
    );
  }
...
function removeErr() {
  ...
  const addedErr = document.querySelector('.SD002-emailErr');
  if (addedErr) {
...
function fillEmail() {
  ...
  let addedEmail = ''; // window.sessionStorage.getItem('SD002-email');
  let addedEmailPlusKey = getCookie('SD002-email');
...
function fetchEmail() {
  let storedEmail = ''; // window.sessionStorage.getItem('SD002-email');
  const emailCookie = getCookie('SD002-email');
