...
const buildBrandList = (brandList, ref, positon) => {
  ...
  if (brandList.length && ref) {
    if (!document.querySelector('.SD-87-brands')) {
      ref.insertAdjacentHTML(positon, `
        <div class="SD-87-brands">
          <div class="SD-87-info">
            <p>Shop by Brand</p>
          </div>
          <div class="SD-87-brands-wrap">
            ${brandList.map(brand => `<div class="SD-87-brand">${brand.outerHTML}</div>`).join('')}
            <div id="SD-87-more-brands"><p>More brands</p></div>
          </div>
        </div>
      `);
    }
 ...
 */
const removeEvents = () => {
  const addedBrands = document.querySelectorAll('.SD-87-brands .SD-87-brand a.FilterAnchor');
...
const checkActiveFilters = () => {
  setTimeout(() => {
    const addedFilters = document.querySelectorAll('.SD-87-brands .SD-87-brand .FilterListItem');
    if (addedFilters.length) {
      for (let i = 0; addedFilters.length > i; i += 1) {
        const isActive = addedFilters[i].querySelector('.SelectedFilter');
        if (isActive) {
          addedFilters[i].classList.add('SD-87-active');
        } else {
          addedFilters[i].classList.remove('SD-87-active');
        }
      }
    }
  }, 600);
...
const toggleFromOriginalFilters = () => {
  ...
  if (!oGFilters.length) {
    for (let i = 0; addedFilters.length > i; i += 1) {
      if (addedFilters[i] && addedFilters[i].classList.contains('SD-87-active')) {
        addedFilters[i].classList.remove('SD-87-active');
      }
      ...
      oGFilters[i].addEventListener('click', () => {
        for (let j = 0; addedFilters.length > j; j += 1) {
          ...
          if (addedFilterName.textContent.trim().toUpperCase() === filterName.textContent.toUpperCase()) {
            if (addedFilters[j] && addedFilters[j].classList.contains('SD-87-active')) {
              addedFilters[j].classList.remove('SD-87-active');
            }
