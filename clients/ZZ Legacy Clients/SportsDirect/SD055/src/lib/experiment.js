...
const activate = () => {
  ...
  // Exclusions
  let personalisation = document.getElementById('pnlPersonalisation');
  if(personalisation !== null && personalisation !== "undefined") {
    events.send(ID, 'SD055 Skipped Product', 'Product has personalisation options, not relevant');
    return false;
  }

  let atbButton = document.querySelector('.addToBagInner');
  if(atbButton.innerText == "PRE-ORDER NOW" && colourOptions.length == 0) {
    events.send(ID, 'SD055 Skipped Product', 'Product is single colour pre-order, not relevant');
    return false;
  }

  let brandName = document.getElementById('lblProductBrand');
  if(brandName.innerText == "GIFT") {
    events.send(ID, 'SD055 Skipped Product', 'Product is gift card, not relevant');
    return false;
  }

  let fifaProduct = document.querySelector('.PromoContent_SD_PRE_ORDER_FIFA');
  if(fifaProduct) {
    events.send(ID, 'SD055 Skipped Product', 'Product has fifa offer on it, not relevant');
    return false;
  }

  let nddContent = document.getElementById('delivery_Nextday');
  if(!nddContent) {
    events.send(ID, 'SD055 Skipped Product', 'Product does not qualify for free delivery, not relevant');
    return false;
  } 

  // adding initial HTML
  let nddHolderHTML = `<div class="SD055-message-holder"> </div>`;
  ...
  if(document.querySelector('#ulColourImages')) {
    ...
    let messageHolder = document.querySelector('.SD055-message-holder');
  ...
  [].slice.call(sizeOptions).forEach(function(option) {
    ...
    option.addEventListener('click', () => {
      ...
      setTimeout(function() {
        let updatedNDD = document.getElementById('delivery_Nextday');
        if(updatedNDD.style.display == "none") {
          document.querySelector('.SD055-message-holder').classList.add('dropshiphidden');
        } 
      }, 300);
  ...
  const init = () => {
    ...
    let html = `
      <div class="SD055-message">
        <p class="fl62-t">GET YOUR ORDER WITHIN 2 DAYS</p>
        <p class="fl62-s">${timeLeftString}</p>
      </div>
    `;

    let nddHolderElement = document.querySelector('.SD055-message-holder');

    // Add it
    if (nddHolderElement) {
      // If it already exists, remove and add
      if (document.querySelector('.SD055-message')) {
        const el = document.querySelector('.SD055-message');
