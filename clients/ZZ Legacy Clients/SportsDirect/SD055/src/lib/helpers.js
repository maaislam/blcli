  ...
  timeRemaining: () => {
    ...
    */
    ...
    const timeLeft = () => {
      ...
      const returnedTimeLeft = `<strong>${hourDiff}</strong>hr <strong>${minDiff}</strong>m <strong>${secDiff}</strong>s`;
    ...
    const buildMessage = (dateObject) => {
      if (dateObject) {
        ...
        const html = `
          If you checkout within <strong class="datestring">${dateObject.time}</strong>
        `;
        return html;
      }
    };
