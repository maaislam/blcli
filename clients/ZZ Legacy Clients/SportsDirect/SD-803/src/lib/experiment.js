			...
			const fireEvents = () => {
				...
				if (sizeSelected == true && isDiscounted) {
					...
					let prevPrice = document
						.querySelector("#productDetails #TicketPriceDiv2 span")
						?.innerText.trim();
					prevPrice = prevPrice.replaceAll(regex, "");
					if (isCommaUsedInCurrencyForDecimal) {
						currentPrice = currentPrice.replaceAll(".", "");
			...
			document.body.addEventListener("click", (e) => {
				if (
					(e.target.closest("#spanAddToBagWrapper") &&
						e.target.closest("a.ContinueWithPers:not(.hidden)")) ||
			...
			() => {
				...
				const percentageDiscound = () => {
					...
					if (
						...
						if (dif >= 20.0) {
							const discountPercentageDom = `<div class="${ID}-discount-percentage-pdp">
							<span>${Math.floor(dif)}% off</span>
						</div>`;
							if (
								...
								pdpPriceRating?.insertAdjacentHTML(
									"beforeend",
									discountPercentageDom
								);
							} else {
								pdpPriceRating.querySelector(
									`.${ID}-discount-percentage-pdp span`
								).innerText = `${Math.floor(dif)}% off`;
							}
						} else {
							pdpPriceRating
