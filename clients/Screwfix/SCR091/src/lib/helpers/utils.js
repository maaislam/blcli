...
export const urlHandler = (ID, productSku, quantity) => {
  const checkStoresElement = document.querySelector(`.${ID}__checkStores`);
  checkStoresElement.href = `/checkstock?product_id=${productSku}&amp;quantity=${quantity}`;
};

export const dispatchHandler = (value) => {
  const inputElem = document.querySelector('[data-qaid="pdp-product-quantity"]');
  const inputValue = value; //handle max input value
  const nativeInputValueSetter = Object.getOwnPropertyDescriptor(window.HTMLInputElement.prototype, 'value').set;
  nativeInputValueSetter.call(inputElem, inputValue);
