...
const parsedfromResponse = (info, url) => {
  ...
  if (!window.optionsDataArray) {
    window.optionsDataArray = [];
  }

  const isUrlExist = window.optionsDataArray.some((item) => item.url.toLowerCase() === url.toLowerCase());

  if (!isUrlExist) {
    window.optionsDataArray.push(allInfo);
  }
