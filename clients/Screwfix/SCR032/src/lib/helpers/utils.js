...
export const getCompareList = (fetchProd = true) => {
  ...
  return fetch(compareUrl, {
    ...
    .then(({ products }) => {
      if (!fetchProd) {
        return products; // Return SKU array directly
      }

      // Fetch product data using SKU ids
      const productUrl = `/prod/ffx-browse-bff/v1/${tenantId}/search/products/suggested?skuIds=${products.join(',')}`;
...
export const modifySkuList = (sku, method = 'del') => {
  const runtimeData = window.__NEXT_DATA__.runtimeConfig;

  const { tenantId, tykToken } = runtimeData;
  const compareUrl = `/prod/ffx-browse-bff/v1/${tenantId}/product/skuid/${sku}/compare-list`;
  return fetch(compareUrl, {
    method: method === 'del' ? 'DELETE' : 'POST',
    headers: {
      accept: '*/*',
      'accept-language': 'en-US,en;q=0.9',
      authorization: tykToken,
...
export const addSearchParam = (paramName, paramValue) => {
