...
export const extractVolume = (text) => {
  // This regex matches a sequence of digits followed by 'ml', anywhere in the string
  const quantityRegex = /(\d+)ml/i;
  const quantityMatch = text.match(quantityRegex);

  // This regex looks for a pack size indication, which we assume follows the format of 'digits Pack'
  const packRegex = /(\d+)\s*Pack/i;
  const packMatch = text.match(packRegex);

  if (quantityMatch) {
    let quantity = parseInt(quantityMatch[1]);
    // If a pack size is found, multiply the quantity by the pack size
    if (packMatch) {
      let packSize = parseInt(packMatch[1]);
      quantity *= packSize;
    }
    return quantity;
  }
