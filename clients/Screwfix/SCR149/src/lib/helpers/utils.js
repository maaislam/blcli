...
export const createStoreLink = (id, storeName, storeUrl = '/stores') => {
  const maxLength = 15;
  const truncated = storeName.length > maxLength ? storeName.slice(0, maxLength) + '…' : storeName;

  const link = `
    <a href="${storeUrl}" class="${id}__storeLocator">${truncated}</a>
  `;
