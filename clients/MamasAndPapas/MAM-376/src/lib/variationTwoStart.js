...
import hideElem from "./helper/hideElem";
			...
			.then((data) => {
				...
				if (data.itemsComplete.length > 0) {
									...
									if (
										...
										setTimeout(() => {
											...
											const callback = function () {
												...
												if (countData) {
													// document.querySelector('#PageContainer > div.MAM-376-new-modal-var-two > div.MAM-376-main-container > div.MAM-376-mini-bag-content > div.MAM-376-mini-bag-child-two > a').remove()

													// var newViewBagBtn = `<a href="/cart" class="MAM-376-cart-btn-checkout cart__checkout btn btn--solid-color">View Bag ${'('+document.querySelector('#CartCount > span:nth-child(1)').innerText+')'}</a>`;
													// document.querySelector(`.MAM-376-mini-bag-child-two`).insertAdjacentHTML('afterbegin', newViewBagBtn)
					...
					if (document.querySelector(`.${ID}-new-modal-var-two`)) {
						hideElem(`${ID}-new-modal-var-two`, ID);
					}
				}
			});
	}, 1000);
