...
const MP063 = {
	...
	components: {
		minibasketCarousel: {
			...
			create: function(products) {
				...
				container.classList.add('MP063_minibasketCarousel', 'row');

				const heading = document.createElement('div');
				heading.classList.add('MP063_title');
				heading.innerText = 'Don\'t forget your footmuff...';
				...
				const buildProductHtml = function(product) {
					...
					imgWrap.classList.add('MP063_imgWrap');
					...
					addToCartButton.classList.add('btn', 'btn-default', 'w-100', 'MP063_details__btn', 'MP063_details__btn--addToCart');
					...
					infoButton.classList.add('btn', 'btn-default', 'w-100', 'MP063_details__btn', 'MP063_details__btn--info');
			...
			attachEvents: function(component, $) {
				...
				const ajaxAddToBag = (sku) => {
					...
					request.onload = () => {
						if (request.status >= 200 && request.status < 400) {
							...
							events.send('MP063', 'Submit', 'V2 - Added footmuff to bag from minibasket carousel', {sendOnce: true});
							ACC.minicart.getMiniCart();
						}
					};
					request.onerror = () => events.send('MP063', 'Error', 'V2 - Error adding footmuff to bag');
				...
				component.addEventListener('click', (e) => {
					...
					if (target) {
						// More info
						if (target.classList.contains('MP063_details__btn--info')) {
							events.send('MP063', 'Click', 'V2 - Clicked more info on footmuff carousel', {sendOnce: true});
						}

						// Add to bag
						if (target.classList.contains('MP063_details__btn--addToCart')) {
							e.preventDefault();
							events.send('MP063', 'Click', 'V2 - Clicked move to bag on footmuff carousel', {sendOnce: true});
				...
				// 		events.send('MP063', 'Click', 'Clicked footmuff product', {sendOnce: true});
			...
			init: function(products, productElement) {
				...
				if (!document.querySelector('.MP063_minibasketCarousel')) {
	...
	activate: function() {
		// Setup
		fullStory('MP063', 'Variation 2');
		document.body.classList.add('MP063-V2');
		const lib = this.lib;
						...
						if (getCookie('MP063_addToBag')) {
							setTimeout(() => {
								...
								if (lastProductElementName === thisProductName) {
									this.components.minibasketCarousel.init(productData, lastProductElement);
									events.send('MP063', 'View', 'V2 - Minibasket carousel is in view', {sendOnce: true});
								}
							}, 500);
						}
