...
const addEvents = () => {
    ...
    interactivePTBForm.addEventListener('submit', (e) => {
        ...
        if(value !== "") {
            ...
            fireEvent(`User clicked on: [parent-to-be] on ${segmentPlacement} and their due date is: ${value} making them in their ${trimester}`)
    ...
    interactivePTBFormQ2.addEventListener('submit', (e) => {
        ...
        if(firstTimeParentValue !== "") {
            ...
            fireEvent(`User clicked on: [parent-to-be] on ${segmentPlacement} and they are ${firstTimeParentValue == "first-time" ? 'a first time parent' : 'not a first time parent'}`)
...
const insertTrigger = (triggerInsertionPointPlacement) => {
    ...
    let triggerHTML = `
        ...
        <div class="${ID}-segmentation-trigger-holder">
            ...
            <h2> Let us help you with your search </h2>

            <button id="${ID}-segmentation-trigger">Tell Us About Yourself</button>
    ...
    let triggerSegmentationButton = document.getElementById(`${ID}-segmentation-trigger`);

    triggerSegmentationButton.addEventListener('click', (e) => {
        ...
        if(document.querySelector(`.${ID}-section`).classList.contains('hidden')) {
            ...
            let fireMessage = "Click - user has clicked to open the segmentation quiz";
            logMessage(fireMessage);
            fireEvent(fireMessage, true);
    ...
    closeTriggerLink.addEventListener('click', (e) => {
        ...
        let closeMessage = "Click - user has clicked the X to hide the segmentation quiz trigger";
...
const createSection = () => {
        ...
        <div class="${ID}-section ${segmentPlacement} hidden">
            ...
            <div class="${ID}-section-content">
                        ...
                        <div class="${ID}-section-datagather ${ID}-parenttobe">
                            ...
                            <form method="POST" action="" id="${ID}-parenttobe-form" class="${ID}-parenttobe-form">
                                ...
                                <div class="${ID}-duedate-holder">
                                    ...
                                    <button type="submit" class="btn btn--secondary ${ID}-datagather-submit">Submit</button>
                                </div>
                ...
                <div class="${ID}-section-thankyou ${segmentPlacement}">
                    ...
                    <button id="${ID}-back-to-page">Back to ${pageTitle}</button>
    ...
    document.documentElement.addEventListener('click', (e) => {
        if (e.target.classList.contains(`${ID}-noscroll`)) {
            ...
            let closeMessage = "Click - user closed the popup by clicking outside of the element";
            logMessage(closeMessage);
            fireEvent(closeMessage, true);
    ...
    let backtopage = document.getElementById(`${ID}-back-to-page`);
    ...
    backtopage.addEventListener('click', (e) => {
        ...
        let closeBPMessage = "Click - user closed the popup by clicking the back to page link";
        logMessage(closeBPMessage);
        fireEvent(closeBPMessage, true);
    ...
    window.addEventListener('orientationchange', (e) => {
        ...
        if(window.orientation == 90) {
            document.getElementById(`${ID}-section-close`).click();
...
export default () => {
    ...
    if(!getCookie('MAM-283-noshow') && !getCookie('MAM-205-noshow') && !getCookie('MAM-204-noshow')) {
