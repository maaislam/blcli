...
const links = [
	"/collections/baby-boy-clothing",
	"/collections/baby-girl-clothing",
	"/collections/unisex-baby-clothing",
	"/collections/newborn-baby-clothes",
	"/collections/baby-clothing-multipacks",
];
...
const pushchairPLPPage = () => {
	...
	<div class="${ID}-pushchair-roundels-container">
		...
		<div class="${ID}-pushchair-roundels">
		  <a href="${links[0]}" class="${ID}-pushchair-roundels--button">
			<div class="${ID}-pushchair-roundels--img--container">
				<img src="https://blcro.fra1.digitaloceanspaces.com/MAM-526/Boys1.png" alt="Boys Clothing image" />
			</div>
			<h2>Boys Clothing</h2>  
		  </a>
		  <a href="${links[1]}" class="${ID}-pushchair-roundels--button">
			  <div class="${ID}-pushchair-roundels--img--container">
			  <img src="https://blcro.fra1.digitaloceanspaces.com/MAM-526/Girls.png" alt="Girls Clothing image" />
			</div>
			<h2>Girls Clothing</h2>
		  </a>
		  <a href="${links[2]}" class="${ID}-pushchair-roundels--button">
			  <div class="${ID}-pushchair-roundels--img--container">
			  <img src="https://blcro.fra1.digitaloceanspaces.com/MAM-526/Unisex.png" alt="Unisex Clothing image" />
			</div>
			<h2>Unisex Clothing</h2>  
		  </a>
		  <a href="${links[3]}" class="${ID}-pushchair-roundels--button">
			  <div class="${ID}-pushchair-roundels--img--container">
			  <img src="https://blcro.fra1.digitaloceanspaces.com/MAM-526/newborn.png" alt="Newborn Clothing image" />
			</div>
			<h2>Newborn</h2>  
		  </a>
		  <a href="${links[4]}" class="${ID}-pushchair-roundels--button">
			  <div class="${ID}-pushchair-roundels--img--container">
			  <img src="https://blcro.fra1.digitaloceanspaces.com/MAM-526/multipacks.png" alt="Multipacks Clothing image" />
			</div>
			<h2>Multipacks</h2>  
		  </a>
		</div>	
  ...
  `;
	...
	// Scrolling functionalities to avoid implemnting a slider for the roundels
	// On Click of right arrow scroll the roundels horizontally
	...
	// On Click of left arrow scroll the roundels horizontally
	...
	// Toggle the roundel arrow active class
	...
	document.body.addEventListener("click", (e) => {
		...
		if (
			...
			fireEvent("Customer views product from All Clothing page");
		}
	});
};
	...
	if (status) {
		document.body.addEventListener("click", (e) => {
			...
			if (
				...
				fireEvent("Customer views product from Clothing specific page");
			}
		});
		...
		if (innerStatus) {
			submit?.addEventListener("click", (e) => {
				fireEvent(
					"PDP - Customer adds product to basket after visiting the Clothing specific page"
				);
			});
		} else if (document.referrer.includes("collections/baby-clothing")) {
			submit?.addEventListener("click", (e) => {
				fireEvent(
					"PDP - Customer adds product to basket after visiting the All Clothing and didn’t interact with roundels"
				);
			});
		}
	}
...
const startExperiment = () => {
	if (window.location.pathname === "/collections/baby-clothing") {
		pushchairPLPPage();
	} else {
		pushchairInnerPage();
	}
...
export default () => {
	...
	if (window.location.pathname.includes("baby-clothing")) {
		...
		if (status) {
			document.body.addEventListener("click", (e) => {
				...
				if (
					...
					fireEvent(
						"Customer views product from Clothing specific page"
					);
				}
			});
		} else if (window.location.pathname.indexOf("/products/") > -1) {
			...
			if (innerStatus) {
				...
				document.referrer.includes("collections/baby-clothing")
			) {
				submit?.addEventListener("click", (e) => {
					fireEvent(
						"PDP - Customer adds product to basket after visiting the All Clothing Page"
					);
				});
			}
		}
