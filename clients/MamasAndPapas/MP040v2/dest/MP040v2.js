var _MP040v2 = (function () {
    ...
    function activate() {
        var $ = window.jQuery;
        var $body = $('body');
        $body.addClass('MP040v2');

        // ------------------------------------------------------------------------------------------------------------
        // -------------------------------------------TRIGGERS---------------------------------------------------------
        // ------------------------------------------------------------------------------------------------------------
        var url = window.location.href, usp;
        if (/^https?:\/\/www\.mamasandpapas\.com\/en-gb\/search\/?(\?.*)?(\#.*)?$/.test(url)) {
            var searchGroups = {
                beds: {
                    type: 'contains',
                ...
                wardrobes: {
                    type: 'contains',
                    terms: ['wardrobe', 'wardrobes', 'clothing rail', 'clothes rail', 'storage'],
                    usp: 'Sturdy, full-size wardrobe. Built to last.'
                },
                drawers: {
                    type: 'contains',
                    terms: ['drawers', 'changer', 'dresser'],
                    usp: 'Beautiful furniture with practical features.'
                },
                bedroomFurniture: {
                    type: 'contains',
                    terms: ['bedroom', 'bedroom furniture', 'furniture'],
                    usp: 'Beautiful furniture with practical features.'
                },
                collections: {
                    type: 'contains',
                    terms: ['rialto', 'orchard', 'kingston', 'franklin', 'hayword', 'oxford', 'lawson', 'sienna', 'mia'],
        ...
        opts.desc = "\"This 3 piece set is simply beautiful, straightforward instructions, very well made and sturdy." +
					" Lovely big wardrobe, spacious drawers and the cot is stunning." +
					" Couldn't be happier the perfect furniture for our baby.\"";

        var createComponent = function() {
            var $html = $([
                '<div class="col-xs-12 MP040v2_no-padding">',
                '<div class="MP040v2_component">',
                '<div class="col-xs-6 col-sm-3 MP040v2_no-padding MP040v2_component_image_wrap">',
                '<div class="MP040v2_component_image MP040v2_img_1"></div>',
                '</div>',
                '<div class="col-sm-6 hidden-xs MP040v2_component_info">',
                '<div><div class="MP040v2_component_title">' + opts.title + '</div></div>',
                '<div class="MP040v2_component_desc">' + opts.desc + '</div>',
                '<div class="MP040v2_component_subtext">' + opts.subText + '</div>',
                '<a href="' + opts.link + '" class="MP040v2_component_btn">Shop Mia Ivory</a>',
                '</div>',
                '<div class="col-xs-6 col-sm-3 MP040v2_no-padding MP040v2_component_image_wrap">',
                '<div class="MP040v2_component_image MP040v2_img_2"></div>',
                '</div>',
                '<div class="visible-xs">',
                '<div class="col-xs-8 MP040v2_component_info">',
                '<div><div class="MP040v2_component_title">' + opts.title + '</div></div>',
                '<div class="MP040v2_component_desc">' + opts.desc + '</div>',
                '<div class="MP040v2_component_subtext">' + opts.subText + '</div>',
                '</div>',
