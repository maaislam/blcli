...
const MP071 = {
  ...
  init: function init() {
    // Setup
    const { settings, services, components } = MP071;
    // services.tracking();
  ...
  components: {
    ...
    createFiltersDropDown: function createFiltersDropDown() {
      ...
      newFilterBox.innerHTML = '<div class="MP071-sizes"><select class="MP071-size_filter"><option>Select a size</option></select><i class="ico ico-plus productFilter_icon"> </i></div>';
      document.querySelector('.row.productFilter_filterSelectors.p-2').insertAdjacentElement('afterbegin', newFilterBox);
