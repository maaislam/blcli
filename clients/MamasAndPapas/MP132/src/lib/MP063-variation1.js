...
import carouselContent from './MP132carouselContent';

const Promise = window.Promise; // || require('@qubit/es6-promise');
const MP063 = {
	lib: {
		/**
		 * @returns {Array} - JSON of all products on first page of /en-gb/c/footmuffs-liners
		 */
		getData: new Promise((resolve, reject) => {
			const request = new XMLHttpRequest();
			request.open('GET', '/en-gb/c/footmuffs-liners', true);
			request.onload = function() {
				if (request.status >= 200 && request.status < 400) {
					...
					const products = html.querySelectorAll('.productCard');
	...
	components: {
		carousel: {
			...
			create: function(products) {
				...
				container.classList.add('MP063_carousel', 'row');

				const heading = document.createElement('div');
				heading.classList.add('MP063_title');
				heading.innerText = 'Similar pushchairs from other brands';

				const ul = document.createElement('ul');
				ul.classList.add('MP063_products');

				const buildProductHtml = function(product) {
					const li = document.createElement('li');
					li.classList.add('MP063_product');
					...
					details.classList.add('MP063_details');
					const name = document.createElement('div');
					name.classList.add('MP063_product__name');
					name.innerText = product.name;
					const price = document.createElement('div');
					price.classList.add('MP063_product__price');
					price.innerHTML = product.price;
					const button = document.createElement('span');
					button.classList.add('btn', 'btn-default', 'w-100', 'MP063_details__btn');
			...
			attachEvents: function(component, $) {
				const $productRecs = $(component).find('.MP063_products');
				let isSliding = false;
				...
				// 		events.send('MP063', 'Click', 'V1 - Clicked footmuff product', {sendOnce: true});
				// 		qubit && qubit.sendEvent ? qubit.sendEvent('MP063:click:footmuff carousel product') : null;
				// });

				// // Send GA event when carousel is in view
				// viewabilityTracker(document.querySelector('.MP063_carousel'), () => {
				// 	events.send('MP063', 'View', 'V1 - Carousel is in view', {sendOnce: true});
				// }, {removeOnView: true});
			},
        ...
        container.appendChild(component);
				...
				// Build slick
				$(component).find('.MP063_products').slick({
					slidesToShow: 4,
					slidesPerRow: 4,
					infinite: true,
					arrows: true,
					prevArrow: '<div class="MP063_prev"></div>',
					nextArrow: '<div class="MP063_next"></div>',
					responsive: [
						{
							breakpoint: 992,
							settings: {
		...
		// 	create: function() {
		// 		const container = document.createElement('div');
		// 		container.classList.add('MP063_minibasketUpsell');

		// 		const message = document.createElement('p');
		// 		message.classList.add('MP063_minibasketUpsell__text');
		// 		message.innerText = 'Why not add a footmuff to go with this pram?'

		// 		const buttonWrap = document.createElement('div');
		// 		buttonWrap.classList.add('MP063_minibasketUpsell__btnWrap');
				
		// 		const button = document.createElement('a');
		// 		button.classList.add('MP063_minibasketUpsell__btn');
		// 		button.href = '/c/footmuffs-liners';
		// 		button.innerText = 'View all footmuffs';
		...
		// 	// 		events.send('MP063', 'Click', 'V1 - Clicked minibasket upsell', {sendOnce: true});
		// 	// 		qubit && qubit.sendEvent ? qubit.sendEvent('MP063:click:minibasket upsell') : null;
		...
		// 		this.attachEvents(component);
		// 		if (!document.querySelector('.MP063_minibasketUpsell')) {
	...
	activate: function() {
		// Setup
		fullStory('MP063', 'Variation 1');
		document.body.classList.add('MP063');
		const lib = this.lib;

		Promise.all([lib.getData, lib.waitForElements])
			.then((data) => {
        //  const productData = data[0];
        const productData = carouselContent;
				const $ = window.jQuery;
				...
				// 	/* 
				// 	 * Cookie 'MP063_addToBag' exists when a product has been added to bag. If this
				...
				// 			events.send('MP063', 'View', 'V1 - Minibasket upsell is in view', {sendOnce: true});
				// 		}
				// 	}
				...
				// 			setTimeout(() => {
				// 				setCookie('MP063_addToBag', true, null, null, 15000);
				// 				const lastProductElement = document.querySelectorAll('#basket_products .basket_product')[0];
				// 				const lastProductElementName = lastProductElement.querySelector('.basket_productTitle a').innerText;
				// 				this.components.minibasketUpsell.init(lastProductElement);
