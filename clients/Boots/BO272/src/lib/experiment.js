...
export default (data) => {
	...
	const totalPrice =
		...
		parseFloat(data[0].product_data.currentprice.toFixed(2));
	root.querySelector(
		`.${ID}-totalPrice`
	).innerText = `Total price: £${totalPrice.toFixed(2)}`;

	// Add both to bag
	const bundleProductEl = document.querySelector(
		`.${ID}-productOuter.extra .${ID}-product`
	);
	const bundleProductObjectID = bundleProductEl.getAttribute("objectid");
	const bundleProductSap = bundleProductEl.getAttribute("sap");
	const bundleProductName = bundleProductEl.querySelector(
		`.${ID}-card-title`
	).innerText;

	const addToBag1 = new AddToBag(
		currentObjID,
		parseInt(currentObjID, 10) - 1,
		model,
		currentName
	);
	const addToBag2 = new AddToBag(
		bundleProductObjectID,
		parseInt(bundleProductObjectID, 10) - 1,
		bundleProductSap,
		bundleProductName
	);
