	...
	pollerLite(["#productRecommendationsABTest"], () => {
		...
		if (data.hasOwnProperty(url)) {
			...
			pollerLite([() => window.RR?.data?.JSON], () => {
				...
				const promises = productData.map(async (item) => {
					const response = await fetch(
						`https://optimisation-api.co.uk/product-feed?sapcode=${item.id}`
					);
			...
			pollerLite([`.${ID}_recommendations`], () => {
				...
				if (VARIATION == "2") {
					...
					allDetailsCta.forEach((button) => {
						button.addEventListener("click", (e) => {
							fireEvent(
								`Click - User clicked view details CTA from the recommendations`
							);
						});
					});
				}
