 ...
 */
const BOMobileSlidingNav = ({ data, salutation, children, defaultStep }) => {
                  ...
                  <div className="MobileSlidingNav__listing-wrap">
                    ...
                    <ul className="MobileSlidingNav__listing">
                      <div class="MobileTitle" onClick={backClick}>
                        ...
                        <li>
                          <a className = {(level2kid.children ? `Lvl2Title ` : '') + (level2 == slugify(level2kid.name) ? 'MobileSlidingNav__level--active' : '')}  data-target={d.hasSubmenu ? slugify(level2kid.name) : ''} >
                                <span>{level2kid.name}</span>
                            </a>

                            {level2kid.children ?  
                            <ul className={'MobileSlidingNav__level MobileSlidingNav__level-3 innerLinks' + (level2 == slugify(level2kid.name) ? 'MobileSlidingNav__level--active' : '')}>             
                                <div className = {'innerCategories ' + level2kid.type}>
                                  {
                                  (level2kid.children || []).map((level3kid) => (
                                    <div class="thirdLink">
                                      <a className ={!level3kid.link ? 'innerTitle' : ''} href={level3kid.link ? level3kid.link : ''}>{level3kid.name}</a>  
                                    </div>
                                  ))
                                  } 
                                </div>
...
export default BOMobileSlidingNav;
