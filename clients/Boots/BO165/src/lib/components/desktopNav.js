...
const DesktopNav = ({ data, salutation, children, handleClose, defaultStep }) => {
  ...
  document.addEventListener('click', (event) => {
    var container = document.querySelector('.BO165-navigation')
  ...
  const parseData = (data) => {
   ...
   // create all levels within top level
    data.forEach(d => {
      topLevel.push(
        <li className={d.allBrands === true ? 'allBrands' : ''} style={d.desktop == false ? 'display:none' : ''}>
      ...
      if(d.hasSubmenu && d.children) {
        linkedCats.push((
            <div className={'DesktopNav__level DesktopNav__level-2 ' + (step == slugify(d.name) ? 'DesktopNav__level--active' : '')}>
                <div className="DesktopNav__listing-wrap">
                <ul className="DesktopNav__listing">
                {d.children.map((level2kid) => (  
                    <li class="Lvl2Name">
                       <a className = {(level2kid.content ? "dropdownHeader " : '') + "Lvl2Title"} href={level2kid.link || ''} data-target={d.hasSubmenu ? slugify(level2kid.name) : ''} style={level2kid.colour ? 'color:'+ level2kid.colour : ''}>
                            <span>{level2kid.name ? level2kid.name : ''}</span>
                        </a>

                        {level2kid.content ?  
                        <ul className={'DesktopNav__level DesktopNav__level-3 ' + (level2 == slugify(level2kid.name) ? 'DesktopNav__level--active' : '')}> 
