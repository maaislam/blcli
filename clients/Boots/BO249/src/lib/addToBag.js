  ...
  add() {
    return new Promise((resolve, reject) => {
        ...
        setupShop5DataRedesign(
            ...
            '', 
            '-_--_--_--_-','N','N','Y','Y','Search Successful','','',
            `add2CartBtn_${this.entitledItem}`,'1','1 '
        );
        shoppingActionsJS.Add2ShopCartAjaxRedesign(`entitledItem_${this.entitledItem}`,1, false)

        const resolvePromise = (success, error) => {
            //remove event listeners
            window.removeEventListener("add-to-basket:success", () => resolvePromise(true));
            window.removeEventListener("update-basket:success", () => resolvePromise(true));
            window.removeEventListener("add-to-basket:failure", (e) => resolvePromise(false, e));
            window.removeEventListener("update-basket:failure", (e) => resolvePromise(false, e));
        ...
        window.addEventListener("add-to-basket:success", () => resolvePromise(true));
        window.addEventListener("update-basket:success", () => resolvePromise(true));

        //capturing failures
        window.addEventListener("add-to-basket:failure", (e) => resolvePromise(false, e));
        window.addEventListener("update-basket:failure", (e) => rejectOnfail(false, e));
    })
  }
