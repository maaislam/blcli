...
const activate = () => {
  ...
  if(VARIATION == 'control') {
    setup();
    window.cmCreateManualLinkClickTag("/BO071?cm_sp=PharmacyBasket-_-BO071control-_-BO071Started");

    const addCta = document.querySelector('.styles-module__transparentButton--1QECb');
    if (addCta) {
      addCta.addEventListener('click', () => {
        window.cmCreateManualLinkClickTag("/BO071?cm_sp=PharmacyBasket-_-BO071controlAddItem-_-BO071Clicked");
      });
    }
    ...
    if (continueBtn) {
      continueBtn.addEventListener('click', () => {
        window.cmCreateManualLinkClickTag("/BO071?cm_sp=PharmacyBasket-_-BO071controlContinue-_-BO071Clicked");
      });
    }
    ...
    observer.connect(document.querySelector('.styles-modules__container--e_0EB'), () => {
        ...
        if (add) {
          add.addEventListener('click', () => {
            window.cmCreateManualLinkClickTag("/BO071?cm_sp=PharmacyBasket-_-BO071V1AddItem-_-BO071Clicked");
          });
        }
    ...
    // --- Experiment Fired
    window.cmCreateManualLinkClickTag("/BO071?cm_sp=PharmacyBasket-_-BO071v1-_-BO071Started");
    ...
    && !document.querySelector('section .styles-module__emptyBasketText--8QSj0')) {
      ...
      if (!document.querySelector(`.${ID}-subtext`)) {
        basketTitle.insertAdjacentHTML('afterend', `<p class="${ID}-subtext">Don't worry, we'll ask your GP to confirm the request and the quantity</p>`);
      }
    }
    ...
    if (!document.querySelector('section .styles-module__emptyBasketText--8QSj0')) {
      newButtonCtaContainer = `<div class="${ID}-ctaBtn__wrapper">
        <div class="${ID}-text">Do you want to add another item?</div>
    ...
    if (addCta) {
      addCta.addEventListener('click', () => {
        window.cmCreateManualLinkClickTag("/BO071?cm_sp=PharmacyBasket-_-BO071AddItem-_-BO071Clicked");
    ...
    if (skipCta) {
      skipCta.addEventListener('click', () => {
        window.cmCreateManualLinkClickTag("/BO071?cm_sp=PharmacyBasket-_-BO071Continue-_-BO071Clicked");
    ...
    observer.connect(document.querySelector('.styles-modules__container--e_0EB'), () => {
      ...
      if (document.querySelector('section .styles-module__emptyBasketText--8QSj0'))  {
        ...
        document.querySelector(`.BO071-subtext`).setAttribute('style', 'display: none;');
        document.querySelector(`.BO071-ctaBtn__wrapper`).parentNode.removeChild(document.querySelector(`.BO071-ctaBtn__wrapper`));
        newButtonCtaContainer = `<div class="${ID}-ctaBtn__wrapper">
          <div class="${ID}-buttons__wrapper">
            <div class="${ID}-button" id="add-another-item">
              <button>Add an item</button>
