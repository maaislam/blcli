...
import fetchReviews from "../lib/helpers/fetchReviews";
const { ID } = shared;
...
const productRecs = (product) => {
  ...
  const productCard = () => {
    ...
    productData.forEach((product) => {
      // fetchReviews(product.actionURL)
      //   .then(ratingsSummary => {
      //     console.log('Fetched Ratings Summary:', ratingsSummary);
      //     // Further processing of ratingsSummary here
      //   })
      //   .catch(error => {
      ...
      const totalStars = parseInt(product.roundedReviewScore);
      ...
      const reviewUrl = `${product.actionURL}#scrollToReviews`;

      r_string +=
        `<div target="_blank" class="${ID}-personalised--product swiper-slide">
        <div class="${ID}-personalised--product--image">
          <a class='${ID}-personalised--product--link' href="${product.actionURL}">
            ${isOffer ? `<div class="${ID}-roundel_section">
        ...
        <div class="${ID}-personalised--product--content">
          <h4>${productName}</h4>
          <a href="${reviewUrl}" class="${ID}-personalised--product--rating">${stars} 
            <span class='${ID}-reviews-count'>(${reviewsCount})</span>
          </a>
          ...
          ${wasPriceDisplayed ? `<span class="${ID}-saving-calc-mobile">Save £${saving.toFixed(2)}</span>` : ``}
