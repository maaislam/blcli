...
import { pharmacyData, servicesData } from './desktopData';
...
const BODesktopNav = ({ data, salutation, children, handleClose, defaultStep }) => {
  ...
  document.addEventListener('click', (event) => {
    var container = document.querySelector('.BO181-navigation');
    ...
    return ( 
      <div className={"wrapper " + name }>
        <div className={'DesktopNav__level DesktopNav__level-1 DesktopNav__level--active'}>
          ...
          {VARIATION === '1' ? <div className="chevronArr prev"></div> : ''}
          {VARIATION === '1' ? <div className="chevronArr next"></div> : ''}
            <ul className={"DesktopNav__listing DesktopNav__level-1Wrap " + name}>
              {topLevel}
            </ul>
          </div>
        </div>        
  ...
  return (
    <div className="DesktopNav" data-step={step}>
      ...
      { VARIATION == '3' ? parseData(pharmacyData(), 'pharmacy') : ''}
      { VARIATION === '4' ? parseData(servicesData(), 'services') : ''}
