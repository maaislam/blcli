...
const addItems = (productSizes) => {
  ...
  const getAllPricesPromise = new Promise((resolve, reject) => {
    ...
    [].slice.call(productSizes).forEach((el, index) => {
      sapCodeString += index == 0 ? `${el.sapCode}` : `&${el.sapCode}`;
    });

    const promise = getProductFromAPI(sapCodeString).then((response) => {
      response.forEach((element, index) => {
        let modelID = element.model;

        productSizes.find((size) => {
          if (size.sapCode == modelID) {
            let ppu = element.pricePerUnit;
            if (ppu.indexOf('UNI') > -1) {
              ppu = ppu.replace('UNI', ' wipe');
            }
            size.pricePerUnit = ppu;
            size.pricePerUnitAmount = parseFloat(element.pricePerUnit.split(' ')[0].replace('£', ''));
          }
        });
      });
    });
    ...
    Promise.all(promises)
      .then(() => {
        ...
        const indexOfBestValue = productSizes.reduce((minIndex, el, currentIndex, arr) => {
          return el.pricePerUnitAmount < arr[minIndex].pricePerUnitAmount ? currentIndex : minIndex;
        }, 0);
  ...
  getAllPricesPromise.then((productSizes) => {
    ...
    [].slice.call(productSizes).forEach((element, index) => {
      ...
      if (productSizes[index].size == sizeOfCurrent) {
        ...
        let biggerSizeSaving = productSizes[productSizes.length - 1].pricePerUnitAmount;
      ...
      if (VARIATION !== '3') {
        if (savingPriceBestValue < 0) {
          document.querySelector(`.${ID}-root`).style.marginTop = '0';
          return;
        } else {
          domString = `
      <span ${element.bestValue ? `id="${ID}-bestvalue"` : ``} class="${ID}-sizeML ${
            element.bestValue ? `${ID}-bestvalue ${ID}-bestvalue--active` : ``
    ...
    <div class="${ID}-size_inner ${productSizes.length > 1 && VARIATION !== '3' ? `multiple` : ``}">
		</span>${element.bestValue ? `<span class="${ID}-sizeMLbestvalue">Best Value</span>` : ''} 
		<div>	
      ...
      ${domString}
	  ${
      element.bestValue && savingPriceBestValue != 0 && productSizes.length > 1
        ...
        if (VARIATION == '2') {
          ...
          if (sizeEl.classList.contains(`${ID}-current`)) {
            if (element.bestValue) {
              ...
              const savingPrice = smallSizeSaving - biggerSizeSaving;
              if (savingPrice < 0) {
                const rootDiv = document.querySelector(`.${ID}-root`);
              ...
              const bestValueContext = `
				<div class="${ID}-bestValueContext ${ID}-bestvalue--active ">
              ...
              bestValueElDOM.addEventListener('click', () => {
                ...
                if (!bestValueContextEl.classList.contains(`${ID}-display-none`)) {
                  ...
                  fireBootsEvent('Click - best value shown', true, eventTypes.experience_action, {
                    action: actionTypes.click_cta,
                    action_detail: 'Click - best value shown',
                  });
                }
              });
            }
  ...
  if (isChanelOrDior) {
    ...
    if (VARIATION == 'control') {
      const productName = document
        ...
        .textContent.trim()
        .replace(/25ml|30ml|50ml|60ml|75ml|80ml|85ml|90ml|100ml|125ml|150m|200ml/, '')
        .replace(/^\s+|\s+$/g, '');

      const productSizes = sizes[productName];

      if (productSizes) {
        const sizeOfCurrentP = document.querySelector('#estore_product_title').textContent.match(/(\d+(?:\.\d+)?)\s?(ml)\b/i)[0];
        // fireEvent(`Current product size is ${sizeOfCurrentP}`);
        // fireBootsEvent(`Current product size is ${sizeOfCurrentP}`, true, eventTypes.experience_render, {
        // 	rendered_element: elementTypes.Variant_Selector,
        // 	render_detail: `Current product size is ${sizeOfCurrentP}`
        // })
      }
    ...
    let productName = document
      .querySelector('#estore_product_title')
      .textContent.trim()
      .replace(/25ml|30ml|50ml|60ml|75ml|80ml|85ml|90ml|100ml|110ml|125ml|150m|200ml/, '')
      .replace(/^\s+|\s+$/g, '')
      .replace('  ', ' ')
      .toLowerCase();
    ...
    if (productSizes) {
      ...
      // fireBootsEvent("Conditions Met", true, eventTypes.experience_load)
      ...
      const sizeOfCurrentP = document.querySelector('#estore_product_title').textContent.match(/(\d+(?:\.\d+)?)\s?(ml)\b/i)
        ? document.querySelector('#estore_product_title').textContent.match(/(\d+(?:\.\d+)?)\s?(ml)\b/i)[0]
        : '';
      ...
      if (VARIATION == '3') {
        testProductSizes = testProductSizes.filter(
          (element) => parseInt(element.size.replace('ml', '')) >= parseInt(sizeOfCurrentP.replace('ml', ''))
        );
      }
        ...
        if (VARIATION !== 'control') {
              ...
              index.search(sapCode).then(({ hits }) => {
                if (hits[0].pricePerUnit && hits[0].currentPrice) {
                  ...
                  prodToBeAdded.push({
                    ...
                    pricePerUnitAmount: parseFloat(hits[0].pricePerUnit.split(' ')[0].replace('£', '')),
                    bestValue: bestValue ? true : false,
                    url: hits[0].actionURL,
                  });
                  if (prodToBeAdded.length === totalProductLength) {
                    if (VARIATION == '1' || VARIATION == '2') {
                      const lowest = prodToBeAdded.reduce((prev, cur) =>
                        cur.pricePerUnitAmount < prev.pricePerUnitAmount ? cur : prev
                      );
          ...
          document.body.addEventListener('click', (e) => {
            if (e.target.closest(`.${ID}-size`) || e.target.classList.contains(`.${ID}-size`)) {
              ...
              // 	true
              // );
              fireBootsEvent(`Click - size button`, true, eventTypes.experience_action, {
                action: actionTypes.click_cta,
                action_detail: `Click - size button`,
              });
            }
          });
