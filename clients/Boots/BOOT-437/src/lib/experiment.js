...
const updatePLPItems = () => {
  ...
  allCurrentPLPItems.forEach((item) => {
    ...
    if (templateType == "PLC" && item.innerHTML.indexOf('3f2') > -1) {
      item.classList.add(`${ID}-3for2`);
    }

    if (templateType == "PL-NEW" && item.innerHTML.indexOf('3 for 2') > -1) {
      item.classList.add(`${ID}-3for2`);
    }
      ...
      success: function (returnedData) {
        if (returnedData) {
          ...
          if(returnedData.basketDetails === undefined) {
            ...
            let allCurrItemsInBasket = returnedData.basketDetails.orderItems;
            ...
            let lastUniquePromo = allUniquePromoDetails.filter((item) => {
             if(item.promoName.indexOf('3f2') > -1 || item.promoName.indexOf('3 for 2') > -1) {
                return true;
              }
            });
      ...
      <a href="${promoURL}" class="${ID}-basket3for2">
        ...
        <div class="${ID}-basket3for2--inner">
          ...
          <div class="${ID}-basket3for2--inner--text">
            ...
            <p> Add <span id="${ID}-number">${theNumber}</span> more <span id="${ID}-prodstext">product${number > 1 ? `s` : ``}</span> and get the cheapest free. </p>
          </div>
...
const setupBasketCode = (number, promoURL, promoDescription) => {
  ...
  if(overlayObserver == null) {
    overlayObserver = observer.connect(document.querySelector('.oct-overlay'), () => {
      setTimeout(() => {
        checkBasketCode(number, promoURL, promoDescription);
      }, 500);
...
const startExperiment = (currentPromo) => {
  ...
  let addNows = Array(addNowsToAttach).fill().map((item, index) => {
    ...
    <div class="${ID}-deal-container">
      ...
      <div class="${ID}-deal">
        ...
        ${allItems.map((item) => {
          ...
          return `
            <a href="https://www.boots.com/${item.item.displayUrl.indexOf(';') > -1 ? item.item.displayUrl.split(';')[0] : item.item.displayUrl}" class="${ID}-deal--product">
  ...
  if(completelyHidden == false) {
    ...
    fireEvent(`Interaction - slideout created and ${VARIATION == "control" ? `not shown` : `shown`}, user has two 3f2 products in their basket`, true);
...
export default () => {
  ...
  getLast3for2Deal().then((promo) => {
    console.log(promo);
    if((promo.length > 0)) {
      if(promo[0].promoItems?.length > 0) {
        startExperiment(promo[0]);
      }
    } 
  ...
  if(VARIATION == 1) {
    ...
    pollerLite(['.oct-template #octListers'], () => {
      templateType = "PL-NEW";
      observer.connect(document.querySelector('#octListers'), () => {
        updatePLPItems();
      },
        {
          childList: true,
