...
const variation1 = () => {
  fireEvent("V1 Fired Audience:A");
  fireEvent("Audience:A Slide active and visible in user's viewport");
  pollerLite([".oct-carousel-hero .swiper-slide"], () => {
    ...
    const runExperiment = (carousel) => {
      ...
      const targetSlide = carouselSlides[0];
      ...
      const intersectionObserver = new IntersectionObserver((entries) => {
        if (entries[0].isIntersecting) {
          ...
          entries[0].target.addEventListener("click", () => {
            fireEvent("Audience:A Slide clicked by user");
          });
        }
      });
