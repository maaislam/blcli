...
import { pageType, productToStore, scrollToElement } from "../helpers";
...
const { ID, VARIATION } = shared;
  ...
  const addToFavourites = (el) => {
    ...
    // Show how to use once/scroll to top - show overlay, bring heart forward with class and add to storage
    ...
    if (favouritesStorage && favouritesStorage.items && favouritesStorage.items.filter((item) => item.productId == productId).length > 0) {
      ...
      fireEvent('Clicked add to favourites ' + pageType());
    }
  ...
  const closeMsg = () => {
    document.querySelector('.favourites-notif').remove();
    sessionStorage.setItem('favouritesNotif-close', 'true');
  }
  ...
  return (
    <div>
      <div className={activeView == "quick-view" ? "textTop absolute" : "textTop"}>
        ...
        {!sessionStorage.getItem('favouritesNotif-close') ? <div className="favourites-notif"><p><b>Customer Notice:</b> Our on site favourites feature will be reverting back to our old functionality in My Account on 01/01/2023. This means some customers may lose visibility of saved products. We apologise for any inconvenience caused.</p><div className="favourites--notif-close" onClick={closeMsg}></div></div> : ''}
      </div>
