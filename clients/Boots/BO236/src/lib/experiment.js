...
export default () => {
  ...
  //   notif.classList.add(`${ID}-notification`);
  //   notif.innerHTML = `
  //   <div class="${ID}-inner">
  //     <p><span>Customer Notice:</span> Our on site favourites feature will be revert back to our old functionality in the My Account on 01/01/2023. This means some customers may lose visibility of saved products. We apologise for any inconvenience caused.</p>
  //   </div>`;
  //   document.querySelector(`#header`).insertAdjacentElement('afterend', notif);
  // }
  ...
  addModalToggle();
  addFavouritesIcon();
  // if(!sessionStorage.getItem('faveNotif')) {
  //   favouriteNotification();
  //   sessionStorage.setItem('faveNotif', true);
  // }
  openFavourites();
};
