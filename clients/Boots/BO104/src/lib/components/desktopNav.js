                ...
                <div className={'DesktopSlidingNav__level DesktopSlidingNav__level-2 ' + (step == slugify(d.name) ? 'DesktopSlidingNav__level--active' : ''
                  ...
                  <div className="DesktopSlidingNav__listing-wrap DropdownContainer">
                    ...
                    <ul className="DesktopSlidingNav__listing">
                        ...
                        <li className = {level2kid.type}>
                          <a className = {(level2kid.children ? `Lvl2Title ` : '') + (level2 == slugify(level2kid.name) ? 'DesktopSlidingNav__level--active' : '')}  data-target={d.hasSubmenu ? slugify(level2kid.name) : ''} >
                                <span>{level2kid.name}</span>
                            </a>

                            {level2kid.children ?  
                            <ul className={'DesktopSlidingNav__level DesktopSlidingNav__level-3 innerLinks ' + (level2 == slugify(level2kid.name) ? 'DesktopSlidingNav__level--active': '')}>             
                                <div className = 'innerCategories'>
                                  {
                                  (level2kid.children || []).map((level3kid) => (
                                    <div class="thirdLink">
                                      <a className ={!level3kid.link ? 'innerTitle' : ''} href={level3kid.link ? level3kid.link : 'javaScript:void(0)'}>{level3kid.name}</a>  
                                    </div>
                                  ))
                                  } 
                                </div>
