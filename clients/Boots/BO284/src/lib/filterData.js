...
export const categoryData = () => {
  ...
  filters = document.querySelectorAll('[id="product type"] .facetSelect li a');

  for (let index = 0; index < filters.length; index += 1) {
    const element = filters[index];
    const isHidden = window.getComputedStyle(element.closest('li')).display === 'none';
    const elID = element.getAttribute('id');
    const elName = `${element.querySelector('span.outline').innerText.substring(0, element.querySelector('span.outline').innerText.indexOf('(')).trim()}`;
    const elHref = `${element.getAttribute('href')}`;
    const isActive = element.querySelector('.outline').classList.contains('facetSelectedHighlight') ? true : false;

    logMessage(`Elname: ${elName} isHidden: ${isHidden} elID: ${elID} elHref: ${elHref} isActive: ${isActive}`)
    ...
    dataObj['target'] = elID;
    dataObj['active'] = isActive;
    dataObj['link'] = elHref;
    !isHidden && filterArr.push(dataObj);
  }
...
export const addCategoryFilters = () => {
  ...
  Object.keys(allFilters).forEach((i) => {
    ...
    if (data.active === true) {
      filter.classList.add(`${ID}-filterActive`);
    } 
