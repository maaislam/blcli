...
const offerWrapper = (id, device, acceptOffers) => {
  const html = `
        <div class="${id}__offerWrapper ${id}__${device}">
            <div class="${id}__offersContainer">
                <!-- Load Now Section -->
                <div class="${id}__load-now">
                    <span class="${id}__load-title" style="display:none;">Load now:</span>
                    <button class="${id}__load-button ${acceptOffers.length === 0 ? `${id}__loaded` : ''}">
                        <span class="${id}__load-card-icon">${cardIcon}</span>
                        <span class="${id}__load-card-title">${
    acceptOffers.length === 0 ? 'All offers added' : 'Add all offers to card'
  }</span>   
