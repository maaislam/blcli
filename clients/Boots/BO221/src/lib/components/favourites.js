...
const { ID, VARIATION } = shared;
  ...
  const addToFavourites = (el) => {
    ...
    if(VARIATION === '2' || VARIATION === '3') {
      if(favouritesStorage.items.length === 1 && !localStorage.getItem('BO-usageShown')) {
        let showOnce = false;
        if(showOnce === false) {
          showOnce = true
          scrollToElement(document.body);
          document.querySelector(`.${ID}-favouritesToggle`).classList.add("usage-active");
          localStorage.setItem('BO-usageShown', true);
          setTimeout(() => {
            document.querySelector(`.${ID}-favouritesToggle`).classList.remove("usage-active");
          }, 4000);
        }
      }
    }
