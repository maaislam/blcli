...
const getLastUniquePromo = (returnedData) => {
  ...
  return new Promise((resolve) => {
    ...
    let lastUniquePromo = allUniquePromoDetails.filter((item) => {
      if (item.promoName.indexOf('3f2') > -1 || item.promoName.indexOf('3 for 2') > -1 || item.promoName.indexOf('Xmas Mix and Match') > -1) {
        return true;
      }
    });
...
export default () => {
  ...
  if(sessionStorage.basketDetails !== null) {
    let details = JSON.parse(sessionStorage.basketDetails);
    console.log(details.basketDetails);
  ...
  window.addEventListener("oct-basket:updated", (data) => {
    ...
    console.log("BASKET UPDATE DATA: ", data);
