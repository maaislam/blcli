...
const init = () => {
  ...
  setTimeout(() => {
    ...
    elements.forEach((element) => {
      ...
      element.querySelector('span').textContent = "Contacta con ventas";
...
export default () => {
  ...
  document.body.addEventListener('click', (e) => {
    ...
    if (target.closest('.css-140tqjo')) {
      ...
      if (target.getAttribute('href').includes('https://content.gocardless.com/historial-administrativo-de-pagos/el-futuro-es-ahora')) {
        fireEvent('User interacts with the “Más información” CTA');
      }

      if (target.getAttribute('href').includes('https://content.gocardless.com/historial-administrativo-de-pagos-m/travel-perk')) {
        fireEvent('User interacts with the “Leer estudio de caso” CTA');
      }

      if (target.getAttribute('href').includes('https://content.gocardless.com/historial-administrativo-de-pagos-m/existe-una-mejor-forma-de-cobrar')) {
        fireEvent('User interacts with the “Mirar infografía” CTA on the third customer story');
      }
    }

    if (target.closest('.css-1lma31c')) {

      if (target.closest('.css-1lma31c').querySelector('a').getAttribute('href').includes('https://content.gocardless.com/historial-administrativo-de-pagos-m/travel-perk')) {
        fireEvent('User interacts with case studies (travel-perk-case-study)');
      }

      if (target.closest('.css-1lma31c').querySelector('a').getAttribute('href').includes('https://content.gocardless.com/historial-administrativo-de-pagos-b/suop')) {
        fireEvent('User interacts with case studies (suop-case-study)');
      }

      if (target.closest('.css-1lma31c').querySelector('a').getAttribute('href').includes('https://content.gocardless.com/historial-administrativo-de-pagos-b/referencias-docusign')) {
        fireEvent('User interacts with case studies (docusign-case-study)');
      }

      if (target.closest('.css-1lma31c').querySelector('a').getAttribute('href').includes('https://content.gocardless.com/historial-administrativo-de-pagos-b/witei')) {
        fireEvent('User interacts with case studies (witei-case-study)');
      }
    ...
    if (
      ...
      target.getAttribute('href').includes('https://gocardless.com/es-es/contacto/')
    ) {
