...
const init = () => {
  ...
  setTimeout(() => {
    ...
    elements.forEach((element) => {
      ...
      element.querySelector('span').textContent = "Contact sales";
...
export default () => {
  ...
  document.body.addEventListener('click', (e) => {
    ...
    if (target.closest('.css-140tqjo')) {
      ...
      if (target.getAttribute('href').includes('https://content.gocardless.com/payment-admin-history/what-are-bank-payments')) {
        fireEvent('User interacts with the “Read the blog” CTA');
      }

      if (target.getAttribute('href').includes('https://content.gocardless.com/payment-admin-history/capital-on-tap-case-study')) {
        fireEvent('User interacts with the “See case study” CTA');
      }

      if (target.getAttribute('href').includes('https://content.gocardless.com/payment-admin-history-m/costs-of-collecting-payments-ebook')) {
        fireEvent('User interacts with the “See the ebook” CTA on the third customer story');
      }
    }

    if (target.closest('.css-1lma31c')) {

      if (target.closest('.css-1lma31c').querySelector('a').getAttribute('href').includes('https://content.gocardless.com/payment-admin-history/capital-on-tap-case-study')) {
        fireEvent('User interacts with case studies (capital-on-tap-case-study)');
      }

      if (target.closest('.css-1lma31c').querySelector('a').getAttribute('href').includes('https://content.gocardless.com/payment-admin-history-m/plum-case-study')) {
        fireEvent('User interacts with case studies (plum-case-study)');
      }

      if (target.closest('.css-1lma31c').querySelector('a').getAttribute('href').includes('https://content.gocardless.com/payment-admin-history-b/nude-story')) {
        fireEvent('User interacts with case studies (nude-case-study)');
      }

      if (target.closest('.css-1lma31c').querySelector('a').getAttribute('href').includes('https://content.gocardless.com/payment-admin-history-b/nhs-open-space')) {
        fireEvent('User interacts with case studies (nhs-open-space-case-study)');
      }
    ...
    if (
      ...
      target.getAttribute('href').includes('https://gocardless.com/contact-sales/')
    ) {
