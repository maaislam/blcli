...
const init = () => {
  ...
  // ...
  const getGeo = () => {
    return window.location.pathname.includes('/en-us') ? 'usa' : window.location.pathname.includes('/en-au') ? 'au' : 'uk';
  };
  ...
  const anchorElm = pageHasNoGco08 ? firstHeading : articleMid;
  anchorElm.insertAdjacentHTML('beforebegin', benefitBlock(ID, casestudyCopy[getGeo()], pageHasNoGco08));
