...
const init = (pageData) => {
  ...
  if (window.location.pathname !== '/guides/posts/uncovering-payment-costs-webinar/') {
...
export default (pageData) => {
  ...
  document.body.addEventListener('click', (e) => {
    ...
    if (window.location.pathname === '/guides/posts/uncovering-payment-costs-webinar/') {
      if (target.closest('.css-1w1zhcb')) {
        fireEvent('User interacts with the anchor on the webinar page');
      }

      if (target.closest('.css-1xg51cq')) {
        fireEvent('User interact with the contact sales CTA on the webinar page');
      }
    }
