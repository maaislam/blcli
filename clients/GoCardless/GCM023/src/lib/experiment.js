...
const init = () => {
  ...
  setTimeout(() => {
    ...
    elements.forEach((element) => {
      ...
      element.querySelector('span').textContent = "Contactez-nous";
...
export default () => {
  ...
  document.body.addEventListener('click', (e) => {
    ...
    if (target.closest('.css-140tqjo')) {
      ...
      if (target.getAttribute('href').includes('https://content.gocardless.com/paiement-admin-passe/paiements-bancaires')) {
        fireEvent('User interacts with the “Read the article" CTA');
      }

      if (target.getAttribute('href').includes('https://content.gocardless.com/paiement-admin-passe-m/temoignage-payfit')) {
        fireEvent('User interacts with the “See case study” CTA');
      }

      if (target.getAttribute('href').includes('https://content.gocardless.com/paiement-admin-passe/le-vrai-cout-de-la-collecte-des-paiements')) {
        fireEvent('User interacts with the “view infographic” CTA on the third customer story');
      }
    }

    if (target.closest('.css-1lma31c')) {

      if (target.closest('.css-1lma31c').querySelector('a').getAttribute('href').includes('https://content.gocardless.com/paiement-admin-passe-b/luko-temoignage')) {
        fireEvent('User interacts with case studies (luko-case-study)');
      }

      if (target.closest('.css-1lma31c').querySelector('a').getAttribute('href').includes('https://content.gocardless.com/paiement-admin-passe-b/zenchef')) {
        fireEvent('User interacts with case studies (zenchef-case-study)');
      }

      if (target.closest('.css-1lma31c').querySelector('a').getAttribute('href').includes('https://content.gocardless.com/paiement-admin-passe-b/doctolib-fr')) {
        fireEvent('User interacts with case studies (doctolib-case-study)');
      }

      if (target.closest('.css-1lma31c').querySelector('a').getAttribute('href').includes('https://content.gocardless.com/paiement-admin-passe-m/payfit')) {
        fireEvent('User interacts with case studies (payfit-case-study)');
      }
    ...
    if (
      ...
      target.getAttribute('href').includes('https://support.gocardless.com/hc/fr/requests/new')
    ) {
