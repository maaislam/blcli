...
const getPanels = () => {
  domPanels.hero = document.querySelector(
    'main > div > [data-testid="heroSlice"]'
  );

  // Few of these exist.
  const mediaAndInfoSlices = document.querySelectorAll(
    'main > div > [data-testid="mediaAndInfoSlice"]'
  );
  ...
  const featuresSlies = document.querySelectorAll(
    'main > div > [data-testid="moduleFeaturesSlice"]'
  );
  if (featuresSlies) {
  ...
  // 3 of these exist.
  const moduleSlice = document.querySelectorAll(
    'main > div > [data-testid="moduleSlice"]'
  );
  if (moduleSlice) {
