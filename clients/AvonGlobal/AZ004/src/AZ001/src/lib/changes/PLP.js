...
export default () => {
  ...
  const moveToolbars = () => {
    ...
    const changesAlreadyMade = $topBar.children(`.${ID}_heading_left, .${ID}_heading_right`).length > 0;
    if (!changesAlreadyMade) {
      ...
      if ($topPagination.length) $topPagination.wrapAll(`<div class="${ID}_heading_left"></div>`);
      const $topLayoutControls = $topBar.find('.ProductListSort, .ProductListLayout');
      if ($topLayoutControls.length) $topLayoutControls.wrapAll(`<div class="${ID}_heading_right"></div>`);

      const $bottomPagination = $bottomBar.find('.ProductListPaging');
      if ($bottomPagination.length) $bottomPagination.wrapAll(`<div class="${ID}_heading_left"></div>`);
      const $bottomLayoutControls = $bottomBar.find('.ProductListSort, .ProductListLayout');
      if ($bottomLayoutControls) $bottomLayoutControls.wrapAll(`<div class="${ID}_heading_right"></div>`);
    }
  };
  ...
  const moveSocialIcons = () => {
    ...
    const $heading = !IsPhone ? $('.ProductListHeading > h1') : $('#BreadcrumbBar > [ng-if="Layout.IsPhone"]:has(h1)');
  ...
  const moveProductCarousel = () => {
    ...
    if ($productCarousel.length) {
      ...
      if (moreThan6) {
        ...
        */
        $productCarousel.after('<div class="ProductListCell" style="display: none;"></div>');
      } else {
  ...
  const init = () => {
    ...
    // Update heading
    const h1 = document.querySelector(`.${ID}.Layout_Phone #BreadcrumbBar > [ng-if="Layout.IsPhone"] h1`);
    const breadcrumbsLinks = document.querySelectorAll('#Breadcrumbs a');
    if(h1 && h1.innerText.trim() == '' && breadcrumbsLinks.length) {
      h1.innerHTML = breadcrumbsLinks[breadcrumbsLinks.length - 1].innerText.trim();
    }
