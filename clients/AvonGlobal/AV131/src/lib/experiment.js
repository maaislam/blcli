...
export default () => {
	...
	const addToCart = (Sku) => {
		...
		return fetch(addToCartEndpoint, options).then((response) =>
			response.json()
		);
	};
	...
	document.body.addEventListener("click", ({ target }) => {
		...
		if (target.closest(`.${ID}__Addtobag`)) {
			...
			addToCart(skuClicked).then((data) => {
				...
				fireEvent(
					`User adds ${data.Data.LastProductChanged.Name} to basket`
				);
	...
	if (window.location.pathname === "/cart") {
		...
		fetch(`/api/Cartapi/Cart?_=${Date.now()}`)
			...
			.then((data) => {
				...
				if (sampleInBasket > 2) {
					fireEvent(
						`User has at least 3 samples in their basket (${sampleInBasket} samples)`
					);
				}
			});
	}
	if (VARIATION === "control") {
		...
		pollerLite(
			[".Cart-Products", () => window.location.pathname === "/cart"],
			() => {
					...
					<div class="${ID}-container">
						<div class="${ID}-header">
							<div class="${ID}-header-content">
								...
								<p>
									Beat the rush and grab one of these
									unmissable offers before you checkout.
								</p>
							</div>
						</div>
				...
				allSampleProductIds.forEach((id) =>
					promises.push(
						fetch(
							`/api/productsapi/getproducts?language=en&campaignNumber=${window._ShopContext.CampaignNumber}&productIds=${id}`
						)
							.then((res) => res.json())
							.then((data) => {
								console.log(data);
								...
								const {
									...
									Sku,
									Name: variantName,
								} = VariantGroups[0].Variants[variantIndex];
								...
								const variantId = Sku;
								const discountPercent = prevPrice
									? ((prevPrice - price) * 100) / prevPrice
									: "0";
								return {
									id,
									url,
				...
				Promise.all(promises).then((res) => {
					...
					while (i < sampleProducts.length && iterations <= 2) {
						...
						const isAlreadyInBasket = !!document
							...
							.querySelector(`[href*="${product.id}"]`);

						const btnText = isAlreadyInBasket
							? "Added"
							: product.soldOut
							? "Out of stock"
							: "Add to bag";
						carousel.add(/* HTML */ `
							<li class="splide__slide ${ID}__splide-slide">
								<div
									class="${ID}-product-card"
									data-sample-card
								>
									<a
										class="${ID}-product-card-image"
										href="${product.url}"
									>
										<img
											src="${product.image}"
										...
										${product.discountPercent > 0
											? `<span class="${ID}-product-card-discount-label">${product.discountPercent.toFixed(
													0
											  )}% OFF</span>`
											: ""}
									</a>
									<div class="${ID}-product-card-content">
										...
										<p>
											<span
												class="${ID}-product-card-price"
												>${formatPrice(
													product.price
												)}</span
											>
											${product.isOnSale
												? `<span class="${ID}-product-card-prevprice">${formatPrice(
														product.prevPrice
												  )}</span>`
												: ""}
										</p>
										<div>
											<button
												class="${ID}-cta ${ID}-ghost ${ID}__${btnText
													.split(" ")
													.join("")}"
												type="submit"
					...
					document
						.querySelectorAll(`.${ID}-product-card .${ID}-cta`)
						.forEach((item) => {
							item.addEventListener("click", () => {
								fireEvent(
									"User adds a product from the list to the bag via the add cta"
								);
							});
						});
