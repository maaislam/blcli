...
export default () => {
  ...
  // -----------------------------
  const controlApplySection = document.getElementById('apply');
  if (VARIATION == 'control') {
    ...
    obsIntersection(controlApplySection, intersectingCalbackCtrl, intersectionConfig);
    controlApplySection.addEventListener('click', (e) => {
  ...
  controlApplySection.classList.add(`${ID}__hide`);

  //render new section

  document.querySelector(`.${ID}__steps--section`)?.remove();

  controlApplySection.insertAdjacentHTML('afterend', allSteps(ID, stepsCopy));
