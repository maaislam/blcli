  ...
  addedNodes.length <= 1 &&
    addedNodes.forEach((node) => {
      ...
      if (node.nodeType === 1 && node.matches('.page_products_section')) {
        ...
        //       const newBtn = `<a class="${ID}__custom-btn btn-full" href="https://www.shopwithmyrep.co.uk/checkoutmobile/login/">Checkout online</a>`;
...
export default () => {
  ...
  pollerLite(['body'], () => {
    ...
    //   return location.pathname.indexOf('/checkoutmobile/') !== -1 && document.referrer === 'https://online.shopwithmyrep.co.uk/';
    // };
    // fireEvent('Test Code Fired');
    // //console.log('test running', !!JSON.parse(sessionStorage.getItem('SessionContext_GB'))?.ShopperRepId);
    // const checkShopperRepId = () =>
    //   location.search.split('attach=')[1] || !!JSON.parse(sessionStorage.getItem('SessionContext_GB'))?.ShopperRepId;
    // if (isBrochureUser()) {
    //   (function pollForSession() {
    //     //const shopperRepIdInUrl = location.search.split('attach=')[1];
    //     if (checkShopperRepId()) {
    ...
    const repIdFetchUrl = () => {
      ...
      const idFetchUrl = `/check-rep-url.php?market=UK&rep_id=&url=${encodeURI(location.href + urlParam)}`;
    ...
    if (document.querySelector('.catalogue-cover')) {
      ...
      fetch(idFetchUrl)
        ...
        .then((data1) => {
          fetch(`https://admin.we.avon.digital-catalogue.com/api/campaigns/${campaignNum}/settings/?repId=${data1['rep_id']}`)
