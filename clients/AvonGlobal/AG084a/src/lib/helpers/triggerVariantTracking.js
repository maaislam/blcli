...
const triggerVariantTrackings = (id, target, updateQuantity, fireEvent) => {
  ...
  if (target.matches(`.${id}__plus-btn`) || target.closest(`.${id}__plus-btn`)) {
    ...
    document.querySelector('[data-item-id="wishlistContainer"]').classList.remove('AG084a__basketContainer');
    ...
    const buttons = btnContainer.querySelector('.actions').getElementsByTagName('button');
    const AG085a_button = document.querySelector('.AG085a__custom-btn');
    clickType === 'online'
      ? AG085a_button
        ? AG085a_button.click()
        : buttons[0].click()
