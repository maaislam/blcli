// const renderOrderOptions = (id, repName) => {
//   const isCountryUK = PDP_MANAGER['API_DATA']['market'] === 'en-GB';
...
//         <div class="${id}__no-rep--headline ">Send your order to Avon REP</div>

//         <button class="${id}__no-rep--whatsapp-btn  btn-full">\${SEND WITH WHATSAPP}</button>
//         <button class="${id}__no-rep--email-btn  btn-full">\${SEND VIA EMAIL}</button>
//         <button class="${id}__no-rep--copy-btn  btn-full">\${COPY LINK}</button>
...
//         <div class="${id}__uk-delivery-promo ${!isCountryUK ? `${id}__hide` : ''}">Free delivery when you spend £20 or more</div>
//         <div class="${id}__checkout-option1">
//             <label for="online"><input type="radio"
//                     id="online"
//                     name="checkoutmethod"
//                     value="A"
//                     checked="checked"> ${cardIcon} \${Checkout online}</label>
//             <span>\${pay online now}</span>
//             <span>\${Delivery from our trusted courier}</span>
//         </div>
//         <div class="${id}__checkout-option2">
//             <label for="rep"><input type="radio"
//                     id="rep"
//                     name="checkoutmethod"
//                     value="B"> ${userIcon} \${Send order to rep}</label>
...
//     </div>`;
//   const rep = !!PDP_MANAGER['API_DATA']['rep_id'] ? withRep : withoutRep;
//   return rep;
// };
...
const renderOrderOptions = (id, repName, cartData) => {
  ...
  const detectSingleBtnType = () => {
    ...
    return checkoutButtonns[0].innerText.trim() === 'Send to Representative' ? 'sendToRep' : 'checkout';
  };
  ...
  //<button class="${id}__no-rep--paiddelivery-btn  btn-full">\${SEND WITH PAID DELIVERY}</button>
  const withoutRep = () => {
    const btns = document.querySelector('.checkout_send_to_rep_section').getElementsByTagName('button');
    const duplicateBtns = [...btns]
      .map((btn) => {
        btn.classList.add(`${id}__no-rep-${btn.innerText.split(' ').join('')}--btn`);
        return `<button ${disableBtn ? 'disabled' : ''} class="${id}__no-rep-${btn.innerText
          .split(' ')
          .join('')}--btn  btn-full">${btn.innerText}</button>`;
      })
      .join('\n');

    return `<div class="${id}__no-rep">
             <div class="${id}__no-rep--headline ">Send your order to Avon REP</div>
              ${duplicateBtns}
             </div>
    `;
  ...
  //       <button ${
  //         disableBtn ? 'disabled' : ''
  //       } class="${id}__no-rep--paiddelivery-btn  btn-full">Send your order to Avon REP</button>
