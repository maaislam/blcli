...
const renderbasket = (id, data, brocureSettings, fireEvent) => {
  ...
  if (window.matchMedia('(max-width: 778px)').matches) {
    ...
    document.querySelector('.AG084a__totals').insertAdjacentElement('afterend', couponSection);
  } else {
  ...
  const setPosition = () => {
    const promoContainer = document.querySelector(`.AG084a__promo`);
  ...
  document.querySelector(`.AG084a__checkout-total`).style.transform = `translateY(-${setPosition()}px)`;
};
