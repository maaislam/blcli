const saveOrder = (id, repName) => {
  ...
  const controlSaveorderBtn = document.querySelector(`input[placeholder="Write an order reference"]`);
  controlSaveorderBtn.classList.add(`${id}__controlsaveorder-input`);
  controlSaveorderBtn.closest('.order_details_section').classList.add(`${id}__hide`);
