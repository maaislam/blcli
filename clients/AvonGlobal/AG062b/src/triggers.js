      ...
      () => {
        ...
        if (VARIATION.toLowerCase() !== "control") {
          ...
          $("#BrandsSection select").change(() => {
            events.send(`${ID}-${VARIATION}`, "Click", "Brands filter");
          });

          $("#CategoriesSection .CategoryItem").click(() => {
            events.send(`${ID}-${VARIATION}`, "Click", "Category filter");
          });
          ...
          $("#AdditionalLinksSection .AdditionalLinkItem").click(() => {
            events.send(
              `${ID}-${VARIATION}`,
              "Click",
              "Additional item filter"
            );
          });

          $(".ProductListHeading .select2-choice").click(() => {
            events.send(`${ID}-${VARIATION}`, "Click", "Sort by");
          });
        }
      }
    );
