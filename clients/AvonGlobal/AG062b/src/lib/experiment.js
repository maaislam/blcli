...
`;
  ...
  const toggleButtons = () => {
    ...
    $(`.${ID}_filtersButton`).click(function () {
      if (isToggleOpen($(this))) hideToggles();
        ...
        events.send(`${ID}-${VARIATION}`, "Click", "Filters open");
      }
    ...
    $(`.${ID}_sortButton`).click(function (e) {
      if (isToggleOpen($(this))) hideToggles();
        ...
        events.send(`${ID}-${VARIATION}`, "Click", "Sort by open");
      }
  ...
  const adjustFiltersSort = () => {
    const $title = $(`<h2 class="${ID}_title">Фільтри</h2>`);
      ...
      <div class="${ID}_selected">
        ...
        >
          ...
          <div
            ...
            class="${ID}_selectedItem"
          >
            <span>Ціна: {{filter.Min | currency}} - {{filter.Max | currency}}</span>
    ...
    const $titleSorts = $(`<h2 class="${ID}_title">Сортувати за</h2>`);
    ...
    $navScope.$apply(() => {
      ...
      $navScope.FacetService.clearAllFilters = () => {
        events.send(`${ID}-${VARIATION}`, "Click", "Clear all filters");
  ...
  const updateRatings = () => {
    ...
    $(`.${ID}_ratings .Star`).click(function () {
      ...
      events.send(`${ID}-${VARIATION}`, "Filter selected - Rating", star);
    });
  };
  ...
  const updateSorts = () => {
    ...
    if ($sortScope) {
      ...
      $sorts = $(`
        <div class="${ID}_content">
          <p style="font-size: 16px">Нема чого сортувати</p>
        </div>
      `);
    }
  ...
  const eventTracking = () => {
    ...
    $(".CategoriesAndBrands li a").click(function () {
      ...
      events.send(`${ID}-${VARIATION}`, "Filter selected - Category", label);
    });
    ...
    $(".CategoryAndBrandFilters.brand_facet li label").click(function () {
      ...
      events.send(`${ID}-${VARIATION}`, "Filter selected - Brand", label);
    });
    ...
    $(".CategoryAndBrandFilters.discountband_facet li label").click(
      function () {
        ...
        events.send(`${ID}-${VARIATION}`, "Filter selected - Discount", label);
      }
    );
    ...
    $(".CategoryAndBrandFilters.new_facet li label").click(function () {
      ...
      events.send(`${ID}-${VARIATION}`, "Filter selected - New", label);
    });

    $("#PriceSlider .ngrs-handle").mousedown(function () {
      events.send(`${ID}-${VARIATION}`, "Filter selected - Price");
    });
  };
  ...
  const init = () => {
    ...
    // Variation 2 - sticky toggles
    if (VARIATION === "2") {
      const $products = $(".ProductList");
      $(window).scroll(
        throttle(() => {
          if ($(window).scrollTop() > $products.offset().top) {
            $buttonsWrapper.addClass(`${ID}_fixed`);
          } else {
            $buttonsWrapper.removeClass(`${ID}_fixed`);
          }
        }, 100)
      );
    }
