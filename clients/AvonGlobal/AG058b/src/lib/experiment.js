...
export default () => {
  ...
  const init = () => {
    ...
    $headerWrapper.append($socials).append($backWrapper).append($description).append($catsWrapper);
    $breadcrumbs.after($headerWrapper);
    ...
    setTimeout(() => $('[ng-controller="ProductListController"]').before($('.ProductListHeading')), 500);
  // Event listeners
