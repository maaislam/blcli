...
const renderProdlist = (fireEvent, data, favResult, parentElm, id) => {
  ...
  const htmlStr = (info) => {
    ...
    const productUrl = prodUrl || `/product/${Id}/${Slug}`;

    const imageToUse =
      imgUrl ||
      `${
        window.location.origin
      }//assets/${Language.toLowerCase()}-${Market.toLowerCase()}/images/product/prod_${ProfileNumber}_${1}_613x613.jpg`;
    ...
    return `
            <div class="swiper-slide" data-prodname="${Name}" data-prodUrl="${productUrl}">
                <div class="${id}__prodcard">
                    <div class="${id}__prodcard--image-container">
                        <a href="${productUrl}" class="${id}__img--wrapper">
                            <img src="${imageToUse}" alt="${Name}" />
                        </a>
                    ...
                    <h1 class="${id}__prodcard--title"><a href="${productUrl}">${Name}</a></h1>
      ...
      '\n'
                    ...
                    <div class="quantity_wrapper">
                        ...
                        <div class="add-to-cart ${
                          Availability === 1 && Variants[0].Availability === 1 ? '' : 'out-of-stock'
                        }" data-sku="${Variants[0].Sku}">ADD TO BASKET</div>
                    </div>
                    </div>
                </div>
            </div>`;
  ...
  parentElm.insertAdjacentHTML(
    'beforeend',
    ` <div class="${id}__prodcards"><div class="swiper-wrapper">${cards}</div><div class="${id}__swiper-button-prev"></div>
      <div class="${id}__swiper-button-next"></div></div>`
  );
  ...
  document.querySelector(`.${id}__prodcards`).addEventListener('click', (e) => {
    ...
    const swiperArrows = document.querySelectorAll(`[class^="${id}__swiper-button-"`);
    if (targetMatched(`.${id}__plus-btn`)) {
      ...
      swiperArrows.forEach((item) => item.classList.add(`${id}__remove-z`));
      addToCart(target.getAttribute('data-sku'), parseInt(inputBox.value)).then((res) => {
        ...
        setTimeout(() => {
          ...
          swiperArrows.forEach((item) => item.classList.remove(`${id}__remove-z`));
          console.log(res);
      ...
      const prodName = card.dataset.prodname;
      ...
      fireEvent(`user ${operation}s ${prodName} ${operation === 'add' ? 'to' : 'from'} the wishlisht`);
      fireEvent(`User interacts with wishlist icon in the experiment`);
    ...
    if (targetMatched(`.${id}__img--wrapper`) || targetMatched(`.${id}__prodcard--title`)) {
      ...
      fireEvent(`User interacts with the product ${prodName} and is taken to the PDP`);
    }
  });
};
