...
export default () => {
  const { ID, $, rootScope, cartScope } = shared;
  ...
  const editTitle = () => {
    const $titleWrapper = $productsSectionHeading.children('.title');
    const $titleIconClosed = $productsSectionHeading.children('.sectionicon[ng-show="!showItems"]');
    const $titleIconOpened = $productsSectionHeading.children('.sectionicon[ng-show="showItems"]');
    const titleLabel = translate('Your Shopping Bag');
    ...
    const $basketTitle = $(`
      <div ng-show="showItems">
        <div class="lineHeightSmall">${titleLabel}</div>
      </div>
      <div ng-show="!showItems">
        <div class="lineHeightSmall">${titleLabel}:</div>
        <span class="${ID}_titleCount lineHeightSmall">
        {{ CurrencyFilter( BasketItems.TotalPrice, Locale.NUMBER_FORMATS.CURRENCY_SYM, 2) }} (<span ng-bind="BasketItems.TotalCount"></span> ${translate('items')})
        </span>
      </div>
    `);

    // Toggle label - add above the icon.
    const $titleIconClosedLabel = $(`
      <div class="${ID}_toggleBag cursor-pointer" ng-show="!showItems">
        <div class="${ID}_toggleBagLabel lineHeightSmall">${translate('Open Bag')}</div>
        <span class="${ID}_toggleBagIcon lineHeightSmall">+</span>
      </div>
    `);
    const $titleIconOpenedLabel = $(`
      <div class="${ID}_toggleBag cursor-pointer" ng-show="showItems">
        <div class="${ID}_toggleBagLabel lineHeightSmall">${translate('Close Bag')}</div>
        <span class="${ID}_toggleBagIcon lineHeightSmall">-</span>
      </div>
    `);

    // Make dom node moves.
    $titleIconClosed.prepend($titleIconClosedLabel);
    $titleIconOpened.prepend($titleIconOpenedLabel);
    $titleWrapper.html($basketTitle);
    
    // Recompile.
    angularCompile($basketTitle, $, checkoutScope);
    angularCompile($titleIconClosedLabel, $, checkoutScope);
    angularCompile($titleIconOpenedLabel, $, checkoutScope);
  ...
  /**
   * Show total basket price when shipping is selected, but before payment is.
   */
  const showTotalAfterShippingSelection = () => {
    const $priceElement = $(`
      <div class="placeOrderSection" ng-if="VM.SelectedShippingOption && !VM.SelectedPaymentOption">
        <div class="buttons-total">
          <span>${translate('Total')}:</span>
          <div class="bold">{{ CurrencyFilter( CartSummary.TotalPrice, Locale.NUMBER_FORMATS.CURRENCY_SYM, 2) }}</div>
        </div>
      </div>
    `);

    // Inject after payment option.
    $('#summaryBox').after($priceElement);
    angularCompile($priceElement, $, checkoutScope);
  }
  ...
  showTotalAfterShippingSelection();
};
