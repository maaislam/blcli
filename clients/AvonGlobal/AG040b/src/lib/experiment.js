...
export default () => {
  ...
  if(VARIATION != 'control') {
     ...
     */
    const createMinibag = (shouldShowExtraHtml = false) => {
      const minibag = new Minibag(shouldShowExtraHtml);
    };
    ...
    rootScope.$on('CartService_AddSuccess', () => {
      ...
      const isPagePDP = window.location.href.indexOf('/product/') !== -1;
      if (isPagePDP) {
        const { Breadcrumbs } = rootScope.ShopContext;
        let categories = [];
        const salePage = {
          Url: 'https://my.avon.co.za/special-offers/',
          Text: 'Sale',
        };
      ...
      createMinibag(isPagePDP ? true : false);
    });
  ...
  */
  ...
  if (INCREASINGLY_SPLIT_TEST && shared.VARIATION != 'control') {
    const $body = $('body');
