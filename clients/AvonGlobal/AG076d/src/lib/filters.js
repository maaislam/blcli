...
`;
  ...
  const toggleButtons = () => {
    ...
    $(`.${ID}_sortButton`).click(function (e) {
      if (isToggleOpen($(this))) hideToggles();
        ...
        fireEvent("Click - sortby open");
      }
    });
  };

  const rearrangeHeader = () => {
    const $wrapper = $("#BreadcrumbBar");
    const $socials = $(".DashboardSocialIcons");

    $headerWrapper.append($socials).append($buttonsWrapper);
  ...
  const updateRatings = () => {
    ...
    $(`.${ID}_ratings .Star`).click(function () {
      ...
      fireEvent(`Click - rating filter (${star})`);
    });
  };
      ...
      if (!$sortScope.hideToggles) {
        $sortScope.$apply(() => {
          ...
          $sortScope.trackClicks = (label) => {
            fireEvent(`Click - sortby selected (${label})`);
          };
        });
      }
