...
export default () => {
  ...
  const init = (data) => {
    ...
    console.log(document.querySelector('[data-modulename="M - Main banner"]'));
    console.log(testAnchorElm);
        ...
        isFavorite().then((favResult) => {
          ...
          getProductDetails(groupIds).then((res) => {
            ...
            const dataPlusImage = res.Data.map((item) => {
              const filteredData = data.slots.filter(
                (singleProd) => singleProd.item['group_id'] == item.Id && singleProd.item['in_stock']
              )[0];
            ...
            const staticIds = ['108834', '116258', '112400', '23731', '116260', '53430', '112241']; // for sorting
            ...
            setTimeout(() => {
              ...
              renderProdlist(fireEvent, sortedArr, favResult, categoryShowcase, ID);
