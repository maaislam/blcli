    ...
    if (customerSavings) {
      const markup = `
        <div class="${shared.ID}__savings">
          <img class="${shared.ID}__savings__img" src="https://service.maxymiser.net/cm/images-eu/avon-mas/6A83DEEB3A12909B4E3D316D4DE3325746B586D54F9A0B7A1560F1F59C43C9A8.png?meta=/AG061b---Checkout-promotion-cards/ag061-icon.png" />
          <p class="${shared.ID}__savings__text">You are saving <span class="${shared.ID}__savings__text--b">${customerSavings}</span> by shopping today</p>
        </div>
      `;
...
const addLoginMarkup = (savings) => {
  ...
  if (isGuest === 'showGuest') {
    ...
    formHeaderSpan.innerHTML =  `Login and save <span class="${shared.ID}__bold">£${savings}</span> today!`;
  }
}
