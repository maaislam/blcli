...
export default class PDPCtas {
  ...
  create() {
    ...
    const $component = $(`
      <div class="${componentName}">
        ...
        <div class="Button button-add-to-cart vi-btn vi-btn--primary ng-scope ${componentName}AddToCart" ng-class="{'button-add-to-cart-disabled': !hasSelectedVariant()}">
          <div class="button-add-to-cart-content button-add-to-cart-text text-not-selectable">
            <svg-icon icon="basket_pdp" class="ng-isolate-scope">
              <!-- ngIf: Ready --><svg ng-if="Ready" class="" style=""><use xlink:href="#Svg_basket_pdp" ng-attr-xlink:href="{{TrustedSvgHref()}}"></use></svg><!-- end ngIf: Ready -->
            </svg-icon>
              <img class="${componentName}ButtonTickIcon" src="https://service.maxymiser.net/cm/images-eu/avon-mas/A1308CC55218878E7DC3FCDA6EE7BAE71F11705E96221B27CEB23F506CC01C9B.svg?meta=/AV069---Hero-PDPs/tick.svg" alt="tick" />
              <span class="${componentName}ButtonTextInitial">přidat do košíku</span>
              <span class="${componentName}ButtonTextActive">přidal</span>
              <span class="${componentName}ButtonTextActiveDesktop">přidat do košíku</span>
             </div>
        </div>
      </div>
    `);
  ...
  // Change button state to confirm user action (add to cart) for 3s
  toggleButtonState($btn) {
    $btn.addClass(`${this.componentName}Active`);
    setTimeout(() => {
      $btn.removeClass(`${this.componentName}Active`);
    }, 3000);
  }

  bindEvents() {
    const { $component, componentName, addToCartScope, getQuantity } = this;
    ...
    const addToCart = () => {
      addToCartScope.addToCart(getQuantity());
      this.toggleButtonState($addToCart);
      fireEvent("sticky-add-to-cart-clicked");
    };
