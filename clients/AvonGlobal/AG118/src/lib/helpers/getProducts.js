const getStrategyData = (strategyId = 204430, prodData = [], numberOfProducts = 20) => {
  const styles = {
  ...
  return fetch(`https://rcom-eu.dynamicyield.com/v3/recommend/${window.DY.section}`, {
    ...
    .then((data) => {
      ...
      return fetch(
        `/api/productsapi/getproducts?language=${Language}&campaignNumber=${campaignNumber}&productIds=${productGroupIds.join()}`
      )
        .then((response) => response.json())
        .then(({ Data }) => {
