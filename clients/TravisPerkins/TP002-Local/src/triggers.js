...
(function($) {
  ...
  var loggedin = bodyVar.querySelector('.nav .button_text .sessioncamhidetext');
  if(loggedin){
      ...
      productPrice.append(toggle);
       ...
       $('#tp2-togglecheckbox').change(function () {
        ...
        // Check if product per price exists, move if it does
        let productPerPrice = bodyVar.querySelector('.productEQPrice');
        if (productPerPrice) {
          bodyVar.querySelector('.tp2-toggle').insertAdjacentElement('afterend', productPerPrice);
        }
