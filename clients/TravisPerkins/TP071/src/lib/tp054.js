    ...
    var _TP054 = (function () {
        ...
        function sendEvent(e,n,a,r,t,o){var c=function(c){var i={};i.nonInteraction=r,t&&o&&(i["dimension"+t]=o),window.ga(c+".send","event",e,n,a,i)};trackerName?c(trackerName):UC.poller([function(){return window.ga.getAll}],function(){trackerName=window.ga.getAll()[0].get("name"),c(trackerName)})}var trackerName;
         ...
         // -----------------------------------------------
            ...
            // Variation
            function TP054() {
              // Prevent experiment from triggering twice on retrack 
                  var body = document.getElementsByTagName('body')[0];
                  if (body.className.match('TP054')) return false;
                ...
                var run = function() {
                    ...
                    if (products) {
                        products.each(function () {
                            if(!$(this).find('.tp54-techlink').length) {
                        ...
                        products.each(function () {
                            ...
                            var showTechSpec;
                            showTechlink.off('click').on('click', function (e) {
                ...
                run(); // test run on page load

                // -----------------------------------------------
                // On add to basket complete re-run tech specs
                // -----------------------------------------------
                $(document).off('ajaxComplete.tp054').on('ajaxComplete.tp054', function(e, request, settings) {
                    if(settings && settings.url) {
                        if(settings.url.match(/^\/cart\/addForCollection\/?$/g)
                                || settings.url.match(/^\/cart\/add\/?$/g)) 
                        {
                            // Test run on add to basket
                            run();
                        }
                    }
                });
