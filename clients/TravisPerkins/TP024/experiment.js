var experiment = {
	...
	            GA: function() {
					...
					// Click events
					$('#branches_with_stock').one('click', '.TP024_clickAndCollect', function() {
						if (!status.clickAndCollectClicked) {
							sendEvent('TP024 - Mobile', 'Click', 'Clicked add for collection', true);
					...
					$('#stockCheckerButton').one('click', function() {
						if (!status.checkStockClicked) {
							sendEvent('TP024 - Mobile', 'Click', 'Clicked check stock', true);
	...
	                        modules.clickAndCollect.init(store);
							if (!analytics.eventStatus.clickAndCollectShown) {
								analytics.sendEvent('TP024 - Mobile', 'View', 'Add for collection shown', true);
