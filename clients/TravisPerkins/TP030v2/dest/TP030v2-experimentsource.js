var _TP030v2 = (function () {
    ...
    var events = {
        category: 'TP030v2',
        status: {
            clickedDeliveryTerms: false
        },
        send: {
            clickedDeliveryTerms: function () {
                if (!events.status.clickedDeliveryTerms) {
                    window.ga('send', 'event', 'TP030v2', 'click', 'User clicked delivery terms', {nonInteraction: true});
                    events.status.clickedDeliveryTerms = true;
                }
            }
        }
    };
    ...
    function activate() {
        ...
        var $body = $('body');
        var $window = $(window);
        $body.addClass('TP030v2');

        var $convenientDelivery = $([
            '<div class="TP030v2_deliveryWrapper">',
            '<div class="TP030v2_imageWrapper">',
            '</div>',
            '<h1 class="TP030v2_freeDelivery">FREE UK Delivery*</h1>',
            '<p class="TP030v2_deliveryMessage">Your local branch will call you to arrange a convenient ' +
            'time slot for delivering your order <a href="/delivery">Read delivery terms</a></p>',
            '</div>'
        ].join(''));

        // Create and add the structure of the convenient delivery message after the top navigation bar
        var $headerButtons = $body.find('#header');
        $convenientDelivery.insertAfter($headerButtons);

        if(!$body.hasClass('page-mobile-homepage')) {
            $body.find('.TP030v2_deliveryWrapper').css("margin-top", "68px");
            $body.find('#content').css("margin-top", "0");
        }

        // Track when user wants to read delivery terms
        var $readDeliveryTerms = $body.find('.TP030v2_deliveryMessage a');
        $readDeliveryTerms.on('click', events.send.clickedDeliveryTerms);
