...
import tradepriceBadge from './components/tradePriceBadge';
...
const init = () => {
  ...
  if (isPDP() && skuValidity(tradeSkuPrice) && postcodeMatch(postcodes)) {
    ...
    if (!document.querySelector(`.${ID}__newprice`) && isLoggedIn) {
      ...
      const perItemText = priceAnchor.querySelector('span').innerText || 'EACH';
      VARIATION != 2 && priceAnchor.insertAdjacentHTML('beforeend', newPrice(ID, tradeSkuPrice[extractSkuNumber], perItemText));
    }
    ...
    prodCards.forEach((card) => {
      ...
      if (skuValidity(tradeSkuPrice, cardSku)) {
        ...
        if (!card.querySelector(`.${ID}__tradeprice`)) {
          const badgeAnchor = isMobile
            ? card.querySelector('[class^="ProductImageComponent__ImageWrapper-sc"]')
            : card.querySelector('[data-test-id="product-card-image"]');
          const priceAnchor = card.querySelector('[data-test-id="main-price"]');
          if (!card.querySelector(`.${ID}__tradeprice-badge`)) {
            badgeAnchor.classList.add(`${ID}__badge-container`);
            VARIATION != 2 && badgeAnchor.insertAdjacentHTML('afterbegin', tradepriceBadge(ID, 'plp'));
          }
          if (!card.querySelector(`.${ID}__newprice`) && isLoggedIn) {
            priceAnchor.classList.add(`${ID}__newprice-anchor`);
            const perItemText = priceAnchor.querySelector('span').innerText || 'EACH';
            VARIATION != 2 && priceAnchor.insertAdjacentHTML('beforeend', newPrice(ID, tradeSkuPrice[cardSku], perItemText));
          }
  ...
  const controlInit = () => {
    if (isPDP() && skuValidity(tradeSkuPrice) && postcodeMatch(postcodes)) {
      ...
      pollerLite(['[data-test-id="plp-list"]'], () => {
        ...
        const prodCards = document.querySelector()?.querySelectorAll('[data-test-id="product"]');
        prodCards.forEach((card) => {
          const cardSku = card.querySelector('[data-test-id="product-card-code"]').innerText.split(': ')[1];
