...
const tradePriceMsg = (id, loginStatus, tradePrice, anchorElem, fireEvent, pageType) => {
  ...
  const config = {
    login: {
      ...
      msgLine1: ``,
      msgLine2: 'YOUR TRADE PRICE IS UNLOCKED',
      specialPrice: ``,
    ...
    notLogin: {
      ...
      msgLine1: '',
      msgLine2: ' UNLOCK TRADE PRICE OF',
  ...
  const adjustedWidth = (data) => {
    ...
    return pageType === 'plp' && data.specialPrice.length >= 5 ? 'style="width:calc(100% + 10px)"' : '';
  };
  const htmlStr = (data) => `
    
    <div ${adjustedWidth(data)} class="${id}__tradeprice ${loginStatus ? `${id}__tradeprice-margin` : ''} ${
    pageType === 'plp' ? `${id}__plp-changes` : ''
        ...
        <div class="${id}__loginstatus--msg ">
            <div class="${data.msgClass}">
                <div class="${pageType === 'plp' ? `${id}__hide` : ''}">${data.msgLine1}&nbsp;</div>
                <div>${data.msgLine2}&nbsp;<span>${data.specialPrice ? formatPrice(data.specialPrice) : ''}</span></div>
            </div>
  ...
  eventAnchor.addEventListener('click', (e) => {
    e.preventDefault();
    fireEvent(`Clicked message with price on ${pageType === 'plp' ? 'PLP' : 'PDP'}`);
