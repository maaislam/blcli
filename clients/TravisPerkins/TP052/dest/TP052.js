// ----------------------------------------------------
// TP Suggestion Tool - for PLP pages
// ----------------------------------------------------
var TP052 = (function() {
    // ----------------------------------------------------
    // Prevent test running twice Monetate set up issue
    // ----------------------------------------------------
    if(document.body.classList.contains('tp52')) {
        return;
    }

    // ----------------------------------------------------
    // UC library poller
    // ----------------------------------------------------
    var UC=function(a){return a.poller=function(a,b,c){var d={wait:50,multiplier:0,timeout:6000},e=Date.now||function(){return(new Date).getTime()};if(c)for(var f in c)d[f]=c[f];else c=d;for(var g=!!d.timeout&&new Date(e()+d.timeout),h=d.wait,i=d.multiplier,j=[],l=function(c,d){if(g&&e()>g)return!1;d=d||h,function(){var a=typeof c;return"function"===a?c():"string"!==a||document.querySelector(c)}()?(j.push(!0),j.length===a.length&&b()):setTimeout(function(){l(c,d*i)},d)},m=0;m<a.length;m++)l(a[m])},a}(UC||{});

    // ----------------------------------------------------
    // Helper send events
    // ----------------------------------------------------
    var trackerName;
    function sendEvent(action, label, dimensionValue, dimensionName) {

        var category = 'TP052---Suggestion-Tool';
        var nonInteractionValue = true;

        var fire = function (tracker) {
            var options = {};
            options.nonInteraction = nonInteractionValue;
            if(dimensionValue && dimensionName){
                options['dimension' + dimensionValue] = dimensionName;
            }
            window.ga(tracker + '.send', 'event', category, action, label, options);
        };

        if (trackerName) {
            fire(trackerName);
        } else {
            UC.poller([
                function () {
                    return window.ga.getAll;
                }
            ], function () {
                trackerName = window.ga.getAll()[0].get('name');
                fire(trackerName);
            });
        }
    }

    // ----------------------------------------------------
    // Full Story Integration
    // ----------------------------------------------------
    UC.poller([
     function() {
         var fs = window.FS;
         if (fs && fs.setUserVars) return true;
     }
    ], function () {
     window.FS.setUserVars({
         experiment_str: 'TP052-Suggestion-Tool',
         variation_str: 'Variation 1'
     });
    }, { multiplier: 1.2, timeout: 0 });

    // Setup
    document.body.classList.add('tp52');

    // ------------------------------------------------------------------------
    // Poll required elements
    // ------------------------------------------------------------------------
    UC.poller([
        function () {
            if (window.jQuery) {
                return true;
            }
        }
    ], function() {
        run();
    });

    // ------------------------------------------------------------------------
    // Entry point for test
    // ------------------------------------------------------------------------
    function run() {
        var associatedProductsRetrievalInProgress = false;
        var numAddToBasketsInLightbox = 0;

        // Sku
        var sku = null;

        // ------------------------------------------------------------------------
        // We need a valid csrf token to run - bail out
        // ------------------------------------------------------------------------
        if(!$('[name=CSRFToken]').val()) {
            return;
        }

        /**
         * Helper for adding an upsell product to cart
         * Ajax request depending on 
         */
        function addRelatedProductToCart(type, sku, qty) {
            var targetUrl = null;
            var storeId = null;
            switch(type) {
                case 'delivery':
                    targetUrl = '/cart/add';
                    break;
                case 'collection':
                    targetUrl = '/cart/addForCollection';
                    storeId = $('[name=storeId]').val();
                    break;
            }

            var csrf = $('[name=CSRFToken]').val(); // Grab any existing CSRF token from the page

            var data = {
                productCodePost: sku,
                qty: qty || 1,
                CSRFToken: csrf
            };

            if(storeId) {
                data.storeId = storeId;
            }

            var request = $.ajax({
              url: targetUrl,
              type: 'post',
              dataType: 'json',
              data: data
            });

            return request;
        }

        /**
         * Get the associated products from our API
         */
        function getAssociatedProducts(forSku) {
            var request = $.ajax({
               url: 'https://ab-test-sandbox.userconversion.com/custom-client-scripts/tp-suggestion-tool/', 
               type: 'get',
               dataType: 'json',
               data: {
                 'sku': forSku
               },
            });

            return request;
        }

        /**
         * Trigger listen for add to basket and then show associated products
         */
        function triggerListenForAddToBasket() {
            switchOffListenForAddToBasket();
            switchOnListenForAddToBasket();
        }

        /**
         * Unbind ajax complete add to basket
         */
        function switchOffListenForAddToBasket() {
            $(document).off('ajaxComplete.tpsuggestiontool');
        }

        /**
         * Bind ajax complete add to basket
         */
        function switchOnListenForAddToBasket() {
            $(document).on('ajaxComplete.tpsuggestiontool', function(e, request, settings) {
                if(settings && settings.url) {
                    if(settings.url.match(/^\/cart\/addForCollection\/?$/g)) {
                        retrieveAndShowAssociatedProducts('collection');
                    }
                    if(settings.url.match(/^\/cart\/add\/?$/g)) {
                        retrieveAndShowAssociatedProducts('delivery');
                    }
                }
            });
        }

        /**
         * Helper Create lightbox
         *
         * Destroy and re-create lightbox
         */
        function createLightbox() {
           $('.tp52-lightbox').remove();

           var lightbox = $([
            '<div class="tp52-lightbox">',
                '<div>',
                    '<a class="tp52-close_btn">X</a>',
                    '<div class="tp52-lightbox__content tp52-overflow_fix">',
                        '<p class="tp52-added-to-basket"><i class="fa fa-check"></i> Added to Basket</p>',
                        '<p class="tp52-lightbox__preloader">',
                            '<img src="data:image/gif;base64," />',
                        '</p>',
                    '</div>',
                '</div>',
            '</div>'
           ].join(''));

           $('body').prepend(lightbox);
               
           // Initialise lightbox
           var modal = $(".tp52-lightbox");

           // Show lightbox
           function showLightbox() {
               $('body').addClass('tp52-popup-active');
               modal.addClass('active');
               modal.fadeIn("slow", function() {
                 modal.addClass("active");
               });

               // Prevent listen for add to basket
               switchOffListenForAddToBasket();
           }

           showLightbox();

           function hideLightbox() {
               $('body').removeClass('tp52-popup-active');

               modal.fadeOut("slow", function() {
                 modal.removeClass("active");
               });
               
               // Re-listen for add to basket
               switchOffListenForAddToBasket();

               // If number of times added to basket > 0, refresh page to update mini basket
               if(numAddToBasketsInLightbox) {
                   window.location.reload();
               }
           }

           $(".tp52-lightbox .tp52-close_btn").on("click", function(e) {
               e.preventDefault();

               hideLightbox();
           });

           $(document).off('mousedown.tp52-modal');
           $(document).on("mousedown.tp52-modal", function(e) {
             if (!$(e.target).closest(".tp52-lightbox > div").length) {
               if (modal.hasClass("active")) {
                   hideLightbox();
               }
             }
           });
        }

        /**
         * Retrieve and show associated products in lightbox
         */
        function retrieveAndShowAssociatedProducts(type) {
           if(!sku) {
               return;
           }
           if(associatedProductsRetrievalInProgress) {
              return;
           }
           associatedProductsRetrievalInProgress = true;

           // Get associated products and display in lightbox
           getAssociatedProducts(sku).success(function(associatedProducts) {
               sendEvent('did-return-products-did-show-lightbox', sku);

               // Create lightbox
               createLightbox();

               // -- Hide preloader
               $('.tp52-lightbox__preloader').remove();
               
               // -- Create tabs
               var tabNames = Object.keys(associatedProducts);

               var tabs = $([
                '<p class="tp52-tabs-title">Recommended Products</p>',
                '<div class="tp52-tabs">',
                    '<div class="tp52-tabs__headings">',
                    '</div>',
                    '<div class="tp52-tabs__content">',
                    '</div>',
                '</div>'
               ].join(''));

               $('.tp52-lightbox__content').append(tabs);

               $.each(tabNames, function(idx, item) {
                   var identifier = generateIdentifierFromString(item);

                   var tabHeading = $([
                       '<div class="tp52-tabs__tab" data-identifier="' + identifier + '">',
                           item,
                       '</div>'
                   ].join(''));

                   $('.tp52-tabs__headings').append(tabHeading);

                   var tabContentItem = $([
                       '<div class="tp52-tabs__content-item clearfix" data-identifier="' + identifier + '">',
                       '</div>'
                   ].join(''));

                   $.each(associatedProducts[item], function(idx, val) {
                       tabContentItem.append([
                           '<div class="tp52-tabs__product clearfix">',
                             '<form>',
                               '<div class="tp52-tabs__product-inner">',
                                   '<div class="tp52-tabs__product-main clearfix">',
                                       '<img src="' + val.product_image_url + '" />',
                                       '<span class="tp52-tabs__product-title">',
                                            '<strong>' + val.title + '</strong>',
                                            '<em>' + (val.sku ? ('(' + val.sku + ')') : '') + '</em>',
                                       '</span>',
                                   '</div>',
                                   '<div class="tp52-tabs__product-addto clearfix">',
                                       '<div class="tp52-tabs__product-quantity">',
                                           '<label>Qty</label>',
                                           '<input class="tp52-tabs__product-quantity-input" min="1" required type="number" value="1">',
                                       '</div>',
                                       '<div class="tp52-tabs__product-price">',
                                           '<span class="tp52-tabs__product-price-price">£' 
                                                + formatMoney(val.price) + ' <em>(inc VAT)</em></span>',
                                           '<span class="tp52-tabs__product-unit">' + val.unit + '</span>',
                                       '</div>',
                                       '<div class="tp52-tabs__product-addto-button">',
                                           '<button '
                                                + ' data-sku="' + val.sku + '" '
                                                + ' data-type="' + type + '" '
                                                + '>',
                                               'Add to basket',
                                           '</button>',
                                       '</div>',
                                   '</div>',
                               '</div>',
                             '</form>',
                           '</div>'
                       ].join(''));
                   });

                   $('.tp52-tabs__content').append(tabContentItem);

               });

               $('.tp52-tabs__tab:first').addClass('tp52-tabs__tab--active');
               $('.tp52-tabs__content-item:first').addClass('tp52-tabs__content-item--active');

               associatedProductsRetrievalInProgress = false;

               // Tabs UI
               $('.tp52-tabs__tab').on('click', function() {
                   $('.tp52-tabs__tab--active').removeClass('tp52-tabs__tab--active');
                   $(this).addClass('tp52-tabs__tab--active');

                   var identifier = $(this).attr('data-identifier');
                   $('.tp52-tabs__content-item').removeClass('tp52-tabs__content-item--active');
                   $('.tp52-tabs__content-item[data-identifier=' + identifier + ']').addClass('tp52-tabs__content-item--active');
               });

               // Add links
               $('.tp52-lightbox__content').append([
                '<div class="tp52-bottom-links">',
                    '<a class="tp52-bottom-links--view-basket" href="/cart">View Basket</a>',
                    '<a class="tp52-bottom-links--checkout" href="/checkout/guest/delivery_details">Checkout</a>',
                '</div>'
               ].join(''));

               // Events
               $('.tp52-bottom-links--view-basket').on('click', function() {
                   sendEvent('did-click-view-basket-link-from-lightbox', sku);
               });
               $('.tp52-bottom-links--checkout').on('click', function() {
                   sendEvent('did-click-checkout-link-from-lightbox', sku);
               });

               $('.tp52-tabs__product-addto-button button').on('click', function() {
                   sendEvent('clicked-add-to-basket', $(this).attr('data-sku'));
               });

               // -- Add to Basket
               $('.tp52-tabs__product-addto-button button').on('click', function() {
                   var productContainer = $(this).parents('.tp52-tabs__product:first'), 
                       qtyInput = productContainer.find('.tp52-tabs__product-quantity-input'),
                       qty = qtyInput.val(),
                       sku = $(this).attr('data-sku'),
                       type = $(this).attr('data-type');

                   // Match heights of products
                   var currentHeight = productContainer.find('.tp52-tabs__product-inner').outerHeight();
                   productContainer.find('.tp52-tabs__product-inner').height(currentHeight);

                   // Show preloader
                   productContainer.find('.tp52-tabs__product-inner').empty().append(
                    '<img class="tp52-tabs__product-preloader" src="data:image/gif;base64," />'
                   );

                   // Add product to cart
                   addRelatedProductToCart(type, sku, qty).success(function() {
                       productContainer.find('.tp52-tabs__product-inner').empty().append([
                        '<p class="tp52-tabs__product-success">',
                            '<i class="fa fa-check"></i>',
                            ' Added to basket',
                        '</p>'
                       ].join(''));

                       numAddToBasketsInLightbox++;
                   });

                   return false;
               });

           }).error(function() {
               $('.tp52-lightbox__preloader').remove();

               associatedProductsRetrievalInProgress = false;

               sendEvent('error--unable-to-retrieve-products', sku);
           });
        }

        /**
         * Helper generate identifier from string
         */
        function generateIdentifierFromString(string) {
            var identifier = string.replace(/[^\w\d]/gi, '').toLowerCase().trim();

            return identifier;
        }

        /**
         * Helper format money
         * 
         * @see https://stackoverflow.com/questions/149055/how-can-i-format-numbers-as-dollars-currency-string-in-javascript
         */
        function formatMoney(n, c, d, t){
             var c = isNaN(c = Math.abs(c)) ? 2 : c, 
                 d = d == undefined ? "." : d, 
                 t = t == undefined ? "," : t, 
                 s = n < 0 ? "-" : "", 
                 i = String(parseInt(n = Math.abs(Number(n) || 0).toFixed(c))), 
                 j = (j = i.length) > 3 ? j % 3 : 0;
             return s + (j ? i.substr(0, j) + t : "") + i.substr(j).replace(/(\d{3})(?=\d)/g, "$1" + t) + (c ? d + Math.abs(n - i).toFixed(c).slice(2) : "");
         }
        
        // ---------------------------------------------
        // ON ADD TO BASKET
        //
        // ---------------
        // Add to Delivery
        // ---------------
        //
        // - If delivery postcode is already chosen, it will be an asynchronous request
        // --> Listen for ajax complete and match URL requested
        // - If no delivery postcode entered, we await a page refresh
        // --> Look for mini basket appearing check if hidden 
        //
        // ---------------
        // For Collection
        // ---------------
        //
        // - Asynchronous request 
        // --> Listen for ajax complete
        //
        // ---------------------------------------------
        
        // -- Click event handlers on buttons, trigger listen 
        $('#content').on('click', '.add_to_cart_button', function() {
            sku = $(this).parents('form:first').find('[name=productCodePost]').val();

            if($(this).hasClass('postCodePresent')) {
                // Postcode already entered, listen for ajax complete
                triggerListenForAddToBasket();
            } else {
                // Flag did intend to buy with delivery
                localStorage.setItem('tp--suggestiontool--did-init-delivery', 1);
            }
        });

        // -- Add for collect button
        $('#content').on('click', '.addForCollectButton', function() {
            sku = $(this).parents('form:first').find('[name=productCodePost]').val();

            // Listen for cart add event
            triggerListenForAddToBasket();
        });

        // -- If postcode not already entered
        var deliveryIntentTimeout;
        function deliveryIntent() {
            clearTimeout(deliveryIntentTimeout);
            deliveryIntentTimeout = setTimeout(function() {
                var isHidden = $('#cart_popup').is(':hidden');
                if(!isHidden && localStorage.getItem('tp--suggestiontool--did-init-delivery')) {
                    localStorage.removeItem('tp--suggestiontool--did-init-delivery');

                    retrieveAndShowAssociatedProducts('delivery');
                } else {
                    deliveryIntent();
                }
            }, 200);
        }
        deliveryIntent();
    }
})();
