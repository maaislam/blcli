export default function tp040() {
    var _TP040 = (function() {
        ...
        function activate() {
            ...
            var $addProduct = $('#cart_popup');

            $('body').addClass('TP040');

          /*  var $checkout = $([
                '<a href="/checkout/guest/login" id="TP040_button--checkout">',
                    //'Go to Checkout',
                 '</a>'
            ].join('')); */
          ...
          // Amend finished
            ...
            function addCheckoutBtn() {
                $checkout.prependTo($miniBasket.find('.links '));
                 ...
                 $checkout.one('click', function() {
                    sendEvent('TP040', 'Click', 'User clicked checkout btn', {nonInteraction: 1});
                });
            ...
            function addCheckoutBtntoaddProduct() {
              if($addProduct.length){
                $checkout.prependTo($addProduct.find('.links '));
                
                $checkout.one('click', function() {
                    sendEvent('TP040', 'Click', 'User clicked checkout btn after adding product', {nonInteraction: 1});
                });
            }
            }

            UC.observer.connect($addProduct, addCheckoutBtntoaddProduct, {
                config: {attributes: false, childList: true, subtree: false},
                throttle: 100
            });
