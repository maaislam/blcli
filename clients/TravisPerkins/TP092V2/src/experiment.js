...
const experiment = {
    ...
    $('#TP077B_PostcodeSearch').on('keyup', function() {
      var parent = $(this).parent();
      if (parent.hasClass('TP092_no-input')) {
        parent.removeClass('TP092_no-input');
      }
    });
		...
		$('.TP077B_SearchButton').on('click', function(){
			...
			if(val){
				stockInput.val(val);
				stockSubmit.click();
        $('body').addClass('TP092_showloader');
        $(this).parent().removeClass('TP092_no-input');

				experiment.utils.poller([
					'#branches',
				], function(){
          //$('#collectionBranchLocatorButton').click();
          $('body').removeClass('TP092_showloader');
        ...
        $(this).parent().addClass('TP092_no-input');
      }
		});
		...
		*/ 
		...
		body.className += ' ' + 'TP024 TP092 TP077B'; // Namespace CSS
	...
	restructure: function(){
		...
		'#branches:not(.TP092_built) > li:not(.TP092_stock-row)',
          ...
          var wrap = el.closest('.TP092_new-stock');
          var wrapNext = wrap.next();
          var nextFind = wrapNext.find('.TP024_clickAndCollect');

          if (nextFind.length === 0) {
            experiment.modules.clickAndCollect.init(el.closest('#branches').find('> li'));
          } 
