...
import { prodIds } from './data/tradeProductList';
...
const init = () => {
  ...
  pollerLite(
    ...
    () => {
      ...
      getProducts(prodIds).then((result) => {
        const products = result.tpplcProducts;
        attachpoint.insertAdjacentHTML('beforebegin', tradeContainer(ID, products));
