...
export default function tp036002() {
    ...
    var TP036 = function() {
        ...
        UC.poller([
            ...
            var shouldShowAttributes = true; 
            if(!length && breadcrumbSegmentExists('Decking')) {
              $('body').removeClass('tp036');
              return; // Amend: ignore products with options from test - length must be set as a tech attribute on page
              
                //shouldShowAttributes = false; // We can get the length from the dropdown, but don't show it in UI
    ...
    (function() {
        ...
        UC.poller([
            ...
            '.TP002 .tpInfoWrapper .productPrice .price_info_holder',
            function () {
                if (window.ga) return true;
            }
        ], function(){
            if($('.TP002 .price_inc_vat_section .promotion_message').length == 0){
                $('.price_info_holder.uom_value + .promotion_message').clone().appendTo($('.TP002 .price_inc_vat_section'));
            }
            if($('.TP002 .product_price_section .promotion_message').length == 0){
                $('.price_info_holder.uom_value + .promotion_message').clone().appendTo($('.TP002 .product_price_section'));
            }
        }, {
            timeout: 8000,
            multiplier: 0
        ...
        var loggedin = $('.nav .button_text .sessioncamhidetext');
        if(loggedin.length < 0){

            $('body').addClass('tp20noToggle');
            $('.tpInfoWrapper .product_price_section').addClass('tp2-loggedIn');
          
          }else{
               
            $('body').removeClass('tp20noToggle');
            ...
            /*Buttons*/
            $('#tpPdpRightPanelComponent .ccButton').insertAfter('.button > .add_to_cart_form'); // Amend 25/9/17
        ...
        // Call TP036 now that we're done
        TP036();
