var _TP030 = (function () {
        ...
        send: {
            clickedDeliveryTerms: function () {
                if (!events.status.clickedDeliveryTerms) {
                    window.ga('send', 'event', 'TP030', 'click', 'User clicked delivery terms', {nonInteraction: true});
    ...
    function activate() {
        ...
        var $convenientDelivery = $([
            '<div class="TP030_deliveryWrapper">',
            '<div class="TP030_imageWrapper">',
            '</div>',
            '<h1 class="TP030_freeDelivery">FREE UK Delivery*</h1>',
            '<p class="TP030_deliveryMessage">Your local branch will call you to arrange a convenient ' +
            'time slot for delivering your order <br /><a href="/delivery">Read delivery terms</a></p>',
            '</div>'
        ].join(''));

        // Create and add the structure of the convenient delivery message after the top navigation bar
        var $headerButtons = $body.find('.header_buttons');
        $convenientDelivery.insertAfter($headerButtons);

        // Track when user wants to read delivery terms
        var $readDeliveryTerms = $body.find('.TP030_deliveryMessage a');
