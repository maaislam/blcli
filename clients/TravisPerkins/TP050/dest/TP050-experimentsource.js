...
function sendEvent(e,n,a,r,t,o){var c=function(c){var i={};i.nonInteraction=r,t&&o&&(i["dimension"+t]=o),window.ga(c+".send","event",e,n,a,i)};trackerName?c(trackerName):UC.poller([function(){return window.ga.getAll}],function(){trackerName=window.ga.getAll()[0].get("name"),c(trackerName)})}var trackerName;
    ...
    // -----------------------------------------------
        ...
        UC.poller([
            ...
            UC.poller([
				'body',
				'#postcodeLookupResultsContainer',
                function() {
                ...
                function TP050() {
					...
					//stop test running twice
					var body = document.getElementsByTagName('body')[0];
					if (body.className.match('TP050mobile')) return false;

               var $ = window.jQuery;
                $('body').addClass('TP050mobile ');

				sendEvent('TP050 Find Address Prominance Mobile', 'Page View', 'TP050', true);

				var selectedAddress = $('#postcodeLookupResultsContainer #selectedAddress'),
					selectedEvent;

					selectedAddress.click(function(){
						if(!selectedEvent){
							sendEvent('TP050 Find Address Prominance Mobile', 'Page View', 'TP050 User clicked on select address', true);
							selectedEvent = true;
						}
					});
