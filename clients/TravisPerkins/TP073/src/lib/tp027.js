    ...
    var _TP027 = (function() {
        ...
        var _options = {
            ...
            textDeliveryInfoCopy: [
                ...
                '<p>Please note that all deliveries will be delivered to the kerbside only</p>'
            ].join(''),
            ...
            imgPadlock: '//sb.monetate.net/img/1/581/1065414.png',
            cardLogos: [
                {
                    img: '//sb.monetate.net/img/1/581/1065263.png',
                    title: 'Visa'
                },
                {
                    img: '//sb.monetate.net/img/1/581/1065269.png',
                    title: 'Mastercard'
                },
                {
                    img: '//sb.monetate.net/img/1/581/1065270.png',
                    title: 'Maestro'
                }
            ],
            secureLogos: [
                {
                    img: '//sb.monetate.net/img/1/581/1078866.jpg',
                    title: 'Secured by QuoVadis'
                },
                {
                    img: '//sb.monetate.net/img/1/581/1065328.png',
        ...
        function run() {
            ...
            // --------------------------------------------------------------------
            $('.pickup-order-group, .pick-and-pack-order-group, .delivery-order-group, .divert-delivery-order-groups').each(function() {
            ...
            // --------------------------------------------------------------------
            $('.delivery-order-group, .pick-and-pack-order-group').each(function() {
            ...
            var totalIncVat = parseFloat(totalPriceElm.text().trim().replace('Â£', '').replace('£', '').replace(',', ''));
          
            var deliveryRow = $('.total_price_wrapper .basketDelivery');
            var deliveryElm = $('.total_price_wrapper .basketDelivery .sessioncamhidetext');
            var deliveryIncVat = parseFloat(deliveryElm.text().trim().replace('Â£', '').replace('£', '').replace(',', ''));
            ...
            // If the VAT = Â£0.00, force hide the VAT toggle
            if(!vatValue) {
            ...
            // total_estimation wrapper breaks total_price_wrapper...
            // TP haven't yet rolled out the 'vouhcer' that belongs within this wrapper
            $('.basket .total_price_wrapper').insertBefore($('.basket .total_estimation:first'));
            ...
            */
            ...
            $('.total_estimation').insertBefore('.tp27_end-actions');
            ...
            */
            ...
            // Secure badges
            // removed 13/07/2017 at request of client DD
            /*var secureBox = $([
            ...
            // Spend Â£xx.xx to qualify for free delivery
            // NB only apply to orders where standard delivery is applicable (i.e. non-bulky or special products)
            // condition for this is that the delivery price is Â£5.00
            var subtotalValue = parseFloat( $('.basketSubtotal .sessioncamhidetext').text().replace(/[Â£,]/, '') );

            var standardDeliveryApplies = false;
            var deliveryAmount = parseFloat( $('.basketDelivery .sessioncamhidetext').text().replace(/[Â£,]/, '') );
            if(deliveryAmount.toFixed(2) == 12.00 || deliveryAmount.toFixed(2) == 12.01 || deliveryAmount.toFixed(2) == 11.99) {
            ...
            // Amend 3/11/17 - Don't show if free delivery required spend is negative (i.e. basket value is above 50 but it still shows a delivery cost)
            if(standardDeliveryApplies && freeDeliveryRequiredSpend > 0) {
                var spendBox = $([
                    '<div class="tp27_free-delivery-spend">',
                        'Spend &pound;' + freeDeliveryRequiredSpend + ' ex VAT more to qualify for free delivery',
                    '</div>'
                ].join(''));
          ...
          // Delivery fix due to delivery banners being added
            // --------------------------------------------------------------------
          
          $('.delivery-entry-button:first').insertAfter('.consignment-header');
          $('.all-delivery .delivery-entry-button').not(':first').hide();
          $('.collection-entry-button .delivery-entry-button').hide();
            // Events
            // -----------------------------------------------------------
            var trackerName = ga.getAll()[0].get('name');
             
                  
            ga(trackerName + '.send', 'event', 'TP027---Basket', 'Did-Load-Test');
