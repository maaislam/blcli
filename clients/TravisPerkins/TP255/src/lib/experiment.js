...
const init = (mutation) => {
  if (location.pathname !== '/' || document.querySelectorAll('[data-test-id="account-menu-wrapper"]').length > 0) {
  ...
  const anchorElem = document.querySelector('[data-test-id="prompt-to-login-benefits"]');
  const controlPromoTitle = document.querySelector('[data-test-id="prompt-to-login-title"]');
  const mainWrapper = document.querySelector('[class^="styled__LoginInfoWrap-sc"]');
  mainWrapper?.classList.add(`${ID}__loginbanner-left`);
  controlPromoTitle.parentElement.classList.add(`${ID}__hide`);
  anchorElem.classList.add(`${ID}__hide`);
  anchorElem.insertAdjacentHTML('afterend', loginPromo(ID));
};
