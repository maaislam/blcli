// ----------------------------------------------------
// TP028 Category Best sellers - mobile
// ----------------------------------------------------
var TP028m = (function() {
...
    }
    ...
    UC.poller([
        ...
        function() {
            return document.querySelectorAll('#tp_product_lister_enumeration .product_item').length >= 4;
        },
        function () {
            if (window.jQuery) {
                return true;
    ...
    function run() {
        ...
        var products = $('#tp_product_lister_enumeration .product_item');
        var bestsellingProducts;

        console.log(products.length);

        if(products.length <= 5) {
            bestsellingProducts = products.slice(0,2);
            sendEvent('num-bestsellers-shown', 2);
        } else {
            bestsellingProducts = products.slice(0,4);
            sendEvent('num-bestsellers-shown', 4);
        }

        var catTitle = $('#breadcrumb li:last').text().trim();

        // Create a bestsellers container
        $('.tp_prodViewWrapper').before([
        ...
        // Week commencing
        $('.tp28-bestsellers-container__heading h2').append([
            '<span class="tp28-commencing">',
                'w/c ' + getLastSunday(),
            '</span>'
        ].join(''));
        ...
        function getLastSunday() {
            ...
            var resultDate = new Date(d.setDate(diff));

            return (resultDate.getDate() + '/' + (resultDate.getMonth() + 1) + '/' + (resultDate.getFullYear()));
        }
        ...
        // Layout amends
        $('.tp28-bestsellers-container .product_item').each(function() {
            var productUrl = $(this).find('.ui-link').attr('href');
