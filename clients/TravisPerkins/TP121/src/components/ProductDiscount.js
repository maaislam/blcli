...
export default class ProductDiscount {
  ...
  calcDiscount() {
    const { cache } = this;
    if (!cache.subtotal) cache.subtotal = Number(cache.product.querySelector('.itm_total p:last-child').innerText.replace(/£|,|\(Ex VAT\)/g, '').trim());
    if (!cache.productQty) cache.productQty = Number(cache.product.querySelector('input[name="quantity"]').getAttribute('value'));
    if (!cache.rrpTotal) cache.rrpTotal = cache.rrp * cache.productQty;
  ...
  create() {
    ...
    element.classList.add('TP121_ProductDiscount');
    ...
    if (ProductDiscountVal !== '0.00') {
      element.innerHTML = `
        <p class="TP121_ProductDiscount__header">Discount</p>
        <span class="TP121_ProductDiscount__price">£${parseFloat(ProductDiscountVal).toFixed(2)}</span>
        <span class="TP121_ProductDiscount__saving">(${this.cache.percentageDiscount}% off)</span>
      `;
    }
  ...
  render() {
    this.cache.product.querySelector('.basket-item-content').append(this.cache.component);
  }
}
