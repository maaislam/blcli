    ...
    Search: {
      ...
      bindEvents: function bindEvents(element) {
        ...
        const updateResults = () => {
          ...
          if (postcode) {
            events.send(Experiment.settings.ID, 'Submit', 'User sorted by nearest');
        ...
        search.addEventListener('click', () => {
          events.send(Experiment.settings.ID, 'Click', 'User clicked search for course dates');
        });
      },
      ...
      updateDistances: function updateDistances(postcode) {
        ...
        services.getUserLatLng(postcode, (userLocationData) => {
          ...
          events.send(Experiment.settings.ID, 'View', 'Venues sorted by nearest');
        }, () => {
          // Error
          events.send(Experiment.settings.ID, 'Error', 'Error occurred with user location');
          [].forEach.call(venues, (el) => {
            const distanceEl = el.querySelector('.RC031_distance');
            if (distanceEl) {
