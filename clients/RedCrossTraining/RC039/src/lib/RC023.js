  ...
  // -----------------------------------------------
    ...
    function rc018() {
      ...
      shareBarwrap.each(function () {
        $(this).find('.rc18-shareBox:last').click(function () {
          ...
          if (!sharelinkevent) {
            sendEvent('RC039', 'click', 'Red Cross Training — Search Results Iteration - User clicked share url link', true);
            sharelinkevent = true;
          }
        });
      ...
      $('.course-result-name').each(function () {
        ...
        $(this).parent().find('.rc18-shareBar').find('.rc18-shareBox:first').click(function () {
          ...
          var emailclickevent;
          if (!emailclickevent) {
            sendEvent('RC039', 'click', 'RC039 - User clicked share by email', true);
        ...
        $('.rc18-urlcopy').each(function () {
          $(this).find('.rc18-copyButton').click(function () {
            ...
            var URLclickevent;
            if (!URLclickevent) {
              sendEvent('RC039', 'click', 'User clicked share by URL, URL is '+newEmailURL, true);
              URLclickevent = true;
            }
          });
        });
    ...
    function rc023(){
      var labelBar = $([
          ...
          '</div>'].join(''));
      labelBar.prependTo('.table-responsive');
      ...
      // Amend the first aid for baby and child content
      const descriptions = document.querySelectorAll('.course-result-description');

      for (let index = 0; index < descriptions.length; index += 1) {
        const element = descriptions[index];
        if(element.textContent.trim().indexOf('Perfect for parents or carers of babies and children') > -1){
          const newText = document.createElement('div');
          newText.classList.add('RC023-smalltext');
          newText.innerHTML = 'Please note attendance on our courses is limited to adults only';
    ...
    poller(['.rc18-courseListingGroup'], ()=>{
      rc023();
    });
}
