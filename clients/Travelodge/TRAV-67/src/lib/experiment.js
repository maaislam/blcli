...
const copyLinkToClipboard = (button) => {
  ...
  fireEvent(`Click - user copied link: ${document.getElementById(`${ID}-share--linkboxinput`).value} to clipboard using the ${button} button`, true);
  ...
  if (!document.querySelector(`.${ID}-share--copycompletetext`)) {
    document.querySelector(`.${ID}-share--copybutton`).insertAdjacentHTML('afterend', `<p class="${ID}-share--copycompletetext">Link copied to clipboard</p>`);
  }
...
const startExperiment = () => {
  ...
  let theCopyingURL = VARIATION == 1 ? window.location.href + `${window.location.href.indexOf('?') > -1 ? `&` : `?`}blcopy=${ID}` : "https://www.travelodge.co.uk" + decodeURIComponent(window.globalDataLayer.ocb1) + `${window.globalDataLayer.ocb1.indexOf('?') > -1 ? `&` : `?`}blcopy=${ID}`;
  let fbCopyingURL = `http://www.facebook.com/dialog/send?app_id=700976514890467&name=${encodeURIComponent(`A post from Travelodge`)}&link=${theCopyingURL}&redirect_uri=${encodeURIComponent(theCopyingURL)}`;
  if(window.outerWidth < 992) {
    fbCopyingURL = `fb-messenger://share/?link=${encodeURIComponent(theCopyingURL)}`;
  }

  console.log(fbCopyingURL);
  ...
  let newShareBoxHTML = `
    ...
    <div class="${ID}-share">
      ...
      <div class="${ID}-share--button ${window.location.href.indexOf('/ocb/') > -1 ? `ocb-button` : `normal-button`}"><span class="${ID}-share--buttonsvg"><svg fill="#000000" width="20px" height="20px" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path d="M8.137,11.631a.908.908,0,0,1-.17.038A3.016,3.016,0,0,1,8,12a2.962,2.962,0,0,1-.033.328.949.949,0,0,1,.173.042L16.9,16.485a.991.991,0,0,1,.242.18A2.969,2.969,0,0,1,19,16a3.037,3.037,0,1,1-2.911,2.306c-.014-.006-.029,0-.044-.01L7.289,14.181a.953.953,0,0,1-.134-.1,3,3,0,1,1,0-4.162,1,1,0,0,1,.133-.1L16.045,5.7c.014-.007.029,0,.044-.011A2.93,2.93,0,0,1,16,5a3,3,0,1,1,3,3,2.969,2.969,0,0,1-1.862-.665,1.03,1.03,0,0,1-.242.18Z"/></svg></span><span class="${ID}-share--buttontext">Share</span></div>
  ...
  if(window.location.href.indexOf('/ocb/') > -1) {
    ...
    pollerLite(['.hero__content-right'], () => {

      document.querySelector('.hero__content-right').insertAdjacentHTML('afterbegin', newShareBoxHTML);
...
const addTracking = (placement) => {
  ...
  [].slice.call(allShareButtons).forEach((button) => {
    button.addEventListener('click', () => {
      fireEvent(`Click - user clicked the: ${button.getAttribute('data-share-button')} share button to go to link: ${button.href}`, true);
    });
  });
