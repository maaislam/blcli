...
const homeBookingShortcut = (ID, bookingObj, index) => {

    let prefix = '';
    const url = window.location.href;
    
    if (url.includes('https://www.travelodge.co.uk/')) {
        prefix = 'https://www.';
    } else if (url.includes('uat.dev.travelodge.co.uk')) {
        prefix = 'https://uat.dev.';
    } else if (url.includes('ddm.dev.travelodge.co.uk')) {
        prefix = 'https://ddm.dev.';
    }

    console.log('prefix', prefix);  
    const isStaging = window.location.hostname.includes('uat.dev');

    // const bookingObjDummy = {
    //     location: 'London',
    //     checkIn: '2021-01-01', 
    //     checkOut: '2021-01-02',
    //     surname: 'johnson',
    ...
    //       },
    //       {
    //         confirmationNumber: '01257212',
    //         rate: 'Saver Rate',
    //         roomType: 'Double Room',
    //         roomExtras: ['Wifi', 'Breakfast', 'Dinner'],
    //       },
    //       {
    //         confirmationNumber: '01257213',
    //         rate: 'Saver Rate',
    //         roomType: 'Double Room',
    //         roomExtras: ['Wifi', 'Breakfast', 'Dinner'],
    //       },
    ...
    const homeBookingShortcutHTML = `
        <div class="${ID}-home-booking-shortcut ${ID}-booking-${index}">
            ...
            <div class="${ID}-home-booking-shortcut-body">
                ...
                <div class="${ID}-home-booking-shortcut-body-right">
                    ${bookingObj.rooms.map((room, index) => {
                        const bookingLink = `${prefix}travelodge.co.uk/manage-bookings?confirmationNo=${room["confirmationNumber"]}&surname=${bookingObj.surname}&journey=share`;
                        return `
                        <div class="${ID}-home-booking-shortcut-body-right-room ${index >= 1 ? `${ID}-display-none` : ''}">
