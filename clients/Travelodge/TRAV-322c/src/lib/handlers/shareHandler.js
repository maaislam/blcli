...
const shareInput = (ID) => {
    const shareInputBtn = document.querySelector(`.${ID}-share-container .${ID}-share-button`);
    shareInputBtn.addEventListener('click', () => {
        console.log('Share button clicked');
        document.querySelector(`.${ID}-share--social`).style.display = 'block';
...
const copyLinkToClipboard = (ID) => {
    navigator.clipboard.writeText(
        document.querySelector(`.${ID}-share-input`).value
    );
    document
...
const shareFunctionality = (ID) => {
    ...
    let theCopyingURL = document.querySelector(`.${ID}-share-input`).value;
  ...
  `;
        ...
        pollerLite([`#main .${ID}-share-container .${ID}-share-button`], () => {
            document
                .querySelector(
                    `#main .${ID}-share-container .${ID}-share-button`
                )
...
export {shareInput, copyLinkToClipboard, shareFunctionality};