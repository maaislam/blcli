...
export default () => {
	...
	//   const alreadyAddedButton = el.querySelector(`.${ID}-mobile-quick-button`);
	...
	const fetchProductData = async (url) => {
		...
		const menu = [];
		const pid = data.querySelector(
			".product-add-to-cart form input#pid"
		)?.value;
	...
	const renderNewModal = (modal) => {
		const productURL =
			modal.querySelector(".view-details").href + "#tabReviews";
