export const observeDOM = (targetSelectorString, callbackFunction, configObject) => {
    ...
    const observer = new MutationObserver((mutations) => {
        mutations.forEach((mutation) => {
            ...
            if (mutation.target.classList.contains('select-address') && mutation.type === 'childList' && (!href.includes('shipping?dwcont') || !href.includes('checkout/gift-options'))) {
                callbackFunction(mutation);
            } else if (href.includes('shipping?dwcont') || href.includes('checkout/gift-options')) {
                callbackFunction(mutation);
            }
