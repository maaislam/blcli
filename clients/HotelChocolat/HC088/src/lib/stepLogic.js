...
export default () => {
    ...
    if (currentTab === 0) {
        ...
        const makeActive = (e) => {
            ...
            const matchingSlide = document.querySelector(`.${ID}-productSlider .${ID}-image[attr="${activeElID}"]`);
            window.jQuery(`.${ID}-productSlider .${ID}-images`).slick('slickGoTo', parseFloat(matchingSlide.getAttribute('data-slick-index')));
    ...
    const checkDiscount = () => {
        ...
        for (let index = 0; index < allSelected.length; index += 1) {
            ...
            if(element.classList.contains(`${ID}-kit`) || element.classList.contains(`${ID}-flake`)) {
                return true;
            }
        }
    }
        ...
        const ajaxAdd = () => {
            ...
            if (allSelected) {
                ...
                for (let index = 0; index < allSelected.length; index += 1) {
                    ...
                    // if(sentDataLayer === false) {
                    //     sentDataLayer = true;
                    //     dataLayerPush(storedProducts);
                    // }
