...
export default () => {
		...
		for (let index = 0; index < kitproduct.length; index += 1) {
			...
			element.addEventListener("click", () => {
				...
				if (element.classList.contains(`${ID}-selected`)) {
					...
					updatePrice();
					fireEvent(
						`Clicked to remove a starter kit and removed ${
					...
					updatePrice();
					fireEvent(
						`Clicked to add a starter kit and choose ${
							element.querySelector(`.${ID}-name`).textContent
						}`
					);
				}
	...
	if (currentTab === 0) {
		...
		const makeActive = (e) => {
			...
			if (isStatus) {
				fireEvent(
					`Clicked Colour option and choose ${selectedColor?.getAttribute(
						"title"
					)} Edition`
				);
			} else {
		...
		const ajaxAdd = () => {
			...
			if (allSelected) {
				...
				for (let index = 0; index < allSelected.length; index += 1) {
					...
					fireEvent(
						`Click - User clicks add to bag CTA with options: ${names}`
					);
				}
			}
		};
