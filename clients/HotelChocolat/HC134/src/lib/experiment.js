...
import { hc120_varA } from "./helper/hot-214/hc120_varA";
...
export default () => {
  ...
  if (!isCheckoutPage) {
    pollerLite(["#navigation", "#main-header"], () => {
      ...
      navBar.querySelector(".icon.pin")?.closest(".mb-grid-item")?.classList.add("location-item");
      const mainHeader = document.querySelector("#main-header");
      mainHeader?.querySelector("div.hlp-centered-wrapper")?.classList.add("main-search-wrapper");
      const imageUrl = mainHeader.querySelector(".primary-logo .icon.logo-mobile");
      ...
      navBar.querySelector(".mb-grid-item.help-nav")?.insertAdjacentHTML("afterend", content);
      ...
      if (links.length > 0) {
        links.forEach((item) => {
          item.addEventListener("click", () => {
            fireEvent(`User clicks on navigation item ${item.innerText.trim() ? item.innerText.trim() : item.getAttribute("href")}`);
          });
        });
      }

      const toggleMenu = document.querySelector("#hamburger-menu a.menu-toggle");
      ...
      window.onscroll = function (e) {
        ...
        let stickyMenuScrollTopOffset = document.querySelector("#header-search")?.getBoundingClientRect().bottom;
        ...
        } else if (st > 10 && st > lastScrollTop + stickyMenuScrollPixelVal) {
          document.body.classList.remove(`${ID}-search-bar-shown`);
