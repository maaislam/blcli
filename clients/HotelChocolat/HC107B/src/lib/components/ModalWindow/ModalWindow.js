...
class ModalWindow {
	...
	#init() {
		...
		this.el.innerHTML = /* html */ `
          ...
          <button class="${ID}-modal__close ${ID}-modal__close__main">
						<span>Close modal</span>
					</button>
				<div class="${ID}-modal__inner">
          <button class="${ID}-modal__close ${ID}-modal__close__fake">
            <span>Close modal</span>
          </button>
		...
		const closeButton = this.el.querySelector(`.${ID}-modal__close__main`);
		closeButton.addEventListener("click", () => {
			this.onClose();
			this.el.remove();
		});

		const closeButton2 = this.el.querySelector(`.${ID}-modal__close__fake`);
	...
	#removeLoader() {
		this.loader.remove();
		this.el
			.querySelector(`.${ID}-modal__close__main`)
			.classList.add(`show`);
		this.el
			.querySelector(`.${ID}-modal__close__fake`)
			.classList.add(`hide`);
	}
	...
	#renderData(data) {
		this.#removeLoader();
		this.el
			.querySelector(`.${ID}-modal__inner`)
			.classList.add("shown-content");
		const modalContainer = this.el.querySelector(`.${ID}-modal__container`);
		...
		if (addToBasketButton) {
			...
			addToBasketButton.addEventListener("click", () => {
				if (
					...
					document.querySelector(".HC089-ageBox") &&
					document
						.querySelector(".HC089-ageBox")
						.classList.contains("HC089-modalHide")
				) {
					document
						.querySelector(".HC089-ageBox")
						.classList.remove("HC089-modalHide");
					document
						.querySelector(".HC089-overlay")
						.classList.remove("HC089-overlayHide");
				} else {
