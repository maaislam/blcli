 ...
 */
 const discountPrice = (currentPrice, discount) => {
    const parseprice = parseFloat(currentPrice.textContent.replace('£', ''));
    const newPrice = parseprice.toFixed(2) - discount;
...
export const QuickView = (el, productPrice) => {
    document.querySelector(`#QuickViewDialog .price-sales`).style.display = 'none';
    ...
    setTimeout(() => {
        ...
        document.querySelector('#QuickViewDialog .price-sales').style.display = 'block';
        document.querySelector('#QuickViewDialog .price-sales').innerHTML = elPrice.innerHTML;

        reduceDropDownPrices(plpSku, document.querySelector(`#QuickViewDialog .price-sales`));
...
export const hidePromotions = (el, sku) => {
    ...
    if(allPromotionText && alcoholSKUs.indexOf(sku) === -1) {
        if (allPromotionText.querySelector('.promotion-callout') && allPromotionText.querySelector('.promotion-callout').textContent.trim().indexOf('MIX & MATCH') > -1) {
...
export const checkProducts = (elToLoop) => {
    ...
    for (let index = 0; index < elToLoop.length; index += 1) {
        ...
        if (element.querySelector('#pid')) {
            ...
            sku = JSON.parse(element.nextElementSibling.value).impression_product_SKU;
        } else if (element.querySelector('input') && JSON.parse(element.querySelector('input').value).impression_product_SKU) {
            sku = JSON.parse(element.querySelector('input').value).impression_product_SKU;
        } else if (element.querySelector('input') && JSON.parse(element.querySelector('input').value).productID) {
            sku = JSON.parse(element.querySelector('input').value).productID;
        } else if (element.querySelector('.mini-cart-product-remove')) {
            sku = element.querySelector('.mini-cart-product-remove .button-text').getAttribute('data-pid');
        }
        ...
        Object.keys(products).forEach((i) => {
            ...
            if (datasku === sku) {
                ...
                if(element.classList.contains('cart-row')) {
                    ...
                    element.querySelector('.price-sales').style.display= "none";
                }
                ...
                if (VARIATION === '2') {
                    priceDeduction = data.v2;
                }
                if (VARIATION === '3') {
                    priceDeduction = data.v3;
                }
                ...
                if (!element.classList.contains(`${ID}-priceChanged`)) {
                    ...
                    setTimeout(() => {
                        if(element.classList.contains('cart-row')) {
                            element.querySelector('.item-total').style.display= "table-cell";
                            element.querySelector('.price-sales').style.display= "table-cell";
                        }
                    }, 100);
                }
            }
...
const checkVoucher = () => {
    ...
    if(allVouchersApplied) {
        ...
        for (let index = 0; index < allVouchersApplied.length; index++) {
            ...
            // if error
            if (document.querySelector('.coupon-error') && document.querySelector('.coupon-error').textContent.indexOf(voucherCode) > -1) {
                apply = false;
            }

            if (element.querySelector('.applied-coupon-code').textContent.indexOf(voucherCode) > -1) {
                apply = false;
            } 
...
const basketNotice = () => {
    ...
    document.querySelector('.order-totals-table').insertAdjacentElement('afterend', basketMessage);
    ...
    if(allVouchersApplied) {
        ...
        for (let index = 0; index < allVouchersApplied.length; index += 1) {
            ...
            if (element.querySelector('.applied-coupon-code').textContent.indexOf(voucherCode) > -1) {
                ...
                if(document.querySelector('.cart-coupon-code .toggle-title').classList.contains('on')) {
                    document.querySelector('.cart-coupon-code .toggle-title').click();
                }
                if(VARIATION !== '1') {
                    basketNotice();
                }
