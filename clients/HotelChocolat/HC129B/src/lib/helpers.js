...
export const klarnaAdditon = () => {
	document
		...
		.insertAdjacentHTML(
			"beforeend",
			' <klarna-placement data-key="top-strip-promotion-standard" data-locale="en-GB"></klarna-placement>'
		);
	...
	if (reviewLink) {
		reviewLink.addEventListener("click", (e) => {
			...
			if (window.innerWidth <= 767) {
				if (
					!document
						.querySelector(".reviews.tab-mobile-title")
				...
				scrollToElement(
					document.querySelector(".reviews.tab-mobile-title")
				);
			} else {
				if (document.querySelector(`.${ID}-tab.${ID}-reviews`)) {
					const desktopReview = document.querySelector(
						`.${ID}-tab.${ID}-reviews`
					);
...
export const updatePrice = () => {
	...
	const total = parseFloat(colourPrice + kitPrice);
	if (VARIATION == "2") {
		document.querySelector("#price-with-Kit").innerHTML = `£${total}`;
	} else {
	...
	// 	`.${ID}-addToBagMobile .${ID}-price span`
	// );
	// if (fixedPrice) {
...
export const plus = () => {
	if (document.querySelector(`.${ID}-quantity-input`)) {
		var value = parseInt(
			document.querySelector(`.${ID}-quantity-input`).value,
			10
		);
