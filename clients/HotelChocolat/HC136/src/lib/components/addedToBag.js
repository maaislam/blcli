...
export default () => {
	...
	const addedToBagMessage = () => {
		...
		addedProductMessage.innerHTML = `<div class="${ID}-innerText">
		...
		document
			.querySelector("#main")
			.insertAdjacentElement("afterbegin", addedProductMessage);
	...
	const addProductsFromStorage = () => {
		...
		if (products.length > 1) {
			document.querySelector(`.${ID}-productsAdded`)?.insertAdjacentHTML(
				"afterbegin",
				`<p class="promo-msg">
					Don't forget! Use <b>VELVKIT15OFF</b> at checkout
				</p>`
			);
		}
		Object.keys(products).forEach((i) => {
			...
			addedItem.innerHTML = `${data.productName} <span class="${ID}-addedToPrice">£${data.price}</span>`;

			document
				.querySelector(`.${ID}-addedBox .${ID}-products`)
				.appendChild(addedItem);
		});
	};
