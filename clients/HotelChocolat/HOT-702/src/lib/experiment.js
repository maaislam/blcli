    ...
    <div class="${ID}-atc-holder ${ID}-hidden">
      ...
      <div class="${ID}-atc">
        ...
        <div class="${ID}-atc--checkout">
          ...
          <p class="${ID}-totals"><span class="${ID}-subtotal">Subtotal</span><span class="${ID}-total">${
        document.querySelector('.mini-cart-wrapper .subtotal')
          ? document.querySelector('.mini-cart-wrapper .subtotal').innerText
          : '£0.00'
      } </span></p>
          <p class="${ID}-items"><span class="${ID}-subitems">${document.querySelector('.minicart-total-qty').innerText} ${
        parseInt(document.querySelector('.minicart-total-qty').innerText) > 1 ? 'items' : 'item'
      }</span> in your basket</p>
      ...
      <div class="${ID}-upsells upsell-2 ${ID}-upsells--v${VARIATION}" style="display:none;">
        <div class="${ID}-upsellcontainer ab-atc-upsells ab-atc-upsells--mightalsolike ab-atc-upsells--mightalsolike--v${VARIATION}"></div>
      </div>
