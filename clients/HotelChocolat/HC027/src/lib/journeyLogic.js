...
export default () => {
    ...
    const chooseFlakes = () => {
      ...
      const kitproduct = document.querySelectorAll(`.${ID}-accordionStep.${ID}-flakesSlider .${ID}-product`);
     ...
     /* const makeActive = (e) => {
        ...
        const price = parseFloat(e.currentTarget.querySelector(`.${ID}-price`).textContent.replace('£', ''));
          ...
          if(e.currentTarget.classList.contains(`${ID}-selected`)) {
            ...
            events.send(`${ID} varation: ${VARIATION} - Velvetiser Journey`, 'click', `starter kit: ${packName}`);
            ...
            element.addEventListener('click', () => {
              ...
              const price = parseFloat(element.querySelector(`.${ID}-price`).textContent.replace('£', ''));
                if(element.classList.contains(`${ID}-selected`)) {
                    ...
                    updatePrice();
                    events.send(`${ID} varation: ${VARIATION} - Velvetiser Journey`, 'click', `added sachet box: ${packName}`);
                }
            });
          }
    ...
    const chooseAccessories = () => {
      // choose flakes
      const product = document.querySelectorAll(`.${ID}-accordionStep.${ID}-accessories .${ID}-product`);
        ...
        element.addEventListener('click', () => {
          ...
          const price = parseFloat(element.querySelector(`.${ID}-price`).textContent.replace('£', ''));
            if(element.classList.contains(`${ID}-selected`)) {
                ...
                updatePrice();
                events.send(`${ID} varation: ${VARIATION} - Velvetiser Journey`, 'click', `added accessory: ${packName}`);
            }
        });
      }
    }
    ...
    if(currentTab === 0) {
        ...
        const makeActive = (e) => {
            ...
            events.send(`${ID} varation: ${VARIATION} - Velvetiser Journey`, 'step', `saw step - starter kits`);
            events.send(`${ID} varation: ${VARIATION} - Velvetiser Journey`, 'step', `saw step - accessories`);
