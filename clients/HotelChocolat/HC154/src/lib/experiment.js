...
const init = () => {
  ...
  const shopAllUrls = ['/shop/christmas', '/shop/collections', '/shop/gift-ideas', '/shop/collections/products/the-velvetiser'];
  const shopAllTexts = ['Christmas', 'Chocolate Collections', 'Gift Ideas', 'Velvetiser'];
  shopAllUrls.forEach((url, i) => {
    const anchorPoint = document.querySelector(`.has-sub-menu[href*="${url}"] + .content-asset ul.sub-level`);
    anchorPoint?.insertAdjacentHTML(
      'beforeend',
      `
        <li class="sub-level-item ${ID}__shopall">
          <a class="has-sub-menu-level-21 ${ID}__shopallUrl" href="https://www.hotelchocolat.com/uk${url}/">Shop all ${shopAllTexts[i]}</a>
        </li>
      `
    );
  });
...
export default () => {
  ...
  document.body.addEventListener('click', (e) => {
    const { target } = e;
    if (target.closest(`.${ID}__viewAllUrl`)) {
      sessionStorage.setItem('isViewAllClicked', 'true');
      fireEvent('Conditions Met');
    } else if (target.closest('.icon.menu-icon')) {
      fireEvent('Conditions met');
  ...
  if (VARIATION == '1') {
    ...
    if (!document.querySelector(`.${ID}__menu-header`)) {
      ...
      document.querySelectorAll(`.has-sub-menu-level-2:not(.${ID}__shopallUrl)`).forEach((item) => {
