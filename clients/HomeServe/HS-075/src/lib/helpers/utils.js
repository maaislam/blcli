...
export const formatText = (text) => {
  ...
  // If the text contains 'in year one', separate it into two parts
  if (formattedText.includes('in year one')) {
    // Split the text at 'in year one' and trim both sides
    let parts = formattedText.split('in year one');
    result.mainText = parts[0].trim() + ' ' + parts[1].trim();
    result.anotherText = 'in year one';
  } else if (formattedText.includes('Introductoryoffer')) {
    result.mainText = 'Introductory offer';
    result.anotherText = '';
  } else if (formattedText.includes('Limitedoffer')) {
    result.mainText = 'Limited offer';
  }
