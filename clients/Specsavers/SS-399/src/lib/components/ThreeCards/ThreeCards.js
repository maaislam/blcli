...
function Card(title, image, text) {
	...
	<div class="${ID}-three-cards__card-content ${title.length > 0 ? "three-rows" : ""}">
		${
			title.length > 0
...
export default function ThreeCards(title, cards, ctaText, ctaLink) {
	const element = document.createElement("div");
	element.classList.add(`${ID}-three-cards`);
	element.innerHTML = /* html */ `
	<div class="${ID}-three-cards__container ${ID}-container">
	${title.length > 0 ? /* html */ `<h2 class="${ID}-three-cards__title">${title}</h2>` : ""}
		<ul class="${ID}-three-cards__list"></ul>
		${
			ctaText.length > 0
	...
	cards.forEach((c) => element.querySelector(`ul`).append(Card(c.title, c.image, c.text)));
