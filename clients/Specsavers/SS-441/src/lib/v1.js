...
import { plusIcon, dashIcon } from "./assets";
...
export default function v1() {
	(function renderHeroBanner() {
		...
		const newEl = HeroBanner(
			...
			"https://blcro.fra1.digitaloceanspaces.com/SS-441/v1.png",
			"Book your scan",
			"/book/location",
			"",
			"Get the most advanced eye test to catch problems before they affect your vision"
		);
	...
	(function renderThreeCards() {
			...
			[
				{
					...
					image: "https://blcro.fra1.digitaloceanspaces.com/SS-441/layers.svg",
				...
				},
				{
					...
					image: "https://blcro.fra1.digitaloceanspaces.com/SS-441/check.svg",
				...
				},
				{
					...
					image: "https://blcro.fra1.digitaloceanspaces.com/SS-441/pound.svg",
		...
		if (currentEl) {
			...
			(function removeOtherContent() {
				...
				document
					.querySelectorAll(".sib-text-multiple-column")[0]
					.remove();
			})();
		}
	})();
	...
	(function renderTwoColumnLists() {
		const currentEls = document.querySelectorAll(
			".sib-text-multiple-column, .sib-cta-ribbon"
		);
		const newEl = TwoColumnLists("Do I Need an OCT Scan?", [
			...
			},
			{
				...
				description:
					"We recommend an OCT scan even if you have an annual diabetic check up:",
				items: [
					{
	...
	(function renderThreeCards() {
		const currentEls = document.querySelectorAll(".sib-cards");
		const newEl = ThreeCards(
			"What should I do?",
			[
				{
					title: "",
					image: "https://blcro.fra1.digitaloceanspaces.com/SS-441/1.svg",
					text: "Click <strong>“book a scan”</strong> below and select an OCT scan when we ask you.",
				},
				{
					title: "",
					image: "https://blcro.fra1.digitaloceanspaces.com/SS-441/2.svg",
					text: "An OCT scan allows your optician to identify eye problems before they start to affect your vision.",
				},
				{
					title: "",
					image: "https://blcro.fra1.digitaloceanspaces.com/SS-441/3.svg",
					text: "From only £10, you will get a full understanding of your eye health.",
				},
			],
			"Book your scan",
	...
	(function editFaqElement() {
		...
		const data = [
			...
			{
				question: "Is it like having an MRI?",
				answer: "No, it’s very different from having an MRI; You simply sit in a chair and look into the device for a matter of seconds.",
			},
			{
				question: "Does it take long?",
				answer: "No, the scan itself will take a matter of seconds!",
			...
			{
				question: "What do I get from having this Scan?",
				answer: "Even if your optician does not find any health concerns, we can use your OCT scans to see the change in your eye health over time, allowing us to identify concerns much faster.",
			},
		];
		const iconDom = `<div class="sib-accordion-icon__container">
				<div class="sib-accordion-icon__inactive">
					<span class="plus-icon-svg">${plusIcon}</span><span class="icon-title">Read more</span>
				</div>
				<div class="sib-accordion-icon__active">
					<span class="plus-icon-svg">${dashIcon}</span><span class="icon-title">Read less</span>
				</div>
			</div>
		</div>`;
		if (currentEl) {
			currentItems.forEach((item, idx) => {
				if (data[idx]) {
					const question = item.querySelector(
						"h3.sib-accordion-question"
					);
					const answer = item.nextElementSibling.querySelector(
						".sib-accordion-answer p"
					);

					question.textContent = data[idx].question;
					answer.textContent = data[idx].answer;
					item.querySelector("i").remove();
					item.insertAdjacentHTML("beforeend", iconDom);
				} else {
					item.remove();
				}
			});
	...
	(function renderThreeCards() {
		...
		const newEl = ThreeCards(
			...
			[
				{
					title: "",
					image: "https://blcro.fra1.digitaloceanspaces.com/SS-441/machine.svg",
				...
				{
					title: "",
					image: "https://blcro.fra1.digitaloceanspaces.com/SS-441/cake.svg",
