...
export default function Wizard({ nodes }) {
  ...
  const didAnswer = (val) => {
    ...
    if (decision === "outcome-success-full") {
      fireEvent("Outcome - Eligible Free");
    } else if (decision === "outcome-success-paid") {
      fireEvent("Outcome - Eligible Private");
    } else if (decision === "outcome-failure") {
      fireEvent("Outcome - Don't Qualify");
    }
    ...
    <div className={`${ID}-wizard`} data-open={isOpenState}>
      <div className={`${ID}-wizard__header`}>
        ...
        <img
          ...
          src="https://blcro.fra1.digitaloceanspaces.com/SS-306/XS_Specsavers_Logo_opt.svg"
        ></img>

        <h2 className={`${ID}-wizard__title`}>Home Eye Test Eligibility</h2>
      </div>
      ...
      {getNodeById(activeStep).markup ? (
        ...
        >
          ...
          {getNodeById(activeStep).extraMarkup ? (
            <div>{getNodeById(activeStep).extraMarkup}</div>
          ) : (
            ""
          )}
        </div>
