 ...
 */
window.ucHelpers = window.ucHelpers || {};
const experiments = window.ucHelpers.experiments = (window.ucHelpers.experiments || {});
...
export const destroyPollers = () => {
  if((window.ucHelpers.experiments[settings.ID] || {}).pollers) {
    const pollers = window.ucHelpers.experiments[settings.ID].pollers;
    for(let i = 0; i < pollers.length; i++) {
        pollers[i].destroy();
    }

    window.ucHelpers.experiments[settings.ID].pollers = [];
  }
};
...
export const destroyIntervals = () => {
  if((window.ucHelpers.experiments[settings.ID] || {}).intervals) {
    const intervals = window.ucHelpers.experiments[settings.ID].intervals;
    for(let i = 0; i < intervals.length; i++) {
        clearInterval(intervals[i]);
    }

    window.ucHelpers.experiments[settings.ID].intervals = [];
  }
};
...
export const killAllEventListeners = () => {
  ...
  window.ucHelpers.experiments[settings.ID].eventListeners = [];
};
...
export const killObservers = () => {
  ...
  window.ucHelpers.experiments[settings.ID].observers = [];
};
