...
export default class Modal {
  ...
  openModal() {
    ...
    setTimeout(() => {
      ...
      if($slides.find('img').length <= 3) {
        document.querySelector(`.${this.componentName}_slides__btn--prev`).remove();
        document.querySelector(`.${this.componentName}_slides__btn--next`).remove();
      } else {
        document.querySelector(`.${this.componentName}_slides__btn--prev`)
          .addEventListener('click', () => mySiema.prev());

        document.querySelector(`.${this.componentName}_slides__btn--next`)
  ...
  createModal(productData, Qty, addProductToCart, viewProduct) {
    ...
    const imageSlides = imageUrlsToCheck.map((image) => {
      return `
      ...
      <div class="${componentName}" class="ng-scope">
          <div class="${componentName}_wrap">
            <div class="${componentName}_titleBlock">
              <div ng-click="viewProduct(product)" class="${componentName}_title">${productData.Name}</div>
            </div>
            ...
            <div class="${componentName}_detailsBlock">
              <div class="${componentName}_details">
                <div class="${componentName}_priceBlock">
                  ${productData.ListPrice && productData.IsOnSale ? `<div class="${componentName}_wasPrice">${productData.ListPriceFormatted}</div>` : ''}
                ...
                <div class="${componentName}_ctaBlock">
                  ...
                  ${(productData.Availability==1 && productData.HasActiveVariant && !productData.SingleVariantSku && productData.IsShadeVariant) ?
    ...
    angularCompile($modalWrapper, $, $modalScope, () => {
      $modalScope.$apply(() => {
        ...
        $modalScope.viewProduct = (product) => {
          viewProduct(product);
          events.send(this.ID, 'Click', 'View product button in modal');
        };
        $modalScope.addProductToCart = (product, Qty) => {
          this.closeModal();
          addProductToCart(product, Qty);
          events.send(this.ID, 'Click', 'Add to bag button in modal');
        };
      })
      this.openModal();
    });
