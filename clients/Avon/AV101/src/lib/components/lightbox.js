...
const renderLightbox = (id, productsData) => {
  const getTotal = (prev = false, discount = 0) =>
    formatPrice(
      productsData.reduce(
  ...
  const discount = productsData.length > 2 ? 0.5 : 0;
  const prevTotal = getTotal('prev');
  const currTotal = getTotal('', discount);

  const htmlStr = `
    
    <div class="${id}__lightbox--wrapper">
        <div class="${id}__lightbox ${id}__width--${productsData.length}">
            <div class="${id}__lightbox--headline-block"><span>Added to Basket!</span><span>Why not try something new with a sample?</span><span class="${id}__close-btn">${crossIcon}</span></div>
            ...
            <div class="${id}__lightbox--buyall-block ${productsData.length < 2 ? `${id}__hide` : ''}">
                ...
                <div class="${id}__total-add-to-bag ${id}__btn">Add ${productsData.length} samples to basket</div>
            </div>
        </div>
    </div>
