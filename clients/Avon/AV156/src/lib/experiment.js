    ...
    hamburgerBtn.addEventListener('pointerup', () => {
      if (document.body.classList.contains('AG060a')) {
        ...
        pollerLite(['nav#HamburgerMenuNew ul#HamburgerCategories a[href*="/top-rated"]'], () => {
          const topRatedLink = document.querySelector('nav#HamburgerMenuNew ul#HamburgerCategories a[href*="/top-rated"]');
...
export default () => {
  ...
  const pollingSelector = isDesktop() ? 'header #HeaderSubmenus ul a[href*="/top-rated"]' : 'header #Hamburger';
