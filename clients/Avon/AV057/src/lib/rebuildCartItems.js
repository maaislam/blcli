 ...
 */
          ...
          <div ng-if="Layout.Name == 'Phone'" class="Cart-Product" ng-repeat="product in campaign.Products">
            ...
            <div class="${ID}-img-and-title">
              ...
              <div class="Cart-ProductName">
                <div ng-bind="(product.LoneVariant.VariantLineNo ? product.LoneVariant.VariantLineNo + ' - ' : '') + product.Name"></div>
        ...
        <div class="${ID}-comp">
          ...
          <div class="${ID}-comp__inner">
            <div class="${ID}-comp__go">
              <h2 class="${ID}__home-header">${_t('Home Delivery')}</h2>
              <p>${_t('Checkout online for delivery by a courier')}.</p>
              ...
              <div class="${ID}-comp__logos">
                <img width="50" height="30" src="/media/2976519/maestro.png" alt="Maestro"> 
                <img width="50" height="30" src="/media/2976520/mastercard.png" alt="MasterCard"> 
                <img width="50" height="30" src="/media/2976525/visa.png" alt="Visa"> 
                <img width="50" height="30" src="/media/2976557/paypal-logo-20141.png" alt="PayPal">
              </div>
            ...
            <div class="${ID}-comp__sendto">
              <div id="SectionShare" class="share-options">
                <div ng-show="!shareSheetsSupport || false">
                    <h2 class="${ID}-sendh2">${_t('Send to your Avon Rep')}</h2>
  ...
  if(!document.querySelector(`.${ID}-comp`)) {
    ...
    // AV057 - add desktop home delivery banner
    pollerLite([
      `.Layout_Desktop`
    ], ()=> {
      const homeDelHeaderDesktop = document.querySelector(`.${ID}-comp__inner`);
      if (homeDelHeaderDesktop) {
        const homeDeliveryBanner = `
          <div class="${ID}__home-del-banner">
            Free delivery on orders over £20
          </div>
        `;
        homeDelHeaderDesktop.insertAdjacentHTML('afterbegin',homeDeliveryBanner);
      }
    })

    // AV057 mobile changes
    pollerLite([
      '.Layout_Phone'
    ], () => {

      // Delivery banner changes
      const homeDeliveryHeading = document.querySelector(`.${ID}__home-header`);
      if (homeDeliveryHeading) {
        const homeDeliveryBanner = `
          <div class="${ID}__home-del-banner">
            Free delivery on orders over £20
          </div>
        `;
        homeDeliveryHeading.insertAdjacentHTML('afterend', homeDeliveryBanner);
      }

      // Handle sharing section
      pollerLite([
        '.AG033-comp__or'
      ], () => {
        const orText = document.querySelector('.AG033-comp__or');
        if (orText) {

          const sharingSect = document.querySelector('.share-options');
          if (sharingSect) {
            sharingSect.classList.add(`${shared.ID}--hidden`);
          }

          const sendToMarkup = `
            <div class="${shared.ID}__rep-send">
              <button class="Button ${shared.ID}__rep-send__btn">
                Send to your Avon Rep
              </button>
            </div>
          `;
          orText.insertAdjacentHTML('afterend', sendToMarkup);

          const sendToRepBtn = document.querySelector(`.${shared.ID}__rep-send__btn`);
          if (sendToRepBtn) {
            sendToRepBtn.addEventListener('click', () => {
              const sharingSection = document.querySelector('.share-options');
              if (sharingSection) {
                sharingSection.classList.remove(`${shared.ID}--hidden`);
              }
            })
          }
