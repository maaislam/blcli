...
export default () => {
  ...
  const addContinueShoppingButton = () => {
    ...
    if (!componentExists) {
      const $component = $(`
        <a class="Button vi-btn vi-btn--secondary ${ID}_continueShopping" href="/" data_action="continueshopping" data_nav="cart">
    ...
    if (!componentExists) {
      const $component = $(`
        <div class="CartHeader-middle ${ID}_cartSummaryTop">
          <ul>
            <li><em>${translate('Artikel')}:</em> <span ng-bind="CartData.NumberOfItemsInCart"></span></li>
            <li class="${ID}_totalPriceTop"><em>${translate('Zwischensumme')}*:</em> <span ng-bind="(CartData.TotalPrice | currency)"></span></li>
          </ul>
        </div>
      `);
      ...
      $title.text(translate('Warenkorb'));
  ...
  const updateTotalPrices = () => {
    ...
    if (!componentExists) {
      ...
      // Change total text
      $('.Cart-SubTotalLabel > span').text(`${translate('Zwischensumme')}*:`);
  ...
  const init = () => {
    ...
    if (isRussianSite) {
      const $cartBottomActions = $('.Cart-BottomActions');
      $cartBottomActions.prepend(`<p class="${ID}_infoText">Доступные способы доставки можно выбрать при оформлении заказа</p>`);
    }
  };
