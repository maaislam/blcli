...
export default () => {
        ...
        <div class="${ID}_cartSummaryTop">
          <div class="${ID}_cartSummaryTopLeft">
            <div class="${ID}_cartTitle">
              <h2><span class="${ID}__mobileTitle">${translate('Warenkorb')}</span></h2>
              <div class="${ID}__mobileQty"><span ng-bind="CartData.NumberOfItemsInCart"></span> <em>${translate('Artikel')}</em></div>
            </div>
          </div>
  ...
  const productChanges = (index, element) => {
    ...
    $removeLinks.each((i, el) => {
      ...
      const $updateLink = $(`<div class="${ID}_updateLink" ng-click="UpdateCart(0)" ng-show="variant.QuantityChanged || product.QuantityChanged">Warenkorb aktualisieren</div>`);
    ...
    if (productScope.product.Savings) {
      ...
      const $savings = $(`
        <div class="${ID}_productSavings" ng-show="HasProductSavings(product)">
          ${translate('Du sparst')} <span ng-bind="(product.Savings | currency)"></span> (<span ng-bind="(((product.Savings / product.RegularPrice).toFixed(2) * 10 * 10).toFixed(0))"></span>%)
        </div>
      `);
    ...
    if (!componentExists) {
      ...
      <div>
        <div class="mm-t09-SubTotal-Wrapper">
          <div class="${ID}__mobileTotal">
            <span>${translate('Zwischensumme')}* (<span ng-bind="CartData.NumberOfItemsInCart"></span>)</span>
          </div>
        ...
        <div class="mm-t09-Savings-Wrapper">
          <div class="${ID}__mobileSavings">
            <span>${translate('Du sparst')}</span>
          </div>
        ...
        <div class="Cart-SubTotal-Wrapper">
          <div class="Cart-SubTotalLabel">
            <span>${translate('Zwischensumme')}*</span>
          </div>
      ...
      const updateProductSavings = (value) => {
        ...
        if(value == 0) {
          $savingsWrap.addClass('xhide');
        } else {
          $savingsWrap.removeClass('xhide');
        }
      };
        ...
        if (cartCoupongWrapper && cartHaveCouponInput && !cartHaveCouponInput.value) {
          cartCoupongWrapper.insertAdjacentHTML('afterbegin', `
            <div class="mm-t09-promocode">
              <span>+</span> &nbsp;${translate('Gutscheincode eingeben')}
            </div>
          `);
  ...
  const changeLoschen = () => {
    const removeWraps = document.querySelectorAll('.Cart-ProductRemove');
    if (removeWraps) {
      [].forEach.call(removeWraps, (wrap) => {
        const span = wrap.querySelector('span');
        if (span) {
          span.innerText = 'Löschen';
        }
      })
    }
  };

  const movePaymentOptions = () => {
    pollerLite(['.ContentRow'], () => {
      const paymentOptions = document.querySelector('.ContentRow');
      if (paymentOptions) {
        const cartFooter = document.querySelector('.Cart-Footer');
        if (cartFooter) {
          cartFooter.insertAdjacentElement('afterend', paymentOptions);
          paymentOptions.style.marginTop="20px";
        }
      }
    })
  }
  ...
  const init = () => {
    ...
    // Helper add additional text
    const isRussianSite = /https?:\/\/my\.avon\.ru\/*/.test(window.location.href);
    if (isRussianSite) {
      const $cartButtonsBottom = $('.Cart-ButtonsBottom');
      $cartButtonsBottom.after(`<p class="${ID}_infoText">Доступные способы доставки можно выбрать при оформлении заказа</p>`);
    }
  };
