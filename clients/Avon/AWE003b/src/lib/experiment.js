      ...
      if(search && searchBar) {
        ...
        document.querySelector('#MainContentWrapper').addEventListener('click', () => {
          if(document.body.classList.contains(`${ID}-active`)) {
            ...
            const searchSpan = search.querySelector('span');
            if(searchSpan.innerText.trim() == translate('Suche')) {
              searchSpan.innerHTML = translate('Schließen');
            } else {
              searchSpan.innerHTML = translate('Suche');
            }
          }
        });
      }
      ...
      if (logoBar) {
        const markup = `
          <div class="${shared.ID}__rep-bar">
            <span class="${shared.ID}__rep-bar__text">Online Shop Von: <span class="${shared.ID}__rep-bar__name">${firstName} ${lastName}</span></span>        
          </div>
        `;
