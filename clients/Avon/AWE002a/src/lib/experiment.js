...
/**
 * AWE002a - Foundation Specific PDP
 *
...
export default () => {
  ...
  const shadeSelectorChanges = () => {
    ...
    modifyTemplate(
      ...
      $template => new Promise((resolve) => {
        $template
          .find('[ng-repeat="variant in availableShadeVariants"]')
          .replaceWith(`
        ...
        $template.find('.Shades > .Group > .GroupHeader').before(`
          <div class="${ID}_SelectedShade">
            <div class="${ID}_SelectedShadeText">
              <div class="${ID}_SelectedShadeLabel">Gewählter Farbton:</div>
  ...
  const productSpecificChanges = () => {
        ...
        const changes = () => {
          ...
          sections.forEach((section) => {
            ...
            shades.forEach((shade) => {
              const $shade = $(`.${ID}_shade:has(.${ID}_shadeName:contains("${shade}"))`);
              if ($shade.length) {
                $shadeSection.find(`.${ID}_shadeSectionShades`).append($shade);
              }
            });
    ...
    const thisProductChanges = productChanges[getProductSkuPDP()];
