    ...
    if(hamburger) {
      ...
      if(search && searchBar) {
        ...
        search.addEventListener('click', () => {
          ...
          if(searchSpan.innerText.trim() == translate('Chiudi')) {
            searchSpan.innerHTML = translate('Cerca');
          } else {
            searchSpan.innerHTML = translate('Chiudi');
          }
      ...
      if (logoBar) {
        const markup = `
          <div class="${shared.ID}__rep-bar">
            <span class="${shared.ID}__rep-bar__text">Stai ordinando con <span class="${shared.ID}__rep-bar__name">${firstName} ${lastName}</span></span>        
          </div>
        `;
