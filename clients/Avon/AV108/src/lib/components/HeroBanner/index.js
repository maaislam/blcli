...
export const HeroBanner = () => {
  ...
  useEffect(() => {
    findElement('.js--title').then((element) => {
    ...
    window.onscroll = () => {
      ...
      if (!modal) {
        if (st >= 0 && window.pageYOffset >= 490) {
          ...
          slideUp(header, 200);
          document.querySelector('.AV093 #hero-banner .CTA').classList.add('cta-fixed');
        } else if (window.pageYOffset < 400 && header.style.display === 'none') {
          slideDown(header, 200);
        }
        if (window.pageYOffset <= 480) {
          document.querySelector('.AV093 #hero-banner .CTA').classList.remove('cta-fixed');
        }
      } else {
        disableScroll();
      }
    ...
    <div className='background-wrapper-top'>
      <div className='background-wrapper--img'>
        <div className='container'>
          ...
          <div className='body-text--text'>It's time to start earning money doing what you love.</div>
          <span className='CTA'>
            <ModalComponent />
          </span>
        </div>
  ...
  return <div id='hero-banner'>{showOnMobileAndTablet ? MobileDesign : DesktopDesign}</div>;
};
