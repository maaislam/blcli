      ...
      <div class="${name}--summary">
        ${hasShades ? `
          <h2 class="${name}--titleShade">
            Gewählter Farbton: <span class="${name}--shadeName" ng-bind="(ViewModel.SelectedShadeVariant.Name)"></span> <br/>
            (<span class="${name}--changeShade">ändern</span>)
          `
        :
          `<h2 class="${name}--title">
            ${window.innerWidth <= 321 ? shortName : Product.Name}
          </h2>
          `}
        </h2>
