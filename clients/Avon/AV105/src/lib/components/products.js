...
const renderProdlist = (data, parentElm, testId, headlineText) => {
  ...
  const delIcon = `<svg aria-hidden="true" class="icon icon-delete" viewBox="0 0 13 15"><path d="M2.621 14.025 1.761 3.7h9.478l-.86 10.325a.3.3 0 0 1-.3.275H2.92a.3.3 0 0 1-.299-.275z" stroke="#BEBEBE" stroke-width="1.4"></path><path fill="#BEBEBE" d="M4 0h5v1.4H4zM0 3h13v1.4H0z"></path></svg>`;
  const getSession = sessionStorage.getItem('deletedItems');
  const isAllIdPresentInDelSession =
    JSON.parse(getSession)?.reduce((prev, curr) => {
      const isIdpesent = data.some((item) => (item.activeVariant ? item.activeVariant.id === curr : item.variants[0].id == curr));
      if (isIdpesent) {
        prev.push(isIdpesent);
      }
      return prev;
    }, []).length === JSON.parse(getSession)?.length || false;
  const userDeletedAll = getSession && JSON.parse(getSession)?.length == data.length && isAllIdPresentInDelSession;
  console.log('userDeletedAll', userDeletedAll);
  const htmlStr = (info, index) => {
    const { activeVariant, available, type, compare_at_price, featured_image, id, ratingData, price, title, url, variants } =
      info;

    const initialActiveVariant = activeVariant || variants[0];

    const hasBeenDeleted = getSession && sessionStorage.getItem('deletedItems').indexOf(initialActiveVariant.id) !== -1;
    ...
    return `
            <div class="swiper-slide ${hasBeenDeleted ? `${testId}__hide` : ''}" >
                  
                <div class="${testId}__prodcard" data-parentid = "${initialActiveVariant.id}">
                    <div class="${testId}__prodcard--image-container ${index === 0 ? 'first-item' : ''}">
                        <a href="${url}" class="${testId}__img--wrapper">
                            <img width="144" height="144" src="${featured_image}" alt="${title}" />
                        </a>
                    </div>
                    <div class="${testId}__detail-wrapper">
                    <div class="${testId}__prodcard--title"><a href="${url}">${title}</a></div>
                    ${ratingStars(testId, ratingData)}
                    <div class="${testId}__prodcard--subtitle ${variants.length <= 1 ? `${testId}__hide` : ''}"><a href="${url}">
                      <span class="active-shadeimage" style="background-image:url(${
                        initialActiveVariant['featured_image']?.src
                      }) "></span>
                      <span class="active-shadename">${initialActiveVariant.title}</span>
                    </a></div>
                    ...
                    <div class="${testId}__cartbtn--container">
                        <div class="quantity">
                        <div class="variant-selected ${initialActiveVariant['featured_image'] ? '' : 'w-100'} ${
      variants.length <= 1 ? `${testId}__hide` : ''
    }"><div class="${testId}__shade-img ${
      initialActiveVariant['featured_image'] ? '' : 'size-variant'
    }" style="background-image:url(${initialActiveVariant['featured_image']?.src})">${
      !initialActiveVariant['featured_image'] ? 'Select Type' : ''
                    ...
                    <div class="quantity_wrapper ${type.indexOf('Bundle') !== -1 ? `${testId}__invisible` : ''}">
                        <span class="btn-quantity ${testId}__minus-btn">
                        ...
                        <span class="btn-quantity-inc ${testId}__plus-btn">
                        ...
                        <div class="add-to-cart mobile-show" data-sku="${initialActiveVariant.id}">${
      type.indexOf('Bundle') !== -1 ? `Buy it Now` : 'Add'
    }</div>
                    </div>
                    
                    </div>
                    <div class="${testId}__action-btns">
                      <div class="${testId}__delete-btn">${delIcon}</div>
                      <div class="add-to-cart ${testId}__desktop-cart-btn " data-sku="${initialActiveVariant.id}">${
      type.indexOf('Bundle') !== -1 ? `Buy it Now` : 'Add'
    }</div>
                    </div>
                </div>
  ...
  parentElm.insertAdjacentHTML(
    'beforeend',
    `<div class="${testId}__prodcards swiper 
    }"><h2 class="${testId}__section-title" >${headlineText}</h2>${renderMobileQuantity(
      testId
    )}<div class="swiper-wrapper">${cards}</div></div>`
  );

  document.querySelectorAll(`.swiper-slide.${testId}__hide`).length === data.length
    ? document.querySelector(`.${testId}__cards-wrapper`).classList.add(`${testId}__hide`)
    : '';
};
