var RC016v2 = (function () {
	...
	var _triggers = (function () {
		...
		UC.poller([
			'.form-item.form-item-date.form-date-to',
			function () {
				return window.jQuery;
			},
			function () {
	...
	function activateVal() {
		if(window.location.href.indexOf('https://www.redcrossfirstaidtraining.co.uk/Where-we-train/EventsSearch.aspx') !== -1){
		}	
		else{
			...
			var dateSplit = $('.form-item.form-item-date.form-date-to input.datepicker').val().split("/"),
				dateSplitYear = 20 + dateSplit[2],
				dateAssemble = dateSplitYear + '/' + dateSplit[1] + '/' + dateSplit[0],
				newDate = addDays(dateAssemble, 60),
				newDay = newDate.getDay().toString(),
				newYear = newDate.getFullYear().toString(),
				newMonth = newDate.getMonth() + 1,
				newMonthString = newMonth.toString();

			$('.form-item.form-item-date.form-date-to input.datepicker').val(newDay + '/' + newMonthString + '/' + newYear);
		
		}
	};
		
	function activate() {
		$('body').addClass('RC016v2');
		...
		sendEvent('RC016---Course Listing Usability Variation 2', 'Landed on EventsSearch.aspx page', '', true);
		...
		$('.RC016_course_tgl').on('click', function (e) {
			e.preventDefault();
			sendEvent('RC016---Course Listing Usability Variation 2', 'Change course button clicked', 'Button to the right of main header', true);
		...
		courseBtn.on('click', function (e) {
			e.preventDefault();
			sendEvent('RC016---Course Listing Usability Variation 2', 'Change course button clicked', 'Button above Can\'t decide line', true);
			if (formMain.hasClass('RC_reveal')) {
				selectWrap.slideUp();
				dateWrap.slideUp();
				submitWrap.slideUp();
		...
		courseDateBtn.on('click', function (e) {
			e.preventDefault();
			sendEvent('RC016---Course Listing Usability Variation 2', 'Change dates button clicked', '', true);
			if (formMain.hasClass('RC_reveal')) {
				selectWrap.slideUp();
				dateWrap.slideUp();
				submitWrap.slideUp();
		...
		courseLocationBtn.on('click', function (e) {
			e.preventDefault();
			sendEvent('RC016---Course Listing Usability Variation 2', 'Change location button clicked', '', true);
			if (formMain.hasClass('RC_reveal')) {
				selectWrap.slideUp();
				dateWrap.slideUp();
				submitWrap.slideUp();
		...
		$('.form-btnAutoSubmit.button-primary.js-location-submit').on('click', function(){
			sendEvent('RC016---Course Listing Usability Variation 2', 'Location Submit Clicked', '', true);
		});
	};
})();