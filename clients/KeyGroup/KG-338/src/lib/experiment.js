...
const startExperiment = () => {
  pollerLite(['#HousePrice'], () => {
    ...
    document.body.addEventListener('click', (e) => {
      if (e.target.classList.contains(`${ID}-equitycalc--button`) || e.key === 'Enter') {
        e.preventDefault();
        moveForwardsStage();
      }
...
const moveForwardsStage = () => {
  ...
  if (currStage == 1) {
    ...
    if (
      !document.querySelector('.form__field-wrapper[data-type="emailfield"]').classList.contains('form__field-wrapper--error')
    ) {
